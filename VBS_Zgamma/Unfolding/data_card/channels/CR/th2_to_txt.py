...
fdir = '/home/pku/anying/cms/PKU-Cluster/Unfolding/common/CR/channels/root/'
...
genbinerror=[]
arr={}
f=open('/home/pku/anying/cms/PKU-Cluster/Unfolding/Uncer/channels/CR/summary_'+sys.argv[2]+"_"+sys.argv[3]+sys.argv[4]+'CR.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
for i in range(1,nbins):
   f = open('./txt/%s_%s%s_control_bin%i.txt'%(sys.argv[2],sys.argv[3],sys.argv[4],i),'w')
...
#   data=th1_DMuon.GetBinContent(i)+th1_DEle.GetBinContent(i)
   ...
   for j in range(1,nbins):
          f.write('genbin%i_Stat_contorl_recobin%d_%s%s\tlnN\t'%(j,i,sys.argv[3],sys.argv[4]))
          if j==1:
   ...
   f.write('QCD_Stat_contorl_recobin%d_%s%s\tlnN\t'%(i,sys.argv[3],sys.argv[4]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('%0.2f\t-\t-\t-\t-\t-\n'%(ZA_binerror))

   f.write('non_prompt_Stat_contorl_recobin%d_%s%s\tlnN\t'%(i,sys.argv[3],sys.argv[4]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t%0.2f\t-\t-\t-\t-\n'%(non_prompt_binerror))

   f.write('TTA_Stat_contorl_recobin%d_%s%s\tlnN\t'%(i,sys.argv[3],sys.argv[4]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t-\t%0.2f\t-\t-\t-\n'%(TTA_binerror))

   f.write('VV_Stat_contorl_recobin%d_%s%s\tlnN\t'%(i,sys.argv[3],sys.argv[4]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t-\t-\t%0.2f\t-\t-\n'%(VV_binerror))

   f.write('ST_Stat_contorl_recobin%d_%s%s\tlnN\t'%(i,sys.argv[3],sys.argv[4]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t-\t-\t-\t%0.2f\t-\n'%(ST_binerror))


   f.write('ZA_SigOut_Stat_contorl_recobin%d_%s%s\tlnN\t'%(i,sys.argv[3],sys.argv[4]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t-\t-\t-\t-\t%0.2f\n'%(ZA_sig_out_binerror))
