...
//        file2<<t_ZA[i]->GetBinContent(k+1)*scale_factor[i]<<endl;}
     ...
     t_ZA[0]->GetYaxis()->SetRangeUser(0,1.3*vec_ymax[0]);
     t_ZA[0]->Draw("HIST");
...
//          t_ZA[i]->Scale(scale_factor[i]);
       ...
       if(filename.Contains("EWK"))nominal->GetYaxis()->SetRangeUser(0.9,1.1);
       else nominal->GetYaxis()->SetRangeUser(0.7,1.25);
       nominal->Draw();
       h_up->Draw("same hist ][");
