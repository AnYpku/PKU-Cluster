...
//     tree->SetBranchStatus("*",0);
     ...
     for(int j=0;j<kk;j++){
	     if(name.Contains("EWK")==0){
		     fout[j]= new TFile("unfold_"+vec_branchname[j]+"_qcdOut_pdf"+tag+".root","recreate");
	     }
	     else{
...
//		if(tag[i].Contains("17")) continue;
		...
		file2[i]=new TFile(dir[i]+"unfold_GenCutla-ZA"+tag[i]+".root");
//		run(file1[i],recovars, bins,cut1,tag[i]);
		if(tag[i].Contains("16"))run(file2[i],recovars, bins,cut1,tag[i]);
	}
	return 1;
}
