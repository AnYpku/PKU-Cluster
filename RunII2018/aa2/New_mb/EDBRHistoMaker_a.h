...
void EDBRHistoMaker::Init(TTree *tree) {
	...
	treename->Branch("realjet1", &realjet1, "realjet1/I");
	treename->Branch("realjet2", &realjet2, "realjet2/I");
...
//data
                ...
                if(  (drll>0.3 &&drla>0.7 && drla2>0.7 && lep == 13 &&  (HLT_Mu1 > 0||HLT_Mu2>0||HLT_Mu3>0) && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep > 70. && massVlep < 110. && photonet > 20. &&( (fabs(photoneta) < 1.4442) /*|| ( fabs(photoneta)<2.5&&fabs(photoneta)>1.566 )*/ ) && ZGmass>100 && ( ((jet1pt>50&&fabs(jet1eta)<4.7)||(jet1pt>30&&jet1pt<50&&fabs(jet1eta)<4.7&&jet1puIdLoose==1)) && ((jet2pt>50&&fabs(jet2eta)<4.7)||(jet2pt>30&&jet2pt<50&&fabs(jet2eta)<4.7&&jet2puIdLoose==1)) ) /*(  (jet1pt>50 && fabs(jet1eta)<4.7) &&  (jet2pt>50 && fabs(jet2eta)<4.7)  )*/ && drj1a>0.5 &&drj2a>0.5 && drj1l>0.5 && drj2l>0.5 && drjj>0.5 && drj1l2>0.5&&drj2l2>0.5 && Mjj>500 && zepp>2.4 ) 
		  ) {
		...
		if (isnotwets > 0 || iswjets > 0 || iszjets > 0 || isttjets > 0) {
			...
			if(Mjj<500)(theHistograms["Mjj"])->Fill(Mjj, actualWeight);
...
//			continue;
		...
		if (theWeight > 0)
			...
			nn = -1;
		actualWeight = pileupWeight*scalef*photon_id_scale*photon_veto_scale;//mc
		detajj = fabs(jet1eta - jet2eta);
		if (fabs(jet1phi-jet2phi)>Pi) drjj = sqrt((jet1eta-jet2eta)*(jet1eta-jet2eta)+(2*Pi-fabs(jet1phi-jet2phi))*(2*Pi-fabs(jet1phi-jet2phi)));
                else drjj = sqrt((jet1eta-jet2eta)*(jet1eta-jet2eta)+(fabs(jet1phi-jet2phi))*(fabs(jet1phi-jet2phi)));
		...
		actualWeight = actualWeight*(muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale)*muon_hlt_scale*puIdweight_L;
...
// mc
	...
	cout << "after cut: " << numbe_out << "; actualweight" << actualWeight<<"; theWeight>0 "<<npp<<"; theWeight<0 "<<nmm<<"; (npp-nmm) = "<<npp-nmm<<"; yields = "<<(npp-nmm)*scalef*59.7<<endl;
		cout<< " result " << sum <<"; yields "<<sum*59.7<<endl;
	this->saveAllHistos(outFileName);
}
