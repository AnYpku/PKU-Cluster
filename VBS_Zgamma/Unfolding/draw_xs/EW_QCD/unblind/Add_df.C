...
void run(TString var, TString recovar, TString title,TString tag){
	...
	TFile*f4=new TFile("../../../pdf_draw/unfold_"+var+"_interf16.root");
	TFile*f5=new TFile("../../../pdf_draw/unfold_"+var+"_interf17.root");
	TFile*f6=new TFile("../../../pdf_draw/unfold_"+var+"_interf18.root");
	...
	TH1D*hist_clone4=(TH1D*)f4->Get(var+"_0");
	TH1D*hist_clone5=(TH1D*)f5->Get(var+"_0");
	TH1D*hist_clone6=(TH1D*)f6->Get(var+"_0");
	hist_clone->Add(hist_clone2,1);hist_clone->Add(hist_clone3,1);
        ...
        hist_clone->Add(hist_clone4,1);hist_clone->Add(hist_clone5,1);hist_clone->Add(hist_clone6,1);
	...
	if(!f_in.is_open())cout<<"can not open the file "<<"/home/pku/anying/cms/PKU-Cluster/Unfolding/data_card/EW_QCD/unblind/combine/r_"+recovar+"_"+tag+".txt"<<endl;
...
//	cout<<"after scale "<<h1[0]->GetBinContent(1)<<" "<<h1[0]->GetBinError(1)<<endl;
	...
	fPads2->SetBottomMargin(0.4);
        fPads1->SetLeftMargin(0.14);
        fPads2->SetLeftMargin(0.14);
	fPads2->Draw();
	fPads1->Draw();
	...
	if(var.Contains("Mjj")==0)
		hist_clone->GetYaxis()->SetTitle("#font[132]{d}#font[132]{#sigma}/#font[132]{d}#font[12]{p}_{#font[132]{T}} [fb/GeV]");
	else
		hist_clone->GetYaxis()->SetTitle("d#sigma/dm [fb/GeV]");
	hist_clone->GetXaxis()->SetLabelSize(0.0);
	hist_clone->GetYaxis()->SetLabelSize(0.085);
	hist_clone->GetYaxis()->SetTitleOffset(0.6);
...
//                err_scale=(*biggest - *smallest)/2/h1[0]->GetBinContent(j);
	...
	hist_clone->Draw("E2");
        ...
        for(int i=0;i<hist_clone->GetNbinsX();i++){
           cout<<var<<" bin"<<i<<" "<<r[i]<<" "<<Err_up[i]<<" "<<Err_down[i]<<" "<<fixed<<setprecision(4)<<hist_clone->GetBinContent(i+1)<<" "<<hist_clone->GetBinError(i+1)<<" "<<ybin[i]<<" "<<yerror_up[i]<<" "<<yerror_down[i]<<endl;
	...
	gr1->SetLineColor(1);
        ...
        nominal->GetYaxis()->SetRangeUser(ye_h[pos_l]-ye_l[pos_l]-0.3,ye_h[pos_h]+0.5);
//	nominal->GetYaxis()->SetRangeUser(0.2,2.);
	nominal->Draw("E2");
	gr1->Draw("P Z SAME");
        gr1_sys->Draw("P SAME");
//	line->Draw();
...
int Add_df(){
	gStyle->SetOptStat(0);
	vector<TString> title={"#font[12]{p}_{#font[132]{T}}^{#font[12]{l}}_{#font[132]{1}}","#font[12]{p}_{#font[132]{T}}^{#gamma}","#font[12]{p}_{#font[132]{T}}^{#font[132]{j}}_{#font[132]{1}}"};
