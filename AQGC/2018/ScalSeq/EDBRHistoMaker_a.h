...
//data
                if(  (drll>0.3 &&drla>0.7 && drla2>0.7 && lep == 13 &&  ( HLT_Mu1 > 0|| HLT_Mu2 > 0) && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep > 70. && massVlep < 110. && photonet > 100. &&( (fabs(photoneta) < 1.4442) || ( fabs(photoneta)<2.5&&fabs(photoneta)>1.566 ) )  && jet1pt> 30 && jet2pt > 30 && fabs(jet1eta)< 4.7 && fabs(jet2eta)<4.7 && drj1a>0.5 &&drj2a>0.5 && drj1l>0.5 && drj2l>0.5 && drjj>0.5 && Mjj>500 &&detajj>2.5&& ZGmass>100) ||
...
//	std::cout << "numberofnp:" << npp << "  numberofnm:" << nmm << std::endl;
	...
	double r2=gRandom->Rndm(jentry*2);
		...
		if (theWeight > 0)
			...
			nn = -1;
		actualWeight = lumiWeight * pileupWeight * scalef;//mc
		detajj = fabs(jet1eta - jet2eta);
		if (fabs(jet1phi-jet2phi)>Pi) drjj = sqrt((jet1eta-jet2eta)*(jet1eta-jet2eta)+(2*Pi-fabs(jet1phi-jet2phi))*(2*Pi-fabs(jet1phi-jet2phi)));
                else drjj = sqrt((jet1eta-jet2eta)*(jet1eta-jet2eta)+(fabs(jet1phi-jet2phi))*(fabs(jet1phi-jet2phi)));
		...
		if(lep==13)  actualWeight = actualWeight*photon_id_scale*muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale*photon_veto_scale;
                if(lep==11)  actualWeight = actualWeight*photon_id_scale*ele1_id_scale*ele2_id_scale*ele1_reco_scale*ele2_reco_scale*photon_veto_scale;
