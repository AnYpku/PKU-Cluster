...
fdir = '/home/pku/anying/cms/PKU-Cluster/WWg/Significance/ml1g/CR/root/'
...
genbinerror=[]
arr={}
f=open('/home/pku/anying/cms/PKU-Cluster/WWg/Significance/ml1g/Uncer/Top_CR/summary_uncer_'+sys.argv[1]+'.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
for i in range(1,nbins):
   f = open('./txt/emu_%s_%s_CR_bin%i.txt'%(sys.argv[1],sys.argv[2],i),'w')
...
# bin error
   ...
   f.write('WWG_Stat_CR_bin%d_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2]))
   f.write('%0.3f\t-\t-\t-\t-\t-\n'%(WWg_binerror))
   f.write('VA_Stat_CR_bin%d_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2]))
   f.write('-\t%0.3f\t-\t-\t-\t-\n'%(ZA_binerror))

   f.write('Nonprompt_photon_Stat_CR_bin%d_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2]))
   f.write('-\t-\t%0.3f\t-\t-\t-\n'%(non_prompt_binerror))

   f.write('Nonprompt_lepton_Stat_CR_bin%d_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2]))
   f.write('-\t-\t-\t%0.3f\t-\t-\t-\n'%(fakeL_binerror))

   f.write('VV_Stat_CR_bin%d_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2]))
   f.write('-\t-\t-\t-\t%0.3f\t-\n'%(VV_binerror))

   f.write('Top_Stat_CR_bin%d_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2]))
   f.write('-\t-\t-\t-\t-\t%0.3f\n'%(ST_binerror))
   ...
   f.write('btag\tlnN\t')
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['btag'+'_WWG'][i-1],arr['btag'+'_ZGJets'][i-1],arr['btag'+'_VV'][i-1],arr['btag'+'_Top'][i-1]))

   if sys.argv[1].find("18") == -1:
      f.write('l1pref\tlnN\t')
      f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['pref'+'_WWG'][i-1],arr['pref'+'_ZGJets'][i-1],arr['pref'+'_VV'][i-1],arr['pref'+'_Top'][i-1]))
   ...
   f.write('%0.3f\t-\t-\t-\t-\t-\n'%(arr['WWG_scale_muR1'][i-1]))

   f.write('Scale_muF_Sig\tlnN\t')
   f.write('%0.3f\t-\t-\t-\t-\t-\n'%(arr['WWG_scale_muF1'][i-1]))

   f.write('Scale_muFmuR_Sig\tlnN\t')
   f.write('%0.3f\t-\t-\t-\t-\t-\n'%(arr['WWG_scale_muFmuR'][i-1]))
