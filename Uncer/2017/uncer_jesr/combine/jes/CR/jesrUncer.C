...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   mjj_JEC_down = new TH1D("h_JEC_down","mjj_JEC_down",1,0,1);
   mjj_JEC_up = new TH1D("h_JEC_up","mjj_JEC_up",1,0,1);
   for(int i=0;i<3;i++){
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	   ...
	   if( !( ( drla>0.7 && drla>0.7 && lep == 13 && HLT_Mu2 > 0 && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep > 70. && massVlep < 110. && photonet > 20.&&  ( (fabs(photoneta) < 1.4442 )|| ( fabs(photoneta)<2.5&&fabs(photoneta)>1.566 ) ) && Mva>100 ) ||
...
//	   Mjj_JEC[0] = Mjj;deltaeta_JEC[0] = deltaeta;
	   ...
	   Mjj_JER[0] = Mjj_new;
	   Mjj_JER[1]=Mjj_JEC_up;
	   Mjj_JER[2]=Mjj_JEC_down;
	   ...
	   deltaeta_JER[1]= deltaeta_JEC_up;
	   deltaeta_JER[2]= deltaeta_JEC_down;
