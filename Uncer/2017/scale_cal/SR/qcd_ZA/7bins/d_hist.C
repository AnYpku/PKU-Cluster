...
//  Double_t pileupWeight;
    ...
    Int_t lep;Double_t photoneta,delta_phi,ZGmass;
	...
	for(Int_t j=0; j<t->GetEntries();j++)
	{
        p=0;
	t->GetEntry(j);
        if( !( Mjj>500 && detajj>2.5 && ZGmass>100 && zepp<2.4 && delta_phi>1.9) ) continue;
        for(Int_t i=0;i<9;i++){
...
//	run("/home/pku/anying/cms/file_in_cms/2017RunIIrootfiles/2017cutla/cutla-outZA_pweight.root");
	run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2017/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2017_ZA-pweight.root");
