...
int Build_Hist(){
	...
	for(int ij=0;ij<njets.size();ij++){
		...
		for(int ik=0;ik<types.size();ik++){
				...
				names={"ZGJets","TTGJets","VV","ST","tZq","TGJets","WGJets","WWG_emu"};
				dir1="/home/pku/anying/cms/PKU-Cluster/WWg/CR_plot/SR/output-slimmed-rootfiles/optimal_emua_";
			}
...
//				if(names[j].Contains("WWG_emu")==0) continue;
				if(names[j].Contains("Muon")==0 && names[j].Contains("plj")==0 )
					met="(n_bjets_nom==0 && PuppiMET_T1Smear_pt > 20 && mT2>30 && mT>60 &&"+jet_cut+")";
				else
					met="(n_bjets_nom==0 && PuppiMET_T1_pt > 20 && mT2>30 && mT>60 &&"+jet_cut+")";
				Reco= LEP+"&&"+photon+"&&"+met;
