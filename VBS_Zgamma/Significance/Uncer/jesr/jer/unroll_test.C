...
void unroll_name(TString filename,TString tag){
	...
	TFile* fout = new TFile("aa_"+filename+"_jer"+tag+".root","RECREATE");
	Double_t ZA_scale= 1;
...
//                t_ZA[i]->Scale(1/t_ZA[i]->Integral());
	...
	for(Int_t i=1;i<num;i++){
		...
		else if(i==1)    l2->AddEntry(t_ZA[i],"JER_up");
		else if(i==2)    l2->AddEntry(t_ZA[i],"JER_down");
	}
        ...
        latex.DrawLatex(6.1,0.9*vec_ymax[0],"#Delta#eta_{jj}>6");
...
//	nominal->GetYaxis()->SetRangeUser(h_down->GetMinimum()-0.1,h_up->GetMaximum()+0.05);
	...
	fPads2->Update();
	c1->Print("./figs/hist-2d_"+filename+tag+"_jer.eps");
	c1->Print("./figs/hist-2d_"+filename+tag+"_jer.pdf");
