...
//        cout<<"begin extrcing JEC uncertainty"<<endl;
	...
	JetCorrectionUncertainty jecUnc(JetCorrPar);
        ...
        for (auto isrc=jecAK4chs_source_.begin(); isrc != jecAK4chs_source_.end(); isrc++) {

//                const char *name = *isrc;
                JetCorrectorParameters *p = new JetCorrectorParameters(jecAK4chs_Uncfile_, *isrc);
                JetCorrectionUncertainty *unc = new JetCorrectionUncertainty(*p);
                vsrc.push_back(unc);
        } // for isrc
        //cout<<"size of jecAK4chs_source "<<jecAK4chs_source_.size()<<endl;
	// JER
	...
	for (size_t i = 0; i< jetColl->size(); i++){
		...
		float JERSF_temp        = res_sf.getScaleFactor(jetParam);
                double JER_corrFactor = get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor);
		...
		smearedP4_JEC_down *= get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor);
                //JEC uncertainty from other sources
                //cout<<"get uncertainty form other sources"<<endl;
                std::vector<double> sup,sdw;
                for (auto isrc = vsrc.begin(); isrc != vsrc.end(); isrc++) {
                        JetCorrectionUncertainty *unc = *isrc;
                        unc->setJetPt(smearedP4_JEC_up.pt());
                        unc->setJetEta(jet.eta());
                        sup.push_back(unc->getUncertainty(true)); // up variation
                        unc->setJetPt(smearedP4_JEC_down.pt());
                        unc->setJetEta(jet.eta());
                        sdw.push_back(unc->getUncertainty(false)); // down variation
                } // for isrc
                vsrc.clear();  
                if(  !(sup.size()>0) ){
			for(size_t i=0;i<jecAK4chs_source_.size();i++){
                ...
                //cout<<"begin adding uncertainty"<<endl;
                std::vector<reco::Candidate::LorentzVector> smearedP4_abs_up_raw,smearedP4_abs_down_raw;
		double sum2_up(0), sum2_dw(0);
		for(size_t i=0;i<jecAK4chs_source_.size();i++){
			smearedP4_abs_up_raw.push_back(jetCorrFactor*JER_corrFactor*rawJetP4*(1+sup[i]));
			smearedP4_abs_down_raw.push_back(jetCorrFactor*JER_corrFactor*rawJetP4*(1-sdw[i]));
			jet.addUserFloat(jecAK4chs_source_[i]+"_Ptup",smearedP4_abs_up_raw[i].pt());
			jet.addUserFloat(jecAK4chs_source_[i]+"_Ptdown",smearedP4_abs_down_raw[i].pt());
			jet.addUserFloat(jecAK4chs_source_[i]+"_Eup",smearedP4_abs_up_raw[i].energy());
			jet.addUserFloat(jecAK4chs_source_[i]+"_Edown",smearedP4_abs_down_raw[i].energy());
                        //cout<<jecAK4chs_source_[i]<<" "<<smearedP4_abs_up_raw[i].pt()<<endl;
			sum2_up += pow(std::max(sup[i],sdw[i]),2);
			sum2_dw += pow(std::min(sup[i],sdw[i]),2);
		}
                reco::Candidate::LorentzVector smearedP4_abs_up_tot,smearedP4_abs_down_tot;
                smearedP4_abs_up_tot=jetCorrFactor*JER_corrFactor*rawJetP4*(1+sum2_up);
                smearedP4_abs_down_tot=jetCorrFactor*JER_corrFactor*rawJetP4*(1-sum2_dw);
                jet.addUserFloat("jetValidation_Ptup",smearedP4_abs_up_tot.pt());
                jet.addUserFloat("jetValidation_Eup",smearedP4_abs_up_tot.energy());
                jet.addUserFloat("jetValidation_Ptdown",smearedP4_abs_down_tot.pt());
                jet.addUserFloat("jetValidation_Edown",smearedP4_abs_down_tot.energy());
                //cout<<"add uncertainty done"<<endl;
                sup.clear();sdw.clear();
                smearedP4_abs_up_raw.clear();smearedP4_abs_down_raw.clear(); 
/*                for(size_t i=0;i<sup.size();i++){
                   cout<<i<<" "<<sup[i]<<" "<<sdw[i]<<endl;
		}
                cout<<"print uncertainty done"<<endl;
		double unc_Absolute_up=0,   unc_Absolute_down=0; 
                if(sup.size()>0){
			unc_Absolute_up=sup[0];     unc_Absolute_down=sdw[0]; 
		}
		else    {unc_Absolute_up=0;     unc_Absolute_down=0;}
                sup.clear();sdw.clear();
                
		reco::Candidate::LorentzVector smearedP4_abs_up_raw =jetCorrFactor*rawJetP4;
		reco::Candidate::LorentzVector smearedP4_abs_down_raw =jetCorrFactor*rawJetP4;
		smearedP4_abs_up_raw *= (1+unc_Absolute_up); 
		smearedP4_abs_down_raw *= (1-unc_Absolute_down); 
                cout<<"smearedPt: "
		    <<smearedP4_abs_down_raw.pt()<<endl;
		jet.addUserFloat("smeared_Absolute_up",smearedP4_abs_up_raw.pt());    
		jet.addUserFloat("smeared_Absolute_down",smearedP4_abs_down_raw.pt());    
		jet.addUserFloat("unc_Absolute_up",unc_Absolute_up);    
		jet.addUserFloat("unc_Absolute_down",unc_Absolute_down);    
                cout<<"addUserFloat OK"<<endl;*/
		...
		smearedP4_JEC_down *= (1-jecUncertainty_up);
