...
fdir = '/home/pku/anying/cms/PKU-Cluster/WWg/Significance/root/'
f_Data=TFile.Open(fdir+'hist_WWG_'+sys.argv[1]+'.root')
...
genbinerror=[]
arr={}
f=open('/home/pku/anying/cms/PKU-Cluster/WWg/Significance/Uncer/SR/summary_uncer_'+sys.argv[1]+'.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
for i in range(1,nbins):
   f = open('./txt/emu_%s_bin%i.txt'%(sys.argv[1],i),'w')
...
# bin error
   ...
   f.write('Nonprompt_lepton_Stat_bin%d_%s\tlnN\t'%(i,sys.argv[1]))
   f.write('-\t-\t-\t%0.3f\t-\t-\t-\t-\n'%(fakeL_binerror))
   ...
   f.write('Scale_muR_TTG\tlnN\t')
   f.write('-\t-\t-\t-\t%0.3f\t-\t-\n'%(arr['TTGJets_scale_muR1'][i-1]))

   f.write('Scale_muF_TTG\tlnN\t')
   f.write('-\t-\t-\t-\t%0.3f\t-\t-\n'%(arr['TTGJets_scale_muF1'][i-1]))

   f.write('Scale_muFmuR_TTG\tlnN\t')
