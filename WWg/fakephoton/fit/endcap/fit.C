...
void fit(float lowpt, float highpt, Bool_t isBarrel,TString channel){
	TString c;
        ...
        vector<TFile*>fdata;
        vector<TFile*>ftrue;
        if(channel!="emu")ftrue.push_back(TFile::Open("../../endcap/hist_"+c+channel+"_outZGJets_f18.root"));
        else ftrue.push_back(TFile::Open("../../endcap/hist_"+c+channel+"_outTTGJets_f18.root"));
	...
	for(int i=0;i<data.size();i++){
		...
		fdata.push_back(TFile::Open("../../endcap/hist_"+c+channel+"_"+data[i]+"_f18.root"));
                cout<<"../../endcap/hist_"+c+channel+"_"+data[i]+"_f18.root"<<endl;
	}
