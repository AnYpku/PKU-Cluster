...
//TString dir_file="./";
TString dir_file="/eos/user/y/yian/2018cutla/";
TString dir = "./txt/";
TString dir1 = "./root/";
...
void style();
void histo();
void select(TFile*file,TH1D *h1[ptnumber],TH1D *h2[ptnumber][21],TH1D *h3[ptnumber]);
...
void select(TFile*file,TH1D *h1[ptnumber],TH1D *h2[ptnumber][21],TH1D *h3[ptnumber]){
 ...
 TString filename;
 if(fname.Contains("Jets"))
	 filename="outZJets_FX.root";
 else filename = "outZA.root";
...
// nentries=100000;
 ...
 TFile* fT = new TFile("./copy_"+filename,"recreate");
...
void runname(TString filename){
	...
	histo();
	TFile*file1=new TFile(dir_file+filename);
	TString rootfiles="hist_"+filename;
	TFile*ff1= new TFile(dir1 + rootfiles,"recreate");
	select(file1,h11,h12,h13);
	ff1->cd();
	for(Int_t i=0;i<ptnumber;i++){
...
int main(){
//	runname("copy_outZA.root");
	runname("cutlep-outZA.root");
//	runname("copy_outZJets_FX.root");
//	runname("cutlep-outZJets_FX.root");
}
...
void histo(){
 Int_t bin=40;
 Double_t xlow=  0.00015;
 Double_t xhigh= 0.02015;
  for(Int_t i=0;i<ptnumber;i++){
      ...
      h13[i]=new TH1D(Form("h13_pt%0.f_%0.f",lowpt[i],highpt[i]),"chiso<1.141",bin,xlow,xhigh);////Zjets && leding_photon && medium_cut && photon_chiso>2
    for(Int_t j=0;j<21;j++){  
