...
//      Gen =  GenLEPele  && GenPhoton && GenJet && GenDr && GenSignalRegion;
      ...
      if(name.Contains("EWK") ){ 
              if(Reco){
                      for(int i=0;i<3;i++){
			      hist1[i]->Fill(Mjj,scalef*lumi*pweight[i]);
		      }
		      i1++;
	      }
              if(Gen){
		      ...
		      for(int i=0;i<3;i++){
			      hist4[i]->Fill(genMjj,scalef*lumi*pweight[i]);
		      }
	      }
              if(Reco && Gen){ 
		      i3++;
		      for(int i=0;i<3;i++){
			      hist2[i]->Fill(genMjj,scalef*lumi*pweight[i]);
			      hist3[i]->Fill(genMjj,scalef*lumi*pweight[i]);
		      }
