...
//        fit->Constrain(1,0,1);
	    ...
	    mcp1->Scale(result->Integral()*(1-p0)/mcp1->Integral());//scale mc prediction
            ...
            mcFake = hist->IntegralAndError(0,hist->FindFixBin(0.0102)-1,mcFake_err);
...
//            fr = p0* (hfake->Integral(1,hfake->FindFixBin(0.0272))/hfake->Integral())/(hdata->Integral(1,hdata->FindFixBin(0.0272))/hdata->Integral());
	    ...
	    ofstream file(dir + TString("txt/TrueNumber_") + Form("pt%0.f-%0.f_chiso%0.f-%0.f.txt", lowpt, highpt,lowchiso,highchiso),ios::out);
	    ofstream file1(dir + TString("txt/FakeNumber_") + Form("pt%0.f-%0.f_chiso%0.f-%0.f.txt", lowpt, highpt,lowchiso,highchiso),ios::out);
            file1<<fake_window<<"\t"<<fit_err<<"\t"<<fr<<"\t"<<fr_Error<<endl;
            myfile <<fr<<"\t"<<fr_Error<<"\t"<<hfake->Integral(0,hfake->FindFixBin(0.0102)-1)<<"\t"<<fakeValue<<"\t"<<fake_window<<endl;
