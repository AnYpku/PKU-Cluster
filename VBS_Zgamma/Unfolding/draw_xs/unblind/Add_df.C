...
void run(TString var, TString recovar, TString title,TString tag){
	...
	TH1D*hist_clone3=(TH1D*)f3->Get(var+"_0");
        ...
        if(!f_in.is_open())cout<<"can not open the file "<<"/home/pku/anying/cms/PKU-Cluster/Unfolding/data_card/unblind/combine/r_"+recovar+"_"+tag+".txt"<<endl;
	...
	for(int i=0;i<hist_clone->GetNbinsX();i++){
                ...
                xbin_sys[i]=hist_clone->GetBinCenter(i+1);//+0.2*hist_clone->GetBinWidth(i+1);
                xerror_up[i]=0;
                xerror_down[i]=0;
...
//                cout<<xbin[i]<<" "<<ybin[i]<<" "<<yerror_up[i]<<" "<< yerror_down[i]<<endl; 
	...
	hist_clone->SetLineColor(1);
        ...
        TGraphAsymmErrors* gr_sys = new TGraphAsymmErrors(kk, xbin_sys, ybin, xerror_sysDown,xerror_sysUp, yerror_sysDown, yerror_sysUp);
        for(int i=0;i<gr->GetN();i++){
		cout<<var<<" bin"<<i+1<<"; obs df xs "<<ybin[i]<<"; err "<<yerror_sysDown[i]<<","<<yerror_sysDown[i]<<endl;
	}
...
//                cout<<"bin"<<j+1<<" MadGraph(stat included) info, scale to binwidth("<<hist_clone->GetBinWidth(j)<<") bin error "<<hist_clone->GetBinError(j)<<"; bin content "<<hist_clone->GetBinContent(j)<<endl;
	}
...
//      for(int i=0;i<hist_clone->GetNbinsX();i++){hist_clone->GetXaxis()->SetBinLabel(i+1,name[i]);}
	...
	for(int i=0;i<hist_clone->GetNbinsX();i++){
           ...
           fout_txt<<var<<" bin"<<i<<" "<<fixed<<setprecision(3)<<r[i]<<" "<<Err_up[i]<<" "<<Err_down[i]<<" "<<fixed<<setprecision(4)<<hist_clone->GetBinContent(i+1)<<" "<<hist_clone->GetBinError(i+1)<<" "<<ybin[i]<<" "<<yerror_up[i]<<" "<<yerror_down[i]<<endl;
	}
...
//        cout<<"high="<<ye_h[pos_h]<<"; low="<<ye_l[pos_l]<<endl;
        nominal->GetYaxis()->SetRangeUser(ye_h[pos_l]-ye_l[pos_l]-0.3,ye_h[pos_h]+ye_l[pos_h]+0.3);
...
//	vector<TString> title={"p_{T}^{l1}","p_{T}^{#gamma}","p_{T}^{j1}"};
...
//        run("genlep1pt","ptlep1",title[0],"full");
