...
f_in = TFile.Open("/afs/cern.ch/user/y/yian/work/PKU-Cluster/HiggsCombineTools/2016/FourChannels/2016elebarrel/mjj5007501000_detajj25456_zepp18/aa.root")
tstack = f_in.Get("hs")
...
jes_QCD  =[1.10,1.15,1.08,1.30,1.33,1.17,1.47]
jes_EWK      =[1.02,1.01,1.05,1.04,1.03,1.05,1.04]
pdf_QCD  =[1.01,1.01,1.03,1.01,1.01,1.02,1.01]
scale_QCD_band3=[1.138,1.191,1.240,1.102,1.113,1.177,1.104]
scale_QCD_band2=[1.108,1.134,1.135,1.102,1.109,1.110,1.103]
scale_QCD_band1=[1.028,1.055,1.102,1.001,1.003,1.063,1.000]
pdf_ewk   =[1.04,1.04,1.04,1.04,1.04,1.04,1.05]
scale_ewk =[1.03,1.06,1.09,1.02,1.05,1.10,1.12]
jer       =[1.01,1.01,1.01,1.01,1.01,1.01,1.01]
jer_QCD   =[1.03,1.08,1.09,1.21,1.20,1.15,1.23]
...
fakephoton=[1.16,1.20,1.08,1.16,1.17,1.08,1.15]
l1pref    =[1.01,1.02,1.03,1.01,1.01,1.03,1.01]
for i in range(1,8):
   f = open('%s_%s_bin_%d.txt'%('ele16', 'barrel', i),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t-\t-\t-\t#0. uncertainty on ele16_barrel\n'%('ele16','barrel'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\t#0. uncertainty on ele16_barrel\n'%('ele16','barrel',fakephoton[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\t-\t-\t-\n'%('ele16','barrel',i,ZA_sig_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\t-\t-\t-\n'%('ele16','barrel',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\n'%('ele16','barrel',i,non_prompt_binerror))
   f.write('TTA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\n'%('ele16','barrel',i,TTA_binerror))
   f.write('VV_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t%0.2f\t-\t-\n'%('ele16','barrel',i,VV_binerror))
   f.write('ST_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t%0.2f\t-\n'%('ele16','barrel',i,ST_binerror))
   f.write('WA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t%0.2f\n'%('ele16','barrel',i,WA_binerror))
