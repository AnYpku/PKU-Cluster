...
void EDBRHistoMaker::createAllHistos(TString isChannel) {
	...
	hs.setHisto("mT_pf", 10, 0, 50);
	hs.setHisto("mT2_pf", 8, 0, 200);
	hs.setHisto("mT_puppi", 10, 0, 50);
	hs.setHisto("mT2_puppi", 8, 0, 200);
...
//        nentries=100;
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if(filename.Contains("17")||filename.Contains("18"))
	        ...
	        if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && lep1_is_tight==1 && lep2_is_tight==1 && mll>40 && mll<80 && ptll > 30 && MET_T1_pt > 20 && mT_pf <50 && n_bjets20_loose_deepFlavB_pc==0 ){
...
//	nentries=100;
	...
	float sum=0;
        ...
        TFile*f1 = new TFile("/home/pku/anying/cms/PKU-Cluster/WWg/add_weight/HLT/HLTSF_20"+y+".root");
...
//                cout<<jentry<<" "<<Flag_goodVertices<<" "<<Flag_globalSuperTightHalo2016Filter<<" "<<Flag_HBHENoiseFilter<<" "<<Flag_HBHENoiseIsoFilter<<" "<<Flag_EcalDeadCellTriggerPrimitiveFilter<<" "<<Flag_BadPFMuonFilter<<" "<<Flag_BadPFMuonDzFilter<<" "<<Flag_eeBadScFilter<<" "<<Flag_ecalBadCalibFilter<<endl; 
		...
		if(  lepton_channel && n_loose_ele==1 && n_loose_mu==1 && mll>40 && mll<80 && ptll > 30 && lepton_flag && MET_T1Smear_pt > 20 && mT_pf<50 && n_bjets20_loose_deepFlavB_pc ==0 ){
			if(gen_weight>0) npp++;
			if(gen_weight<0) nmm++;
			numbe_out++;
			treename->Fill();
