...
//		file_ID->Close();
	...
	TFile*fin;
	fin=new TFile("/home/pku/anying/cms/PKU-Cluster/AQGC/batch_cards/rootfiles/optimal_ZA-EWK"+tag+".root");
	...
	for(int k=0;k<tree->GetEntries();k++){
		...
		if (  tformula->EvalInstance()  ){
			if(lep==13){
				...
				for(int i=0;i<3;i++){
					th1[i]->Fill(ZGmass,muon_weight[i]);//7~8 6~infi800~1200
				}
			}
			if(lep==11){
				...
				for(int i=0;i<3;i++){
					th1[i]->Fill(ZGmass,ele_weight[i]);//8~9 6~infi 800~1200                        
				}
			}
			if(photonet>0){
				...
				for(int i=0;i<3;i++){
					th1[i]->Fill(ZGmass,photon_weight[i]);//8~9 6~infi 800~1200                        
				}
			}
		}
