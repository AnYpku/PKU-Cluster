	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if(isChannel=="emu"){ 
			if(filename.Contains("MuonEG"))
				lepton_channel= ((HLT_emu1||HLT_emu2||HLT_emu3||HLT_emu4) && channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && lep1pt>15 && lep2pt>15 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.5) && drll>0.5;
			else if(filename.Contains("Muon"))
				lepton_channel= ( (HLT_Mu1) && !(HLT_emu1||HLT_emu2||HLT_emu3||HLT_emu4) && channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && lep1pt>20 && lep2pt>25 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.5) && drll>0.5;
			else if(filename.Contains("Ele"))
				lepton_channel= ( (HLT_Ele1) && !(HLT_Mu1) && !(HLT_emu1||HLT_emu2||HLT_emu3||HLT_emu4) && channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && lep1pt>20 && lep2pt>25 && fabs(lep1eta) < 2.4 && fabs(lep2eta) < 2.5) && drll>0.5;
		}
		...
		if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && lep1_is_tight==1 && lep2_is_tight==1 && mll >20 && ptll > 30 && PuppiMET_T1_pt > 20){
...
//		actualWeight = scalef*ele_id_scale*ele_reco_scale*muon_id_scale*muon_iso_scale*photon_id_scale*photon_veto_scale*puWeight*btag_weight_loose*PrefireWeight;//mc
		...
		if(filename.Contains("DY")||filename.Contains("TTJets")||filename.Contains("TTWJets")||filename.Contains("WJets")){
			...
			if(photon_selection==1 && photon_isprompt==1)
				photon_flag=1;
			else if( (photon_selection==2 || photon_selection==3 || photon_selection==4 || photon_selection==5) && photon_isprompt==1 && njets15>0)
				photon_flag=1;
			if(lep1_is_tight==1 && lep2_is_tight==1 && lep1_isprompt==1 && lep2_isprompt==1) 
				lepton_flag=1;
			else if( ! (lep1_is_tight==1 && lep2_is_tight==1) && lep1_isprompt==1 && lep2_isprompt==1 && njets20>0 )
				lepton_flag=1;
		}
		else{//MC
			if(photon_selection==1 && photon_isprompt==1) 
		...
		bool photon_channel=false,lepton_channel=false;
		if(isChannel=="emu") 
			lepton_channel= ((HLT_emu1||HLT_emu2||HLT_emu3) && channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && lep1pt>15 && lep2pt>15 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.5) && drll>0.5;
		if(isBarrel==1)  photon_channel=( (fabs(photoneta) < 1.4442) );
		else if(isBarrel==0) photon_channel= ( fabs(photoneta) < 2.5 && fabs(photoneta)>1.566 );
		else photon_channel=( (fabs(photoneta) < 1.4442) || ( fabs(photoneta) < 2.5 && fabs(photoneta)>1.566 ));
		if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && mll >20 && ptll > 30 && PuppiMET_T1Smear_pt > 20){
			if(gen_weight>0) npp++;
			if(gen_weight<0) nmm++;
			numbe_out++;
