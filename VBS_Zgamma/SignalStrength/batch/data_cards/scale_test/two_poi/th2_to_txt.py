...
genbinerror=[]
arr={}
f=open('../../../Uncer/two_poi/summary_uncer_'+sys.argv[2]+sys.argv[1]+'.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
# bin error
   ...
   f.write('process\t')
   f.write('ZA\tEW\tnon_prompt\tTTA\tVV\tST\tSig_out\n')
   f.write('process\t-1\t0\t1\t2\t3\t4\t5\n')
   f.write('rate\t')
   f.write('%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(ZA_bincontent,ZA_sig_bincontent, non_prompt_bincontent, TTA_bincontent, VV_bincontent, ST_bincontent, ZA_sig_out_bincontent))
   f.write('------------\n')
   ...
   f.write('ZA_sig_Stat_bin%d_%s%s\tlnN\t'%(i,sys.argv[2],sys.argv[1]))
   f.write('\t-\t%0.3f\t-\t-\t-\t-\t-\n'%(ZA_sig_binerror))
   ...
   f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['jes'+sys.argv[1]+'_ZA'][i-1],arr['jes'+'_ZA-EWK'][i-1],arr['jes'+sys.argv[1]+'_TTA'][i-1],arr['jes'+sys.argv[1]+'_VV'][i-1],arr['jes'+sys.argv[1]+'_ST'][i-1],arr['jes'+sys.argv[1]+'_ZAout'][i-1]))

   f.write('JER_%s\tlnN\t'%(sys.argv[1]))
   f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['jer'+sys.argv[1]+'_ZA'][i-1],arr['jer'+'_ZA-EWK'][i-1],arr['jer'+sys.argv[1]+'_TTA'][i-1],arr['jer'+sys.argv[1]+'_VV'][i-1],arr['jer'+sys.argv[1]+'_ST'][i-1],arr['jer'+sys.argv[1]+'_ZAout'][i-1]))
...
#
   ...
   f.write('theory group = pdf_ZA  Scale_ZA Scale_EW pdf_EW\n')

   f.write('photon_id\tlnN\t')
   f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['photon_ZA_ID'][i-1],arr['photon_ZA-EWK_ID'][i-1],arr['photon_TTA_ID'][i-1],arr['photon_VV_ID'][i-1],arr['photon_ST_ID'][i-1],arr['photon_ZA_ID'][i-1]))
#
   if sys.argv[2].find("ele") == -1: 
      f.write('mu_eff\tlnN\t')
      f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['muon_ZA_all'][i-1],arr['muon_ZA-EWK_all'][i-1],arr['muon_TTA_all'][i-1],arr['muon_VV_all'][i-1],arr['muon_ST_all'][i-1],arr['muon_ZA_all'][i-1]))
      f.write('mu_trigger\tlnN\t')
      f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['muon_ZA_trigger'][i-1],arr['muon_ZA-EWK_trigger'][i-1],arr['muon_TTA_trigger'][i-1],arr['muon_VV_trigger'][i-1],arr['muon_ST_trigger'][i-1],arr['muon_ZA_trigger'][i-1]))
      f.write('muon group = mu_eff mu_trigger\n')
#   
   else: 
      f.write('ele_reco\tlnN\t')
      f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['ele_ZA_reco'][i-1],arr['ele_ZA-EWK_reco'][i-1],arr['ele_TTA_reco'][i-1],arr['ele_VV_reco'][i-1],arr['ele_ST_reco'][i-1],arr['ele_ZA_reco'][i-1]))
#
      f.write('ele_ID\tlnN\t')
      f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['ele_ZA_ID'][i-1],arr['ele_ZA-EWK_ID'][i-1],arr['ele_TTA_ID'][i-1],arr['ele_VV_ID'][i-1],arr['ele_ST_ID'][i-1],arr['ele_ZA_ID'][i-1]))
      f.write('egamma group = ele_reco ele_ID photon_id\n')
#
   f.write('pileup\tlnN\t')
   f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['pu_ZA'][i-1],arr['pu_ewk'][i-1],arr['pu_TTA'][i-1],arr['pu_VV'][i-1],arr['pu_ST'][i-1],arr['pu_ZA'][i-1]))
#
...
#
   ...
   f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['ZA_eff'][i-1],arr['ZA-EWK_eff'][i-1],arr['TTA_eff'][i-1],arr['VV_eff'][i-1],arr['ST_eff'][i-1],arr['ZAout_eff'][i-1]))
#
   f.write('pileupId_mis_%s\tlnN\t'%(sys.argv[1]))
   f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['ZA_mis'][i-1],arr['ZA-EWK_mis'][i-1],arr['TTA_mis'][i-1],arr['VV_mis'][i-1],arr['ST_mis'][i-1],arr['ZAout_mis'][i-1]))
#
   ...
   if sys.argv[1].find("18") == -1:
        f.write('l1pref\tlnN\t')
        f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['l1pref_ZA'][i-1],arr['l1pref_ewk'][i-1],arr['l1pref_TTA'][i-1],arr['l1pref_VV'][i-1],arr['l1pref_ST'][i-1],arr['l1pref_ZA'][i-1]))
