...
void run(TString dir,TString name,TString cut,TString tag,TString type,TString vec_branchname,vector<Double_t> bins1,vector<Double_t> bins2){
	...
	for(int j=0;j<names.size();j++){
		for(int k=0;k<tags.size();k++){
			...
			for(int i=0;i<vars.size();i++){
				run(dir1,names[j],Reco,tags[k],"jer1",vars[i],mT_bins,mllg_bins);
				run(dir1,names[j],Reco,tags[k],"jesTotal",vars[i],mT_bins,bins2[i]);
			}
		}
	}
	return 1;
