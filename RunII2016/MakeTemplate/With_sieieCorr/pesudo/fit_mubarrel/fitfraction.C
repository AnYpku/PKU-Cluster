...
//        fit->Constrain(1,0,1);
	    ...
	    Double_t fake_window_Error,data_window,data_window_Error,mcFake,mcFake_err;
            Double_t data_total,data_error;
...
//            double chi2ToNdf = chi2;
	    double chi2ToNdf = chi2/Ndf;
            ...
            cout<<"test "<<fake_window<<" "<< mcp0->Integral(0,mcp0->FindFixBin(0.01015))<<endl;
            data_window = result->IntegralAndError(0,result->FindFixBin(0.01015),data_window_Error);
            data_total  = hdata->IntegralAndError(0,nBins,data_error);
	    ...
	    double fit_err=sqrt( pow(result->Integral()/mcp0->Integral()*p0,2)*pow(fake_window_Error,2)+pow(fake_window*p0/mcp0->Integral(),2)*pow(data_error,2)+pow(fake_window*result->Integral()*p0/(mcp0->Integral()*mcp0->Integral()),2)*pow(fakeError,2)+pow(fake_window*result->Integral()/mcp0->Integral(),2)*pow(errP0,2) );
cout<<pow(result->Integral()/mcp0->Integral()*p0,2)*pow(fake_window_Error,2)<<" "
    <<pow(fake_window*p0/mcp0->Integral(),2)*pow(data_error,2)<<" "
    <<pow(fake_window*result->Integral()*p0/(mcp0->Integral()*mcp0->Integral()),2)*pow(fakeError,2)<<" "
    <<pow(fake_window*result->Integral()/mcp0->Integral(),2)*pow(errP0,2)<<" "
    <<endl;
           cout<<errP0<<" "<<fit_err<<endl;
...
//            fr = p0* (hfake->Integral(1,hfake->GetXaxis()->FindFixBin(0.01015)-1)/hfake->Integral())/(hdata->Integral(1,hdata->GetXaxis()->FindFixBin(0.01015)-1)/hdata->Integral());
//            fr = p0* (hfake->Integral(1,hfake->FindFixBin(0.01015))/hfake->Integral())/(hdata->Integral(1,hdata->FindFixBin(0.01015))/hdata->Integral());
