...
fdir1 = '/home/pku/anying/cms/PKU-Cluster/Unfolding/common/EW_QCD/root/'
f_EW = TFile.Open(fdir+'hist_ZA-EWK_'+sys.argv[1]+sys.argv[3]+'.root')
f_ZA = TFile.Open(fdir1+'hist_ZA_'+sys.argv[1]+sys.argv[3]+'.root')
f_interf = TFile.Open(fdir1+'hist_ZA_interf_'+sys.argv[1]+sys.argv[3]+'.root')
...
th1_EW_sig_out=f_EW.Get('hist_'+sys.argv[2])
th1_ZA_sig_out=f_ZA.Get('hist_'+sys.argv[2])
th1_interf_sig_out=f_interf.Get('hist_'+sys.argv[2])
th1_ZA_sig_out.Add(th1_EW_sig_out,1)
th1_ZA_sig_out.Add(th1_interf_sig_out,1)
...
genbinerror=[]
arr={}
f=open('/home/pku/anying/cms/PKU-Cluster/Unfolding/Uncer/EW_QCD/summary_'+sys.argv[2]+"_"+sys.argv[3]+'.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
#	f.write('kmax %i  number of nuisance parameters (sources of systematical uncertainties)\n'%(n_NP+2))
	f.write('kmax * number of nuisance parameters (sources of systematical uncertainties)\n')
   if sys.argv[3].find("17") == -1 and sys.argv[3].find("16") == -1:#18
#	f.write('kmax %i  number of nuisance parameters (sources of systematical uncertainties)\n'%(n_NP+1))
	f.write('kmax * number of nuisance parameters (sources of systematical uncertainties)\n')
   f.write('------------\n')
   ...
   genhist_QCD =f_ZA.Get(sys.argv[1]+"_{}".format(i-1)) 
   genhist_interf =f_interf.Get(sys.argv[1]+"_{}".format(i-1)) 
   genhist.Add(genhist_QCD,1)
   genhist.Add(genhist_interf,1)
...
# bin error
   ...
   for j in range(1,nbins):
           genbincontent[j-1]=genbincontent[j-1] if genbincontent[j-1]>0 else 0
...
#   data=sum(genbincontent)+ non_prompt_bincontent+TTA_bincontent+VV_bincontent+ST_bincontent+ZA_sig_out_bincontent
   ...
   for j in range(1,nbins):
         f.write('recobin%d\t' %(i))
   f.write('recobin%i\trecobin%i\trecobin%i\trecobin%i\trecobin%i\n'%(i,i,i,i,i))
   f.write('process\t')
   for j in range(1,nbins):
         f.write('genbin%i\t' %(j))
   f.write('non_prompt\tTTA\tVV\tST\tSig_out\n')
   f.write('process\t')
   ...
   f.write('%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(non_prompt_bincontent, TTA_bincontent, VV_bincontent, ST_bincontent, ZA_sig_out_bincontent))
   f.write('------------\n')
   ...
   f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['jes'+sys.argv[3]+'_TTA'][i-1],arr['jes'+sys.argv[3]+'_VV'][i-1],arr['jes'+sys.argv[3]+'_ST'][i-1],arr['jes'+sys.argv[3]+'_SigOut'][i-1]))

   f.write('JER_%s\tlnN\t'%(sys.argv[3]))
   for j in range(1,nbins):
	 f.write('%0.3f\t'%(arr['genbin{}_jer'.format(j)][i-1]))
   f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['jer'+sys.argv[3]+'_TTA'][i-1],arr['jer'+sys.argv[3]+'_VV'][i-1],arr['jer'+sys.argv[3]+'_ST'][i-1],arr['jer'+sys.argv[3]+'_SigOut'][i-1]))
#
   f.write('pdf\tlnN\t')
   for j in range(1,nbins):
...
#
   ...
   for j in range(1,nbins):
	 f.write('%0.3f\t'%(arr['muon_ZA_trigger'][i-1]))
   f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['muon_ZA_trigger'][i-1],arr['muon_TTA_trigger'][i-1],arr['muon_VV_trigger'][i-1],arr['muon_ST_trigger'][i-1]))
#
   f.write('mu_eff\tlnN\t')
   for j in range(1,nbins):
	 f.write('%0.3f\t'%(arr['muon_ZA_all'][i-1]))
   f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['muon_TTA_all'][i-1],arr['muon_VV_all'][i-1],arr['muon_ST_all'][i-1],arr['muon_ZA_all'][i-1]))
#
   f.write('ele_reco\tlnN\t')
   for j in range(1,nbins):
	 f.write('%0.3f\t'%(arr['ele_ZA_reco'][0]))
   f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['ele_TTA_reco'][i-1],arr['ele_VV_reco'][i-1],arr['ele_ST_reco'][i-1],arr['ele_ZA_reco'][i-1]))
#
   f.write('ele_ID\tlnN\t')
   for j in range(1,nbins):
	 f.write('%0.3f\t'%(arr['ele_ZA_ID'][i-1]))
   f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['ele_TTA_ID'][i-1],arr['ele_VV_ID'][i-1],arr['ele_ST_ID'][i-1],arr['ele_ZA_ID'][i-1]))
#
   f.write('photon_id\tlnN\t')
   for j in range(1,nbins):
	 f.write('%0.3f\t'%(arr['photon_ZA_ID'][i-1]))
   f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['photon_TTA_ID'][i-1],arr['photon_VV_ID'][i-1],arr['photon_ST_ID'][i-1],arr['photon_ZA_ID'][i-1]))
#
   f.write('pileup\tlnN\t')
   for j in range(1,nbins):
	 f.write('%0.3f\t'%(arr['genbin{}_pu'.format(j)][i-1]))
   f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n' %(arr['pileup_TTA'][i-1],arr['pileup_VV'][i-1],arr['pileup_ST'][i-1],arr['pileup_ZA'][i-1]))
#
...
#
   if sys.argv[3].find("18") == -1:
        ...
        f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['l1pref_TTA'][i-1],arr['l1pref_VV'][i-1],arr['l1pref_ST'][i-1],arr['l1pref_ZA'][i-1]))
#
   f.write('pileupId_mis_%s\tlnN\t'%(sys.argv[3]))
   for j in range(1,nbins):
          f.write('%0.3f\t'%(arr['genbin{}_mis'.format(j)][i-1]))
   f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['TTA_mis'][i-1],arr['VV_mis'][i-1],arr['ST_mis'][i-1],arr['SigOut_mis'][i-1]))
#
   f.write('pileupId_eff_%s\tlnN\t'%(sys.argv[3]))
   for j in range(1,nbins):
          f.write('%0.3f\t'%(arr['genbin{}_eff'.format(j)][i-1]))
   f.write('-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['TTA_eff'][i-1],arr['VV_eff'][i-1],arr['ST_eff'][i-1],arr['SigOut_eff'][i-1]))
