...
void unroll_name(TString filename,TString tag){
	...
	TFile* fout = new TFile("aa_"+filename+"_pu"+tag+".root","RECREATE");
	Double_t ZA_scale= 1;
	...
	TFile* f_ZA;
	f_ZA=new TFile("./root/hist_"+filename+"_pileup_"+tag+".root");
	TH1D* th1_ZA[num];
	TH1D* t_ZA[num];
	TCanvas* cc[num];
	TLegend *ll[num];
	...
	for(Int_t i=1;i<num;i++){
		if(i==1)t_ZA[i]->SetLineColor(kRed);
...
//	cmsLumi(tag) ;
	...
	fPads2->Update();
	c1->Print("./figs/hist-2d_"+filename+tag+"_pu.eps");
	c1->Print("./figs/hist-2d_"+filename+tag+"_pu.pdf");
