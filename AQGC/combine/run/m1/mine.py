...
input_filename_17e = '/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2017/el-muon-combine/paramsets_'+sys.argv[1]+'_el.txt'
...
for l_num, l in enumerate(input_file_17e):
	...
	a1_17e = float(ll[1])
	for bin_x in range(1,nGridPointsForNewF+1):
	        par1_value=hists[l_num+10].GetXaxis().GetBinCenter(bin_x)
        	yield_bin=1+a1_17e*par1_value+a2_17e*par1_value*par1_value
	        hists[l_num+10].SetBinContent(bin_x,yield_bin)
	hists[l_num+10].Write()#the last five histos are electron channel ZGmass bins
#########################2017#################################
...
for l_num, l in enumerate(input_file_18m):
	...
	a1_18m = float(ll[1])
	for bin_x in range(1,nGridPointsForNewF+1):
        	par1_value=hists[l_num+15].GetXaxis().GetBinCenter(bin_x)#200 ponits from a range, get every point value which is the oprator value
	        yield_bin=1+a1_18m*par1_value+a2_18m*par1_value*par1_value#get aQGC/SM ratio
	        hists[l_num+15].SetBinContent(bin_x,yield_bin)
	hists[l_num+15].Write()#the first five histos are muon channel ZGmass bins
...
for l_num, l in enumerate(input_file_18e):
	...
	a1_18e = float(ll[1])
	for bin_x in range(1,nGridPointsForNewF+1):
	        par1_value=hists[l_num+20].GetXaxis().GetBinCenter(bin_x)
        	yield_bin=1+a1_18e*par1_value+a2_18e*par1_value*par1_value
	        hists[l_num+20].SetBinContent(bin_x,yield_bin)
	hists[l_num+20].Write()#the last five histos are electron channel ZGmass bins
#########################2018#################################
