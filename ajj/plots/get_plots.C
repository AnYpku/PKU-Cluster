...
void run(TString data,TString year,TString var,TString title,float nbins,float low,float high){
    vector<TString> filename={"EWKZ2Jets","DYJets_NLO","TTJets","ST","DiPhoton40ToInf","WWinc","WZinc","ZZinc",data};
    vector<TString> fname={"EWK Zjj","DY NLO","t#bar{t}","t(#bar{t})W","#gamma#gamma","WW","WZ","ZZ",data};
    ...
    if(data.Contains("EGamma")){
	    ...
	    cut= "(n_loose_ele==2 && lep1pt>30 && lep2pt>25 && abs(lep1eta)<2.5 && abs(lep2eta)<2.5 && lepton_is_tight[0]==1 && lepton_is_tight[1]==1 && Vpt>75 && abs(Vmass-91)<15 && drj1l1>0.4 && drj2l1>0.4 && drj1l2>0.4 && drj2l2>0.4 && " + HLT +" && jet1pt>30 && jet2pt>30 )";
    }
    else if(data.Contains("Muon")){
	    HLT="HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass3p8==1";
	    cut = "(n_loose_mu==2 && lep1pt>30 && lep2pt>25 && abs(lep1eta)<2.4 && abs(lep2eta)<2.4 && lepton_is_tight[0]==1 && lepton_is_tight[1]==1 && Vpt>75 && abs(Vmass-91)<15 && drj1l1>0.4 && drj2l1>0.4 && drj1l2>0.4 && drj2l2>0.4 && " + HLT +" && jet1pt>30 && jet2pt>30 )";
    }
    ...
    for(int i=0;i<nfile;i++){
	    if(filename[i].Contains("EGamma")==0 && filename[i].Contains("Muon")==0){
		    ...
		    if(filename[i].Contains("DY") && filename[i].Contains("NLO")==0)
			    histMC[i]->Scale(1/1.4859555);
		    if(filename[i].Contains("DY"))
...
int get_plots(){
	...
	vector<float> low={75,75,-2.5,-5,30,-5,-5,30,-5,-5,500,0,0,0,30,-2.5,-5,30,-2.5,-5};
	vector<float> high={300,110,2.5,5,800,5,5,500,5,5,2000,10,10,70,200,2.5,5,200,2.5,5};
	vector<float> nbins={40,40,30,30,30,30,30,30,30,30,30,30,30,100,30,30,30,30,30,30};
