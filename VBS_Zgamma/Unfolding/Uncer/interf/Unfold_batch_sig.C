#define pi 3.1415926
void run(TString sample,TString tag,TString vec_branchname,TString reco,vector<double> bins,TString cut1,int kk){
     TString dir1;
     ...
     double jet1eta,jet2eta,gendetajj;
     double muon1_id_scale,muon2_id_scale,muon1_iso_scale,muon2_iso_scale,ele1_id_scale,ele2_id_scale,ele1_reco_scale,ele2_reco_scale,photon_id_scale,photon_veto_scale,pileupWeight,prefWeight,muon1_track_scale,muon2_track_scale,muon_hlt_scale,ele_hlt_scale,puIdweight_T,puIdweight_L,puIdweight_M,puIdweight;
     Double_t scalef;
     int lep;
     ...
     TH1D*th1[kk];TH2D*th2[kk];
     TString th1name[kk];
     Double_t mjj_bins[4]={500, 800, 1200, 2000};
     Double_t detajj_bins[4]={2.5, 4.5,  6, 6.5};
     for(int i=0;i<kk;i++){
	     th1name[i]=vec_branchname+"_"+sample+Form("_recobin%d",i+1);
	     if(vec_branchname.Contains("Mjj")==0)
		     th1[i]= new TH1D(th1name[i],th1name[i],bins.size()-1,&bins[0]);
	     else
		     th2[i]= new TH2D(th1name[i],th1name[i],3,mjj_bins,3,detajj_bins);
     }
     double actualWeight;
     for(int k=0;k<tree->GetEntries();k++){
	     ...
	     actualWeight=scalef*pileupWeight*photon_id_scale*photon_veto_scale*puIdweight;
	     if(lep==11)
	     ...
	     if(vec_branchname.Contains("Mjj")==0){
		     for(Int_t j=0;j<kk;j++){
			     if(j<kk-1 && variables[reco]>bins[j]&&variables[reco]<bins[j+1])
				     th1[j]->Fill(variables[vec_branchname],actualWeight);
		     ...
		     for(int iy=0;iy<3;iy++){
			     for(int ix=0;ix<3;ix++){
				     ...
				     if(ix<2&&iy<2&&variables[reco]>mjj_bins[ix]&&variables[reco]<mjj_bins[ix+1]&&detajj>detajj_bins[iy]&&detajj<detajj_bins[iy+1])
					     th2[ix+(iy)*3]->Fill(variables[vec_branchname],gendetajj,actualWeight);                             
				     if(ix==2&&iy<2&&variables[reco]>mjj_bins[ix]&&detajj>detajj_bins[iy]&&detajj<detajj_bins[iy+1])   
					     th2[ix+(iy)*3]->Fill(variables[vec_branchname],gendetajj,actualWeight);                             
				     if(ix<2&&iy==2&&variables[reco]>mjj_bins[ix]&& variables[reco]<mjj_bins[ix+1] &&detajj>detajj_bins[iy])
					     th2[ix+(iy)*3]->Fill(variables[vec_branchname],gendetajj,actualWeight);                             
				     if(ix==2&&iy==2 && variables[reco]>mjj_bins[ix] && detajj>detajj_bins[iy])                            
					     th2[ix+(iy)*3]->Fill(variables[vec_branchname],gendetajj,actualWeight);                             
			     }
		     }
	     }
     ...
     TFile*fout[kk];
     for(int j=0;j<kk;j++){
             if(vec_branchname.Contains("Mjj")==0) fout[j]= new TFile("unfold_"+vec_branchname+Form("_recobin%d_",j+1)+sample+tag+".root","recreate");
             else fout[j]= new TFile("unfold_2d"+vec_branchname+Form("_recobin%d_",j+1)+sample+tag+".root","recreate");
     }
    for(int j=0;j<kk;j++){
             fout[j]->cd();
	     if(vec_branchname.Contains("Mjj")==0)
...
//		if(tag[i].Contains("17")==0) continue;
		for(int k=0;k<recovars.size();k++){
			for(int j=0;j<kk;j++){
				...
				if(genvars[k].Contains("Mjj")==0)
					run(sample[j],tag[i],genvars[k],recovars[k],bins[k],cut1,bins[k].size()-1);
				else
					run(sample[j],tag[i],genvars[k],recovars[k],bins[k],cut1,9);
			}
		}
	}
	return 1;
