	...
	for(int i=0;i<tag.size();i++){
		...
		for(int j=0;j<sample.size();j++){
			...
			for(int ik=0;ik<channels.size();ik++){
				TFile*file=new TFile("hist_"+sample[j]+"_2d_"+tag[i]+channels[ik]+".root");
				TFile*fout;
				if(sample[j].Contains("EWK")){
					fout=new TFile("../root/hist_"+sample[j]+"_genMjj_"+tag[i]+channels[ik]+".root","recreate");
					...
					fout=new TFile("../root/hist_"+sample[j]+"_Mjj_"+tag[i]+channels[ik]+".root","recreate");
