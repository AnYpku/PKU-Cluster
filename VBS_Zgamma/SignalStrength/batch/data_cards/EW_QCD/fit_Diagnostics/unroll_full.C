...
void run(TString channel){
	setTDRStyle();
	TFile* fout = new TFile("aa_QCD_"+channel+".root","RECREATE");
	...
	Double_t lumi=137.1;
        ...
        TH1F*th2_ZA  =(TH1F*)f1->Get("hist_ZA_"+channel);
        TH1F*th2_ZA17=(TH1F*)f2->Get("hist_ZA_"+channel);
        TH1F*th2_ZA18=(TH1F*)f3->Get("hist_ZA_"+channel);
	...
	const char *name[7]={"0.5-0.8","8-1.2","1.2-2","0.5-0.8","8-1.2","1.2-2","0.5-2"};
        ...
        TH1F*th2_ZA_sig  =(TH1F*)f1->Get("hist_Sig_out_"+channel);
        TH1F*th2_ZA_sig17=(TH1F*)f2->Get("hist_Sig_out_"+channel);
        TH1F*th2_ZA_sig18=(TH1F*)f3->Get("hist_Sig_out_"+channel);
...
//      TLine* line = new TLine(th2_ZA->GetXaxis()->GetXmin(),0,th2_ZA->GetXaxis()->GetXmax(),0);
	...
	leg->AddEntry(th2_ZA_sig,Form("EW+QCD Z#gamma_{out}[%.1f] ",th2_ZA_sig->GetSum()) );
	leg->AddEntry(th2_ZA, Form("EW+QCD Z#gamma [%.1f] ",th2_ZA->GetSum()));
	leg->AddEntry(th2_plj, Form("Nonprompt [%.1f] ",th2_plj->GetSum() ),"f");
