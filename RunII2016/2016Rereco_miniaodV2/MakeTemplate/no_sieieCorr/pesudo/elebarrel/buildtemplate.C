...
void style();
void histo();
void select(TTree *tree,TH1D *h1[ptnumber],TH1D *h2[ptnumber][21],TH1D *h3[ptnumber],TGraph* gr);
...
//    cout<<"the biggest pt"<<*biggest_pt<<endl;
    for(Int_t k=0;k<ptnumber;k++){
	    double sietaieta;
	    sietaieta =gr->Eval(photon_sieie[position]);
	    if(photon_chiso[position]< chiso && *biggest_pt<highpt[k] && *biggest_pt>lowpt[k]) 
		    ...
		    if( (photon_isprompt[position]!=1 && isprompt!=1 && (za==false && tta==false)) ||
			    ...
			    h1[k]->Fill(sietaieta,scalef);}//data template
                    if(sietaieta< sieie){ 
			    m1[k] += scalef;} //the number of data in medium sieie region
		    if(sietaieta< sieie && photon_isprompt[position]!=1 && isprompt!=1&&
	              (za==false && tta==false) ){
			    FakeNumber[k]+=scalef;}
		    if(sietaieta< sieie && photon_isprompt[position]==1 &&
		       isprompt==1 && (tta==true||za==true) ){
			    TrueNumber[k]+=scalef;}
		    if(photon_isprompt[position]!=1 && isprompt!=1 && (za==false && tta==false)){
			    hfake[k]->Fill(sietaieta,scalef);}//mcTruth fake
		    if(photon_isprompt[position]==1 && isprompt==1 && (tta==true||za==true) ){
			    htrue[k]->Fill(sietaieta,scalef);}//mcTruth real
		    if(photon_isprompt[position]==1 && isprompt ==1 && (za == true || tta == true) ){
		            h3[k]->Fill(sietaieta,scalef);m3[k] +=scalef;}//true template
		    if(sietaieta< sieie) h_sieie[k]->Fill(sietaieta,scalef);
...
//		    if( za==false &&  isprompt==1) continue;
		    ...
		    { 
			    h2[k][j]->Fill(sietaieta,scalef);m2[k][j] +=scalef;
		    }  
	    }//fake template
    }
    vector_pt.clear();
...
TFile* fgr =new TFile("transformation_barrel.root");
TGraph* gr =(TGraph*)fgr->Get("barrel");
...
void histo(){
 ...
 Double_t xhigh= 0.0202;
  for(Int_t i=0;i<ptnumber;i++){
      ...
      h13[i]=new TH1D(Form("h13_pt%0.f_%0.f",lowpt[i],highpt[i]),"true template",bin,xlow,xhigh);////Zjets && leding_photon && medium_cut && photon_chiso>2
    for(Int_t j=0;j<21;j++){  
