...
fdir = '/afs/cern.ch/user/y/yian/work/PKU-Cluster/Unfolding/common/CR/'
...
genbinerror=[]
arr={}
f=open('/afs/cern.ch/user/y/yian/work/PKU-Cluster/Unfolding/Uncer/CR/summary_'+sys.argv[2]+"_"+sys.argv[3]+'CR.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
for i in range(1,nbins):
   f = open('./txt/%s_%s_control_bin%i.txt'%(sys.argv[2],sys.argv[3],i),'w')
   f.write('imax 1   number of channels\n')
   f.write('jmax 10   number of processes-1\n')
   if sys.argv[3].find("18") == -1:
	f.write('kmax 29  number of nuisance parameters (sources of systematical uncertainties)\n')
   else:
...
#          print genbincontent[j-1],' ',genbinerror[j-1]
   ...
   for j in range(1,nbins):
          f.write('genbin%i_Stat_control_recobin%d_%s\tlnN\t'%(j,i,sys.argv[3],))
          if j==1:
   ...
   f.write('QCD_Stat_control_recobin%d_%s\tlnN\t'%(i,sys.argv[3]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('%0.2f\t-\t-\t-\t-\t-\t-\n'%(ZA_binerror))

   f.write('non_prompt_Stat_control_recobin%d_%s\tlnN\t'%(i,sys.argv[3]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t%0.2f\t-\t-\t-\t-\t-\n'%(non_prompt_binerror))

   f.write('TTA_Stat_control_recobin%d_%s\tlnN\t'%(i,sys.argv[3]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t-\t%0.2f\t-\t-\t-\t-\n'%(TTA_binerror))

   f.write('VV_Stat_control_recobin%d_%s\tlnN\t'%(i,sys.argv[3]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t-\t-\t%0.2f\t-\t-\t-\n'%(VV_binerror))

   f.write('ST_Stat_control_recobin%d_%s\tlnN\t'%(i,sys.argv[3]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t-\t-\t-\t%0.2f\t-\t-\n'%(ST_binerror))

   f.write('WA_Stat_control_recobin%d_%s\tlnN\t'%(i,sys.argv[3]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t-\t-\t-\t-\t%0.2f\t-\n'%(WA_binerror))

   f.write('ZA_SigOut_Stat_control_recobin%d_%s\tlnN\t'%(i,sys.argv[3]))
   for j in range(1,nbins):
         f.write('-\t')
   f.write('-\t-\t-\t-\t-\t-\t%0.2f\n'%(ZA_sig_out_binerror))
...
#
   ...
   for j in range(1,nbins):
         f.write('-\t')
   f.write('%0.2f\t-\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n'%(arr['muon'+sys.argv[3]+'_trigger'][0],arr['muon'+sys.argv[3]+'_trigger'][0],arr['muon'+sys.argv[3]+'_trigger'][0],arr['muon'+sys.argv[3]+'_trigger'][0],arr['muon'+sys.argv[3]+'_trigger'][0],arr['muon'+sys.argv[3]+'_trigger'][0]))
#
   f.write('mu_eff\tlnN\t')
   for j in range(1,nbins):
         f.write('-\t')
