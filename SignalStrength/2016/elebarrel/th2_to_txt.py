...
jes      =[1.02,1.01,1.05,1.04,1.03,1.05,1.04]
...
for i in range(1,8):
   f = open('%s_%s_bin_%d.txt'%('16', 'elebarrel', i),'w')
...
# bin error
   ...
   f.write('bin\telebarrel\telebarrel\telebarrel\telebarrel\telebarrel\telebarrel\telebarrel\telebarrel\n')
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t-\t-\t-\t-\t#0. uncertainty on 16_elebarrel\n'%('16','elebarrel'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\t-\t#0. uncertainty on 16_elebarrel\n'%('16','elebarrel',fakephoton[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\t-\t-\t-\t-\n'%('16','elebarrel',i,ZA_sig_binerror))
   f.write('VBSout_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\t-\t-\t-\t-\n'%('16','elebarrel',i,ZA_sigout_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\t-\n'%('16','elebarrel',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\t-\n'%('16','elebarrel',i,non_prompt_binerror))
   f.write('TTA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t%0.2f\t-\t-\t-\n'%('16','elebarrel',i,TTA_binerror))
   f.write('VV_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t%0.2f\t-\t-\n'%('16','elebarrel',i,VV_binerror))
   f.write('ST_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t%0.2f\t-\n'%('16','elebarrel',i,ST_binerror))
   f.write('WA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t-\t%0.2f\n'%('16','elebarrel',i,WA_binerror))
   f.write('JES_16\tlnN\t%0.2f\t%0.2f\t%0.2f\t-\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n'%(jes[i-1],jes[i-1],jes_QCD[i-1],jes_TTA[i-1],jes_VV[i-1],jes_ST[i-1],jes[i-1]))
   f.write('JER_16\tlnN\t%0.2f\t%0.2f\t%0.2f\t-\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n'%(jer[i-1],jer[i-1],jer_QCD[i-1],jer_TTA[i-1],jer_VV[i-1],jer_ST[i-1],jer[i-1]))
   ...
   f.write('photon_id_16\tlnN\t1.03\t1.03\t1.03\t-\t1.03\t1.03\t1.03\t1.03\n')
