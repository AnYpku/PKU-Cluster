...
input_filename_e = '/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2017/ele/4bins/paramsets_'+sys.argv[1]+'_el.txt'
input_filename_m = '/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2017/muon/4bins/paramsets_'+sys.argv[1]+'_mu.txt'
...
for l_num, l in enumerate(input_file_e):
	...
	a1_e = float(ll[1])
	for bin_x in range(1,nGridPointsForNewF+1):
	        par1_value=hists[l_num+4].GetXaxis().GetBinCenter(bin_x)
        	yield_bin=1+a1_e*par1_value+a2_e*par1_value*par1_value
	        hists[l_num+4].SetBinContent(bin_x,yield_bin)
	hists[l_num+4].Write()#the last five histos are electron channel ZGmass bins
