...
void unroll_name(TString filename,TString tag){
	...
	TFile* f_ZA;
	f_ZA=new TFile("./hist_plj_"+filename+"_"+tag+".root");
	TH1D* th1_ZA[num];
	TH1D* t_ZA[num];
	TCanvas* cc[num];
	TLegend *ll[num];
	th1_ZA[0]=(TH1D*)f_ZA->Get("hist");
	th1_ZA[1]=(TH1D*)f_ZA->Get("hist_up");
	th1_ZA[2]=(TH1D*)f_ZA->Get("hist_down");
...
//                t_ZA[i]->Scale(1/t_ZA[i]->Integral());
	...
	for(Int_t i=1;i<num;i++){
        ...
        latex.SetTextSize(0.06);
        latex.SetLineWidth(2);
        latex.DrawLatex(1.2,0.9*vec_ymax[0],"2.5<#Delta#eta_{jj}<4.5");
        latex.DrawLatex(4.2,0.9*vec_ymax[0],"4.5<#Delta#eta_{jj}<6");
        latex.DrawLatex(6.1,1.3*vec_ymax[0],"#Delta#eta_{jj}>6");
...
//	nominal->GetYaxis()->SetRangeUser(h_down->GetMinimum()-0.1,h_up->GetMaximum()+0.05);
	...
	fPads2->Update();
	c1->Print("./figs/hist-2d_"+filename+tag+"_plj.eps");
	c1->Print("./figs/hist-2d_"+filename+tag+"_plj.pdf");
...
int unroll_test(){
	...
	TString sample[4]={"mubarrel","muendcap","elebarrel","eleendcap"};
	for(int i=0;i<4;i++){
