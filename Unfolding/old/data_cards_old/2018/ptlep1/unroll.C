...
int unroll(){
	...
	TFile* f2 = new TFile("hist_Data.root","RECREATE");
	Double_t ZA_scale= 0.94;
	std::ostringstream strs;
	std::string lumivalue = strs.str();
	Double_t lumi=35.862;

	TFile* f_ZA=TFile::Open("./root/hist_ZA.root");
...
//	const char *name[5]={"20~50","50~80","80~120","120~200","200~400"};
	const char *name[4]={"20~80","80~120","120~200","200~400"};
...
//	hist_ZA_sig->Scale(lumi);
	...
	TFile* f_ZA_sig_tot=TFile::Open("./root/hist_ZA-EWK.root");
        TH1D* hist_ZA_sig_tot=(TH1D*)f_ZA_sig->Get("hist1");
...
//	hist_ST->Scale(lumi);
	...
	for(Int_t i=1;i<=kk;i++){ hist_plj->GetXaxis()->SetBinLabel(i,name[i-1]);}
        ...
        TH1D* hist_Data=(TH1D*)hist_ST->Clone();
        hist_Data->Add(hist_TTA);
        hist_Data->Add(hist_ZA);
        hist_Data->Add(hist_WA);
        hist_Data->Add(hist_plj);
        hist_Data->Add(hist_VV);
        hist_Data->Add(hist_ZA_sig_tot);
