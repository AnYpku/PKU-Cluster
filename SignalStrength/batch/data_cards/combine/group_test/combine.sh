...
#combine -M MultiDimFit --algo grid --points 50  --rMin 0.2 --rMax 4 -m 125 -n ele_${name} higgsCombinebestfit_${name}.MultiDimFit.mH125.root --snapshotName MultiDimFit --freezeNuisanceGroups ele --expectSignal=1
...
#
#plot1DScan.py higgsCombinenominal_${name}.MultiDimFit.mH125.root --others \
#"higgsCombineele_${name}.MultiDimFit.mH125.root:Freeze ele:4" \
#"higgsCombinestat_${name}.MultiDimFit.mH125.root:Freeze all:2" \
#--breakdown eleEff,syst,stat -o scan_freeze_ele_${name}
#
#plot1DScan.py higgsCombinenominal_${name}.MultiDimFit.mH125.root --others \
