...
#include "RoccoR.cc"
#include "TH2.h"
void loopPlot(int isBarrel,TString isChannel,int n) {
...
//		continue;
		...
		if (redoHistograms) {
			EDBRHistoMaker* maker = new EDBRHistoMaker(treeData, fileData,
					hisRatio, out_buffer, &rc,isChannel,n);
			maker->setUnitaryWeights(true);
			maker->Loop(buffer,lumiValue,isBarrel,isChannel,n);
			maker->endjob();
			fileData->Close();
		}
...
//		continue;
		...
		if (redoHistograms) {
			...
			maker->Loop_SFs_mc(buffer,lumiValue,isBarrel,isChannel,n);
			maker->endjob();
			fileMC->Close();
		}
