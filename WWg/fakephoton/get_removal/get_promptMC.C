...
void run(TString var,TString title,TString cut,vector<double> bins,TString year){
        vector<TString>sample={"MuonEG","ZGJets","TTGJets","ST","WGJets","VV","WWG_emu"};
        vector<short> vec_cols = {kBlack,kYellow-7,kBlue,40,kGreen,kCyan,kRed-7,kGreen-6,kViolet};
	...
	for(int i=0;i<num;i++){
		...
		TString histname="hist_"+sample[i]+year;
		...
		TTree*tree=(TTree*)fin->Get("outtree");
			...
			tree->Draw(var+">>"+histname,"("+cut+ "&& PuppiMET_T1_pt>20"+ ")*scalef","goff");
		else
			tree->Draw(var+">>"+histname,"("+cut+" && PuppiMET_T1_pt>20"+")*actualWeight","goff");
		if(sample[i].Contains("WWG"))
			h1[i]->Scale(lumi);
		cout<<sample[i]<<" "<<tree->GetEntries()<<" "<<h1[i]->GetSum()<<endl;
//		fin->Close();
	}
        gStyle->SetOptStat(0);
	TLegend*l1=new TLegend(0.62,0.6,0.89,0.89);
	TLegend*l2=new TLegend(0.15,0.6,0.4,0.89);
	l1->SetTextSize(0.04);
	l2->SetTextSize(0.04);
        l1->SetLineColor(0);
        l2->SetLineColor(0);
...
//        h1[0]->GetYaxis()->SetRangeUser(0.001,h1[0]->GetMaximum()*100);
	...
	l2->AddEntry(h1[0],Form("Nonprompt from data [%.1f]",h1[0]->GetSum()));
        THStack* hs = new THStack("hs", "");

	TH1D*h2=new TH1D("h2_"+year,"",bins.size()-1,&bins[0]);
        h2->SetFillColor(0);
        h2->SetLineColor(2);
	double tot=0;
	for(int i=2;i<num;i++){
	...
	l2->AddEntry(h2,Form("Total MC removal [%.1f]",-1*tot),"f");
	for(int i=1;i<num;i++){
                h1[i]->Scale(-1);
		...
		if(i<=4){
			if(sample[i].Contains("Muon"))
				l1->AddEntry(h1[i],Form("Nonprompt from data [%.2f]",h1[i]->GetSum()));
			else
				...
				if(sample[i].Contains("Muon"))
					l2->AddEntry(h1[i],Form("Nonprompt from data [%.2f]",h1[i]->GetSum()));
				else
...
//	vector<double> ptbins={30,40,50,60,80,120};
	vector<double> mTbins={50,80,120,160,200,250};
	TString cut="(channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && lep1_charge*lep2_charge<0 && drll>0.5 && lep1pt>20 && lep2pt>25 && fabs(lep1eta) < 2.4 && fabs(lep2eta) < 2.5 && n_loose_ele==1 && n_loose_mu==1 && ptll>30 && mll>20 && (lep1_is_tight==1 && lep2_is_tight==1 ) && lep1_isprompt==1 && lep2_isprompt==1 && n_photon>0  && photonet > 20. && ( (fabs(photoneta) < 1.4442) || ( fabs(photoneta) < 2.5 && fabs(photoneta)>1.566 )) && drl1a>0.5 && drl2a>0.5 && (photon_selection==2 || photon_selection==3 || photon_selection==4 || photon_selection ==5 ) && (photon_isprompt==1) && n_bjets20_medium==0 && mT>50 && mT2>20 && njets30<=1)";
