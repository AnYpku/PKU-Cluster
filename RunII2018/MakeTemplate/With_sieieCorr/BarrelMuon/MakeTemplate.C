...
//   nentries = 100000;
   int count=0;
   double Mchiso = 8.75;
   double chisomax=12,chisomin=Mchiso;//6~12
   double actualWeight;
   TFile* ID_photon_file = TFile::Open("./egamma_SFs/2018_PhotonsMedium.root");
   TH2F* ID_photon=0;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	   ...
	   for(Int_t k=0;k<num;k++){
		   if(name.Contains("A")==1){
			   ...
			   actualWeight = scalef*pileupWeight*muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale*photon_id_scale*photon_veto_scale;
