...
// Setup names of data files for trees.
	...
	for (int j = 0; j< pathToTrees.size(); j++) {
		for (int ii = 0; ii < nDATA; ii++) {
			...
			fData.push_back(pathToTrees[j] +"cutlep-out"+ data[ii] + "18.root");
		}
	}
	...
	std::cout << "set data imformation, we have " << nMC << "mc file"
		<< std::endl;
        std::string mc[nMC] ={"fakeL","WW","VV","ST","DYJets","ZGJets","TTJets","TGJets","TTGJets","WJets","WGJets","tZq"};
	std::vector< TString > fMC;
	for (int ii = 0; ii < nMC; ii++) {
		fMC.push_back(pathToTrees[0] +"cutlep-out"+ mc[ii] + "18.root");
		cout<<pathToTrees[0]<<"cutlep-out"<<mc[ii] <<"18.root"<<endl;
	}
	const int nmc=fMC.size();
	std::string mcLabels[nmc] ={"fakeL","WW","VV","ST","DYJets","ZGJets","TTJets","TGJets","TTGJets","WJets","WGJets","tZq"};
	double kFactorsMC_array[nmc] ={1,lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1};
	...
	for (int ii = 0; ii < nMCSig; ii++) {
		fMCSig.push_back(pathToTrees[0] + "cutlep-out" + mcSig[ii] + "18.root");
	}
	...
	std::vector < std::string > fHistosMCSig;
        std::vector < TString > histName={"m_{ll}","p_{T}^{#gamma}","#eta_{#gamma}","#phi_{#gamma}","p_{T}^{l1}","#eta_{l1}","#phi_{l1}","p_{T}^{l2}","#eta_{l2}","#phi_{l2}","m_{ll#gamma}","p_{T}^{ll}","p_{T}^{ll}","#phi_{ll}","#eta_{ll}","PuppiMET","#phi_{PuppiMET}","npvs","N_{bjets}","m_{T_{WW}}","m_{T_{l_{2}+W}}","PuppiMET_T1","#phi_{PuppiMET_T1}"};
