...
TFile* fdata = TFile::Open("../root/Data_template-cutlep-outDEle16.root");
...
TFile* ffake = TFile::Open("../root/Fake_template-cutlep-outDEle16.root");
...
//            fr = p0* (hfake->Integral(1,hfake->FindFixBin(0.03001))/hfake->Integral())/(hdata->Integral(1,hdata->FindFixBin(0.03001))/hdata->Integral());
	    ...
	    file3<<Form("%0.f<pt<%0.f",lowpt,highpt)<<"\t\t"<<"\t"<<fixed<<setprecision(2)<<fr<<"\t"<<fixed<<setprecision(2)<<fr_Error<<"\t"<<chi2ToNdf<<endl;
