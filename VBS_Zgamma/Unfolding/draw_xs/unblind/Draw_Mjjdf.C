...
void run(TString var, TString recovar,TString title,TString tag){
	...
	double r[kk],Err_up[kk],Err_down[kk];
	...
	for(int i=0;i<hist_clone->GetNbinsX();i++){
		...
		cout<<r[i]<<" "<<Err_down[i]<<" "<<Err_up[i]<<endl;
...
//	cout<<yerror_sysDown[3]<<" "<<yerror_sysUp[3]<<endl;
        for(int i=0;i<hin->GetNbinsX();i++){
             hin->SetBinContent(i+1,ybin[i]/1000*BinWidth[i]);
             hin->SetBinError(i+1,yerror_up[i]/1000*BinWidth[i]);
	}
...
//        cout<<"minimum "<<hist_clone->GetMinimum()<<endl; 
	...
	cout<<"max= "<<hist_clone->GetMaximum()<<"; min="<<hist_clone->GetMinimum()<<endl;
//	hist_clone->GetYaxis()->SetRangeUser(hist_clone->GetMinimum()*0.2,hist_clone->GetMaximum()*100);
...
//                 cout<<"bin"<<i+1<<" "<<y_ratio[i]<<" "<< yerror_down_ratio[i]<<" "<<yerror_down[i]<<" "<<nomNoErr->GetBinContent(i+1)  <<endl;
