...
void run(TFile*file, TString cut1,TString tag,int num,bool turn){
     ...
     if(name.Contains("EWK")){
	     ...
	     if(turn==1)	     
		     fout= new TFile("./hist_interfout_scale"+tag+".root","recreate");
	     else
		     fout= new TFile("./hist_interf_scale"+tag+".root","recreate");
     }
     else{
	     if(turn==1)	     
		     fout= new TFile("./hist_qcdout_scale"+tag+".root","recreate");
	     else
...
//		run(file2[i], cut1,tag[i],9,0);
//		run(file2[i], cut2,tag[i],9,1);
		run(file3[i], cut1,tag[i],3,0);
		run(file3[i], cut2,tag[i],3,1);
	}
	return 1;
}
