...
dir16="/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2016/data_crads/muon/"
dir17="/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2017/data_crads/muon/"
dir18="/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2018/data_crads/muon/"
...
fake   =[1.22,1.17,1.24,1.14,1.14,1.11]
interf =[1.02,1.02,1.01,1.00,1.00,1.00]
l1pref= [1.03,1.03,1.03,1.02,1.04,1.04]
...
for i in range(1,kk):
   f = open('./txt/%s_bin_%d.txt'%('muon', i),'w')
...
# bin error
   ...
   f.write('bin\tmuon%i\tmuon%i\tmuon%i\tmuon%i\n'%(i,i,i,i))
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t#0. uncertainty on muon_RunII\n'%('muon','RunII'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t%0.2f\t-\t-\t#0. uncertainty on muon_RunII\n'%('muon','RunII',fake[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\n'%('muon','RunII',i,ZA_sig_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\n'%('muon','RunII',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\n'%('muon','RunII',i,non_prompt_binerror))
   f.write('others_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\n'%('muon','RunII',i,others_binerror))
   ...
   f.write('muon16_trigger\tlnN\t1.02\t1.02\t-\t1.02\n')
   f.write('muon17_trigger\tlnN\t1.02\t1.02\t-\t1.02\n')
   f.write('muon18_trigger\tlnN\t1.02\t1.02\t-\t1.02\n')
   f.write('muon16_efficiency\tlnN\t1.005\t1.005\t-\t1.005\n')
   f.write('muon17_efficiency\tlnN\t1.005\t1.005\t-\t1.005\n')
   f.write('muon18_efficiency\tlnN\t1.005\t1.005\t-\t1.005\n')
