...
//	TTree* t = (TTree*)f1->Get("ZPKUCandidates");
	...
	Double_t Mjj,zepp,delta_phi,ZGmass;
	Double_t detajj;
...
//	Double_t mjj_bins[4]={500, 750, 1000, 2000};
	...
	for(Int_t j=0; j<t->GetEntries();j++)
	{
        p=0;
		t->GetEntry(j);
        if( !(Mjj>500 && detajj>2.5 && zepp<2.4 && delta_phi>1.9 && ZGmass>100) ) continue;
        for(Int_t i=0;i<num;i++){
            int k = 15*i;
...
int d_hist(){
	run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2017/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2017_ZA-EWK-pweight");
