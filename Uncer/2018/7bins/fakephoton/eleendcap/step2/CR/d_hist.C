...
//	TTree* t = (TTree*)f1->Get("demo");
	...
	{
		t->GetEntry(j);
		if( !(lep==11 && fabs(photoneta)>1.566 && fabs(photoneta)<2.5 && ZGmass>100) ) continue;
//		cout<<"scalef "<<scalef<<endl;
		if(Mjj<500 && deltaetajj<6.5)   th2[i]->Fill(Mjj,deltaetajj,scalef);
		if(Mjj<500 && deltaetajj>=6.5)  th2[i]->Fill(Mjj,6.1,scalef);
...
int d_hist(){
	TH2D* h0 = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2018/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2018_plj_eleendcap.root",0);
	TH2D* hup = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2018/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2018_plj_eleendcap_up.root",1);
	TH2D* hdown = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2018/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2018_plj_eleendcap_down.root",2);
