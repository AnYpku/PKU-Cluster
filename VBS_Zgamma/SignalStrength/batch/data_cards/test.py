...
fdir = '/afs/cern.ch/user/y/yian/work/PKU-Cluster/SignalStrength/batch/root/'
...
genbinerror=[]
arr={}
f=open('/afs/cern.ch/user/y/yian/work/PKU-Cluster/SignalStrength/batch/Uncer/summary_uncer_'+sys.argv[2]+sys.argv[1]+'.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
# bin error
   ...
   data= ZA_sig_bincontent+ ZA_bincontent+non_prompt_bincontent+TTA_bincontent+VV_bincontent+ST_bincontent+WA_bincontent+ZA_sig_out_bincontent
   ...
   f.write('bin\t')
   f.write('%s%i\t%s%i\t%s%i\t%s%i\t%s%i\t%s%i\t%s%i\t%s%i\n'%(sys.argv[2],i,sys.argv[2],i,sys.argv[2],i,sys.argv[2],i,sys.argv[2],i,sys.argv[2],i,sys.argv[2],i,sys.argv[2],i))
   f.write('process\t')
   f.write('Sig\tQCD\tnon_prompt\tTTA\tVV\tST\tWA\tSig_out\n')
   f.write('process\t0\t1\t2\t3\t4\t5\t6\t7\n')
   f.write('rate\t')
   f.write('%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n'%(ZA_sig_bincontent,ZA_bincontent, non_prompt_bincontent, TTA_bincontent, VV_bincontent, ST_bincontent, WA_bincontent,ZA_sig_out_bincontent))
   f.write('------------\n')
   ...
   f.write('-\t-\t-\t-\t-\t-\t-\t%0.2f\n'%(ZA_sig_out_binerror))
   f.write('Stat group = VBS_Stat_bin%d_%s%s QCD_Stat_bin%d_%s%s non_prompt_Stat_bin%d_%s%s TTA_Stat_bin%d_%s%s VV_Stat_bin%d_%s%s ST_Stat_bin%d_%s%s WA_Stat_bin%d_%s%s ZA_SigOut_Stat_bin%d_%s%s\n'%(i,sys.argv[2],sys.argv[1],i,sys.argv[2],sys.argv[1],i,sys.argv[2],sys.argv[1],i,sys.argv[2],sys.argv[1],i,sys.argv[2],sys.argv[1],i,sys.argv[2],sys.argv[1],i,sys.argv[2],sys.argv[1],i,sys.argv[2],sys.argv[1]))
...
#   
      ...
      f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(arr['muon_all'][0],arr['ele_reco'][0],arr['ele_reco'][0],arr['ele_reco'][0],arr['ele_reco'][0],arr['ele_reco'][0],arr['ele_reco'][0]))
#
      f.write('ele_ID\tlnN\t')
      f.write('%0.2f\t%0.2f\t-\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n'%(arr['ele_ID'][0],arr['ele_ID'][0],arr['ele_ID'][0],arr['ele_ID'][0],arr['ele_ID'][0],arr['ele_ID'][0],arr['ele_ID'][0]))
      f.write('ele group = ele_reco ele_ID\n')
#
