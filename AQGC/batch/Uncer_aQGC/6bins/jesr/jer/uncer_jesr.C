	...
	for(Int_t i=0;i<num;i++){
		...
		if(bincontent_new[i]<=0||h1->GetSum()==0) uncer[i] =0;
			...
			if(sample.Contains("ZA")){
				...
				uncer[i]=std::max( fabs(h2->GetSum()-h1->GetSum()),fabs(h3->GetSum()-h1->GetSum()) )/h1->GetSum();
				if(uncer[i]>=1) uncer[i]=1;
			}
		}
...
void run(TString sample){
	ofstream f1("./"+sample+"_jer_uncer.txt");
	fstream ftxt("./jer_uncer.txt", ios::app );
	TFile*file1 = new TFile("./hist_"+sample+"_jer16.root");
	TFile*file2 = new TFile("./hist_"+sample+"_jer17.root");
	TFile*file3 = new TFile("./hist_"+sample+"_jer18.root");
