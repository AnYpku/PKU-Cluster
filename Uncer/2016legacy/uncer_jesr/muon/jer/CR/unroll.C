...
void cmsLumi(bool channel) 
{
        ...
        latex.DrawLatex(0.76,0.92,Form("35.68 fb^{-1} (%d TeV)", (beamcomenergytev)));
}
...
//    TH1D* h1_out= new TH1D(hname, "", nbins-1, 0, nbins-1);
    for(Int_t iy=1; iy<=nbinsy; iy++){
	    for(Int_t ix=1; ix<=nbinsx; ix++){
		    ...
		    cout<<"ix = "<<ix<<", iy = "<<iy<<"; bin = "<<ix+(iy-1)*nbinsx<<", BinContent"<<35.68*th2->GetBinContent(th2->FindBin(x_temp, y_temp))<<endl;
	    }
    }
 /*   for(Int_t i=0;i<6;i++){
...
//          t_ZA[i]->Scale(scale_factor[i]);
       ...
       line->SetLineWidth(1);
       line->Draw();
       c1->Print("./figs/hist-"+filename+"_2d.eps");
       c1->Print("./figs/hist-"+filename+"_2d.pdf");
