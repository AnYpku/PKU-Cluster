...
void rm::Loop(TString name)
{
   ...
   Double_t ptBins[3]={100,150,1000};

//   RooUnfoldResponse K(5,ptBins[0],ptBins[5],5,ptBins[0],ptBins[5]);
   TH2D* h2 = new TH2D("h2", "gen selection", 2, ptBins,2,ptBins);
   TH2D* h3 = new TH2D("h3", "gen && reco selection",2, ptBins,2,ptBins);
   TFile* file = new TFile("hist_"+name+".root","recreate");
//   TH1D* hist1[5]; 
   TH1D* hist5[2];
   TH1D* hist2= new TH1D("hist",name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f  reco && gen;leding p_{T}^{reco};yields",ptBins[0],ptBins[2]),2,ptBins);
   TH1D* hist1= new TH1D("hist1",name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f  reco;leding p_{T}^{reco};yields",ptBins[0],ptBins[2]),2,ptBins);
   TH1D* hist4= new TH1D("hist4",name+Form("\t\t %0.f<leading p_{T}^{gen}<%0.f  gen;leding p_{T}^{gen};yields",ptBins[0],ptBins[2]),2,ptBins);
   TH1D* hist3= new TH1D("hist3",name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f reco out of gen;leding p_{T}^{gen};yields",ptBins[0],ptBins[2]),2,ptBins);
   for(int i=0;i<2;i++)
        {
//		hist1[i]= new TH1D(name+Form("_hist_%i",i+1),name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f;leding p_{T}^{gen};yields",ptBins[i],ptBins[i+1]),1,ptBins[i],ptBins[i+1]);
		hist5[i]= new TH1D(Form("hist5_%i",i+1),name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f;leding p_{T}^{gen};yields",ptBins[i],ptBins[i+1]),2,ptBins);
	}
...
//      Gen =  GenLEPele  && GenPhoton && GenJet && GenDr && GenSignalRegion;
      ...
      if(genZGmass>1000) genZGmass=999;if(Mva>1000) Mva=999;
      if(name.Contains("EWK") ){ 
              if(Reco){
		      hist1->Fill(Mva,scalef*lumi);
		      i1++;
	      }
              if(Gen){
		      i2++;
                      h2->Fill(Mva,genZGmass,scalef*lumi);
		      hist4->Fill(genZGmass,scalef*lumi);
	      }
              if(Reco && Gen){ 
		      i3++;
		      hist2->Fill(Mva,scalef*lumi);
                      h3->Fill(Mva,genZGmass,scalef*lumi);
	      }
              if(Reco&& !Gen) {
                      i4++;
		      hist3->Fill(Mva,scalef*lumi);
	      }
      }//signal out gen bincontent
      if(name.Contains("EWK")==0 && !(name.Contains("Muon")==1 || name.Contains("Data")==1) && Reco){
	      i1++;
	      if(name.Contains("plj")){
		      hist2->Fill(Mva,scalef);
	      }
	      else{ 
		      hist2->Fill(Mva,scalef*lumi);}
      }//bkg bincontent
      if( (name.Contains("Muon")==1 || name.Contains("Data")==1) && Reco){
	      hist2->Fill(Mva);}//data bincontent
      for(int kk=0;kk<2;kk++){
	      if(name.Contains("EWK") && Reco && Gen ){ 
		      if(Mva>ptBins[kk] && Mva<ptBins[kk+1]){
			      hist5[kk]->Fill(genZGmass,scalef*lumi);
		      }
	      }//5 genbins content in different reco pt bins 
	      /*if(name.Contains("EWK")==0 && !(name.Contains("Muon")==1 || name.Contains("Data")==1) && Reco){
		      if(ptlep1>ptBins[kk] && Mva<ptBins[kk+1]){
			      if(name.Contains("plj")) {hist1[kk]->Fill(Mva,scalef);}
			      else hist1[kk]->Fill(Mva,scalef*lumi);
		      }
	      }
	      if( (name.Contains("Muon")==1 || name.Contains("Data")==1) && Reco){
		      if(Mva>ptBins[kk] && Mva<ptBins[kk+1]){
