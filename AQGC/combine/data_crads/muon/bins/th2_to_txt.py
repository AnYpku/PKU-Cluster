...
for i in range(1,num+1):
   f = open('./txt/%s_bin_%d.txt'%('muon_all', i),'w')
...
# bin error
   ...
   f.write('------------\n')
   f.write('# now we list the expected events for signal and muon_all backgrounds in that bin\n')
...
#       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t#0. uncertainty on muon_all_full\n'%('ele','full'))
#   else: 
#       f.write('fake_%s_%s\tlnN\t-\t%0.2f\t-\t-\t#0. uncertainty on muon_all_full\n'%('ele','full',fake[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\n'%('muon_all','full',i,ZA_sig_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\n'%('muon_all','full',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\n'%('muon_all','full',i,non_prompt_binerror))
   f.write('others_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\n'%('muon_all','full',i,others_binerror))
...
#   f.write('muon_trigger\tlnN\t1.02\t1.02\t-\t1.02\n')
#   f.write('muon_efficiency\tlnN\t1.023\t1.023\t-\t1.023\n')
