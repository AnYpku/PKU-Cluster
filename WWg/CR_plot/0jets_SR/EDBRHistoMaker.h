...
void EDBRHistoMaker::Loop(std::string outFileName,float luminosity,int isBarrel,TString isChannel) {
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && lep1_is_tight==1 && lep2_is_tight==1 && mll >20 && ptll > 30 && n_photon>0  && photonet > 20. && drl1a>0.5 && drl2a>0.5 && photon_channel && photon_selection==1 && PuppiMET_T1_pt > 20 && n_bjets20_medium==0 &&  mT> 50 && mT2>20 && njets30==0){
...
//		actualWeight = scalef*ele_id_scale*ele_reco_scale*muon_id_scale*muon_iso_scale*photon_id_scale*photon_veto_scale*puWeight*btag_weight_loose*PrefireWeight;//mc
		...
		if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && mll >20 && ptll > 30 && lepton_flag && n_photon>0  && photonet > 20. && drl1a>0.5 && drl2a>0.5 && photon_channel && photon_flag==1 && PuppiMET_T1Smear_pt > 20 && n_bjets20_medium==0 && mT > 50 && mT2>20 && njets30==0 ){
			if(gen_weight>0) npp++;
			if(gen_weight<0) nmm++;
			numbe_out++;
