...
//	Double_t bins[7]={0.0172,0.0222,0.0272,0.0322,0.0372,0.0472,0.0572};
	...
	RooPlot* xframe = sieie.frame(Title(Form("Endcap region, %0.f GeV < photon PT < %0.f GeV,%0.f <chiso<%0.f ",lowpt, highpt,lowchiso,highchiso)), Bins(nBins));
	...
	TString strFR = "FR = (";
        ...
        strFR += (FRErrInt%10);
        strFR += ")%";
        TLatex* textFR = new TLatex(0.63,0.3,strFR);
        textFR->SetNDC();
