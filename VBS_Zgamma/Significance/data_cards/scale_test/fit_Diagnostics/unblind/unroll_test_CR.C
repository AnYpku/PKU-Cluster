...
void run(TString channel,TString tag){
	...
	if(channel.Contains("mu")) par="Muon";
        ...
        TFile*f2=new TFile("/home/pku/anying/cms/PKU-Cluster/Significance/CR/root/hist_"+par+"_"+tag+channel+".root");        
...
//        latex.DrawLatex(6.1,0.8*max,"#Delta#eta_{jj}>6");
	...
	for(int i=0;i<gr1->GetN();i++){
        ...
        leg2->AddEntry(htot,Form("Pred [%0.f #pm %0.f]",htot->GetSum(),err_tot),"f");
        leg2->AddEntry(h_data,Form("Data [%0.f #pm %.0f]",h_data->GetSum(),err_data));
        fPads1->cd();
        leg2->Draw();
        fPads1->Update();
        fPads2->cd();
