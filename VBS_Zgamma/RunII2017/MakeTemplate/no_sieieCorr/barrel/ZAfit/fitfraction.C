...
//        fit->Constrain(1,0,1);
        ...
        if (status == 0) {                       // check on fit status
	        ...
	        double fit_err = mcp0->Integral(0, mcp0->FindFixBin(0.01015)-1)*result->Integral()*errP0/mcp0->Integral();
            fr = p0* (hfake->Integral(0,hfake->FindFixBin(0.01015)-1)/hfake->Integral())/(result->Integral(0,result->FindFixBin(0.01015)-1)/result->Integral());

            fakeValue = mcp0->IntegralAndError(0,nBins,fakeError); 
            fake_window = mcp0->IntegralAndError(0,mcp0->FindFixBin(0.01015)-1,fake_window_Error);
	        real_window = mcp1->IntegralAndError(0,mcp1->FindFixBin(0.01015)-1,real_window_Error);
            data_window = result->IntegralAndError(0,result->FindFixBin(0.01015)-1,data_window_Error);
