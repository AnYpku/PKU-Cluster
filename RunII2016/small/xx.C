...
//  nentries=1000;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if(drj2a==10) drj2a=-1;
/*      int p=0;
      for(int i=446;i<703;i++){
	      weight[p]=pweight[i];
	      p++;
      }*/

      LEPmu = (HLT_Mu1>0||HLT_Mu2>0||HLT_Mu3>0)&&lep == 13 && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep > 70. && massVlep < 110.;
      LEPele = (HLT_Ele1>0||HLT_Ele2>0)&&lep==11 && ptlep1 > 25. && ptlep2 > 25.&& fabs(etalep1) < 2.5 &&abs(etalep2) < 2.5 && nlooseeles < 3 && nloosemus == 0  && massVlep >70.&& massVlep<110;
...
//      if(lep==11)
//	      scalef=scalef*pileupWeight*photon_id_scale*photon_veto_scale*ele1_id_scale*ele2_id_scale*ele1_reco_scale*ele2_reco_scale*prefWeight;
//      if(lep==13)
//	      scalef=scalef*pileupWeight*photon_id_scale*photon_veto_scale*muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale*muon1_track_scale*muon2_track_scale*prefWeight;
      JET=jet1pt> 30 && jet2pt > 30 && fabs(jet1eta)< 4.7 && fabs(jet2eta)<4.7;
      DR =drla>0.7 && drla2>0.7 && drj1a>0.5 && drj2a>0.5;
      if(jentry%1000000==0)   cout<<jentry<<"; "<<nentries<<"; cut1 = "<<cut1<<endl;
      if(  (LEPele||LEPmu) && PHOTON /*&& JET && DR && SignalRegion*/ )
         {
