...
void run(TString sample,TString vec_branchname,TString reco,vector<double> bins,TString cut1[num],int kk,TString tag){
     ...
     if(vec_branchname.Contains("Mjj")){
	     ...
	     tree->SetBranchAddress(reco+"_JER_up", &variables[reco+"_JER_up"]);
	     tree->SetBranchAddress(reco+"_JER_down", &variables[reco+"_JER_down"]);
     }
     ...
     for(int k=0;k<tree->GetEntries();k++){
	     ...
	     if(vec_branchname.Contains("Mjj")){
		     var_reco[0]=variables[reco+"_new"];var_reco[1]=variables[reco+"_JER_up"];
		     var_reco[2]=variables[reco+"_JER_down"];
	     }
	     for(int i=0;i<3;i++){
     ...
     TFile*fout[kk];
     for(int j=0;j<kk;j++){
	     fout[j]=new TFile("./unfold_"+sample+"_"+vec_branchname+Form("_recobin%d",j+1)+"_jer"+tag+".root","recreate");
     }
     for(int j=0;j<kk;j++){
	     fout[j]->cd();
	     for(Int_t i=0;i<num;i++){
