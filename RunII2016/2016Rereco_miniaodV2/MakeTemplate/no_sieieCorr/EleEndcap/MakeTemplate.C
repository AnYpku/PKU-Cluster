    ...
    position = distance( begin(vector_pt), biggest_pt);
      ...
      for(Int_t k=0;k<num;k++){
         if(name.Contains("A")==1){
            actualWeight =scalef*ele1_id_scale*ele2_id_scale*ele1_reco_scale*ele2_reco_scale*prefWeight*pileupWeight;
	    double sieie;
//	    sieie = gr->Eval(photon_sieie[position]);
	    sieie = photon_sieie[position];
            if(photon_isprompt[position]==1 && photon_chiso[position]<0.442 && photon_pt[position]<highpt[k] && photon_pt[position]>lowpt[k])
               { h1[k]->Fill(sieie,actualWeight);m1[k] +=actualWeight;}//true
