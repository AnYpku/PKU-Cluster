...
void unroll_name(TString channel,TString tag){
	...
	fPads2->Update();
	c1->Print("./figs/plj_"+channel+"_"+tag+".eps");
	c1->Print("./figs/plj_"+channel+"_"+tag+".pdf");
...
int unroll_test(){
	...
	TString channel[4]={"elebarrel","eleendcap","mubarrel","muendcap"};
	for(int i=0;i<4;i++){
		unroll_name(channel[i],"16");
		unroll_name(channel[i],"17");
		unroll_name(channel[i],"18");
	}
	return 0;
}
