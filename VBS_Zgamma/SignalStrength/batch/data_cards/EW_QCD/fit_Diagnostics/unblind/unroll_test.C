...
void run(TString channel,TString tag){
	...
	th2_ZA_sig_out->SetFillColor(kRed-7);
	th2_ZA_sig_out->SetMarkerColor(kRed-7);
        th2_ZA_sig_out->SetLineColor(kRed-7);
...
//      TLine* line = new TLine(th2_ZA->GetXaxis()->GetXmin(),0,th2_ZA->GetXaxis()->GetXmax(),0);
	...
	leg->AddEntry(th2_ZA_sig_out,Form("Z#gamma_{out}[%.1f] ",th2_ZA_sig_out->GetSum()) );
...
////
	...
	c1->SaveAs("aa_QCD_"+channel+tag+".pdf");
}
int unroll_test(){
