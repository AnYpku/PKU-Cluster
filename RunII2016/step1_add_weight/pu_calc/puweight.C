...
//	TFile * input1 = new TFile ("MyDataPileupHistogram_nominal.root");
	...
	TFile*fout = new TFile("puweight_up.root", "RECREATE");
//	TFile*fout = new TFile("puweight_down.root", "RECREATE");
//	TFile*fout = new TFile("puweight.root", "RECREATE");
	TH1D *h2   = new TH1D("h2","",75,0,75);
	for(unsigned int npu = 0; npu < nPUMax; ++npu) {
