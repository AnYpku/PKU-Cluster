...
//		    cout<<"ix = "<<ix<<", iy = "<<iy<<"; bin = "<<ix+(iy-1)*nbinsx<<", BinContent"<<35.9*th2->GetBinContent(th2->FindBin(x_temp, y_temp))<<endl;
	    }
    }
    for(Int_t i=0;i<6;i++){
...
//    h1_out->SetBinError(8,h1->GetBinError(9));
    delete h1;
    return h1_out;
}
...
//       cmsLumi(0);
       ...
       nominal->SetMarkerStyle(22);
       nominal->Draw();
       h_up->Draw("same hist ][");
       h_down->Draw("same hist ][");
       c1->SaveAs("./figs/hist-2d_"+filename+".eps");
       c1->SaveAs("./figs/hist-2d_"+filename+".pdf");
