...
void rm::Loop(TString name)
{
   ...
   Double_t ptBins[5]={30,150,250,350,800};

//   RooUnfoldResponse K(5,ptBins[0],ptBins[5],5,ptBins[0],ptBins[5]);
   TH2D* h2 = new TH2D("h2", "gen selection", 4, ptBins,4,ptBins);
   TH2D* h3 = new TH2D("h3", "gen && reco selection",4, ptBins,4,ptBins);
...
//   TH1D* hist1[5]; 
   ...
   double uf[3]={1,2,0.5};
   for(int i=0;i<3;i++){
         	  hist1[i] = new TH1D(Form("hist1_%i",i),Form("\t\t %0.f<leading p_{T}<%0.f  reco  #mu_{f}=%2.1f;leding p_{T}^{reco};yields",ptBins[0],ptBins[4],uf[i]),4,ptBins);
		  hist2[i] = new TH1D(Form("hist2_%i",i),Form("\t\t %0.f<leading p_{T}<%0.f  reco && gen  #mu_{f}=%2.1f ;leding p_{T}^{gen};yields",ptBins[0],ptBins[4],uf[i]),4,ptBins);
		  hist3[i] = new TH1D(Form("hist3_%i",i),Form("\t\t %0.f<leading p_{T}<%0.f reco && gen  #mu_{f}=%2.1f ;leding p_{T}^{reco};yields",ptBins[0],ptBins[4],uf[i]),4,ptBins);
		  hist4[i] = new TH1D(Form("hist4_%i",i),Form("\t\t %0.f<leading p_{T}<%0.f  gen #mu_{f}=%2.2f;leding p_{T}^{gen};yields",ptBins[0],ptBins[4],uf[i]),4,ptBins);
   }
   for(int i=0;i<4;i++)
        {
//		hist1[i]= new th1d(name+Form("_hist_%i",i+1),name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f;leding p_{T}^{gen};yields",ptBins[i],ptBins[i+1]),1,ptBins[i],ptBins[i+1]);
		hist5[i]= new TH1D(Form("hist5_%i",i+1),name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f;leding p_{T}^{gen};yields",ptBins[i],ptBins[i+1]),4,ptBins);
	}
...
//      Gen =  GenLEPele  && GenPhoton && GenJet && GenDr && GenSignalRegion;
      ...
      if(genphotonet>400) genphotonet=399;if(photonet>400) photonet=399;
      if(name.Contains("EWK") ){ 
              if(Reco){
                      for(int i=0;i<3;i++){
			      hist1[i]->Fill(jet1pt,scalef*lumi*pweight[i]);
		      }
		      i1++;
	      }
              if(Gen){
		      i2++;
                      h2->Fill(jet1pt,genjet1pt,scalef*lumi);
		      for(int i=0;i<3;i++){
			      hist4[i]->Fill(genjet1pt,scalef*lumi*pweight[i]);
		      }
	      }
              if(Reco && Gen){ 
		      i3++;
		      for(int i=0;i<3;i++){
			      hist2[i]->Fill(genjet1pt,scalef*lumi*pweight[i]);
			      hist3[i]->Fill(genjet1pt,scalef*lumi*pweight[i]);
		      }
                      h3->Fill(jet1pt,genjet1pt,scalef*lumi);
	      }
              if(Reco&& !Gen) {
                      i4++;
	      }
     ...
     /* if(name.Contains("EWK")==0 && !(name.Contains("Muon")==1 || name.Contains("Data")==1) && Reco){
	      ...
	      if(name.Contains("plj")){
		      hist2->Fill(photonet,scalef);
	      }
	      else{ 
		      hist2->Fill(photonet,scalef*lumi);}
      }//bkg bincontent
      */
      /*if( (name.Contains("Muon")==1 || name.Contains("Data")==1) && Reco){
	      hist2->Fill(photonet);}//data bincontent
	      */
      for(int kk=0;kk<4;kk++){
	      if(name.Contains("EWK") && Reco && Gen ){ 
		      if(photonet>ptBins[kk] && photonet<ptBins[kk+1]){
			      hist5[kk]->Fill(genphotonet,scalef*lumi);
		      }
