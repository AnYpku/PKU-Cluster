...
//  hs->GetYaxis()->CenterTitle();
	...
	double maximumMC = 2.0 * sumMC->GetMaximum();
	double maximumDATA = -100;
	if (isDataPresent_)
		maximumDATA = 2.15 * sumDATA->GetMaximum();
	...
	TLegend* leg1 = new TLegend(0.6, 0.55, 0.94, 0.9);
	TLegend* leg2 = new TLegend(0.25, 0.6, 0.5, 0.9);
...
//                double yieldsData = sumDATA->IntegralAndError(0,sumDATA->GetNbinsX(),yieldsDataErr);;
                double yieldsData = sumDATA->IntegralAndError(0,sumDATA->GetNbinsX(),yieldsDataErr);
                char yData[100];sprintf(yData,"%.2f",yieldsData);
                char yDataErr[100];sprintf(yDataErr,"%.2f",yieldsDataErr);
                TString samplesData = "Data";
                TString LabelData = samplesData +" ["+ yData+"+/-"+yDataErr+"]";
                ftxt<<std::setw(10)<<samplesData.Data()<<" "<<yData<<"$pm$"<<yDataErr<<""<<endl;
...
//
	...
	for (size_t i = 0; i != histosMC.size(); ++i) {
                double yerr;
		double yields;
                yields=histosMC.at(i)->IntegralAndError(0,histosMC.at(i)->GetNbinsX(),yerr);
//		double yields = histosMC.at(i)->GetSumOfWeights();
		...
		ftxt<<std::setw(10)<<samples<<" "<<y<< "$pm$"<<ye<<""<<endl;
		cout<<LabelMC<<endl;
	}
		...
		for (size_t i = 0; i != histosMCSig.size(); ++i) {
			...
			if (kFactorsSig_.at(i) != 2.0) {
				if (i == 0){
					...
					yieldsMCSig= histosMCSig.at(i)->IntegralAndError(0,histosMC.at(i)->GetNbinsX(),yieldsMCSigErr);
					...
					ftxt<<std::setw(10)<<samplesMCSig<<" "<<ySig<< "$pm$"<<ySigErr<<""<<endl;
