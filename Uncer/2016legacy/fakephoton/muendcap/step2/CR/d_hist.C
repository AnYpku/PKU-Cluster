...
//	TTree* t = (TTree*)f1->Get("demo");
	...
	{
		t->GetEntry(j);
		if( !(lep==13 && fabs(photoneta)>1.566 && fabs(photoneta)<2.5 && ZGmass>100) ) continue;
...
int d_hist(){
	TH2D* h0 = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2016/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2016_plj_mubarrel.root",0);
	TH2D* hup = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2016/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2016_plj_mubarrel_up.root",1);
	TH2D* hdown = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2016/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2016_plj_mubarrel_down.root",2);
