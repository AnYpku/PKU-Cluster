void run(TString var,TString tag,TString channel){
	ofstream f1("./"+var+"_l1pref_"+tag+channel+"_uncer.txt");
	TFile* file = new TFile("./unfold_"+var+"_ewk_pref_"+tag+channel+".root");
...
//     vector<TString> genvars={"ptlep1","photonet","jet1pt"/*,"Mjj"*/};
     vector<TString> genvars={"Mjj"};
     vector<TString> channels={"mubarrel","muendcap","elebarrel","eleendcap"};
     for(int i=0;i<genvars.size();i++){
	     for(int j=0;j<channels.size();j++){
		     run(genvars[i],"16",channels[j]);
		     run(genvars[i],"17",channels[j]);
	     }
     }
     return 1;
}
