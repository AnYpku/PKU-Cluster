...
//	   cout<<"scalef = "<<scalef<<"; pileupWeight = "<<pileupWeight<<"; actualWeight = "<<actualWeight<<endl;
	   for(int i=0;i<3;i++){
		   if(JET_JEC[i] && Mjj_JEC[i]<500 && deltaeta_JEC[i]<6.5 ){ 
...
void jesrUncer::makefile(TString filename){
	TFile *file = new TFile("2d_"+filename+"_jec.root","RECREATE");
        TH2D* h2[3];
	Loop(h2);
	cout<<"OK"<<endl;
	file->cd();
