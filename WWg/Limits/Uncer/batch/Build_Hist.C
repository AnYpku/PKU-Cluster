...
void run(TString dir,TString name,TString cut,TString tag,TString njets,TString vec_branchname,vector<Double_t> bins1,vector<Double_t> bins2,TString type,int num){
     ...
     TTree*tree;
     if(fname.Contains("plj")==0) tree=(TTree*)file->Get("outtree");
...
int Build_Hist(){
	...
	TString LEP = "((HLT_emu1||HLT_emu2||HLT_emu3||HLT_emu4) && channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && ptlep1>25 && ptlep2>13 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.5 && n_loose_ele==1 && n_loose_mu==1 && ptll>30 && mll>20 && lep1_charge*lep2_charge<0 && drll>0.5 && lep1_isprompt==1 && lep2_isprompt==1 && lep1_is_tight==1 && lep2_is_tight==1 && drll<2.3 && ( fabs(lep1phi-lep2phi)<2.3 || (2*"+pi+"-fabs(lep1phi-lep2phi))<2.3 ) )";
	TString photon = "(n_photon>0  && photonet > 20. && ( (fabs(photoneta) < 1.4442) ||  (fabs(photoneta) < 2.5 && fabs(photoneta)>1.566) ) && drl1a>0.8 && drl2a>0.8 )";
	TString met;
	...
	for(int ij=0;ij<njets.size();ij++){
		...
		for(int ik=0;ik<types.size();ik++){
				...
				names={"TTGJets","ST","WWG_emu","HGC","HGS","HGU","HGD"};
				dir1="/home/pku/anying/cms/PKU-Cluster/WWg/CR_plot/Hgamma/output-slimmed-rootfiles/optimal_emua_";
			}
			else{
				names={"ZGJets","TTGJets","VV","ST","tZq","TGJets","WGJets","WWG_emu","HGC","HGS","HGU","HGD"};
				dir1="/home/pku/anying/cms/PKU-Cluster/WWg/CR_plot/Hgamma/output-slimmed-rootfiles/optimal_emua_";
			}
...
//				if(names[j].Contains("WWG_emu")==0) continue;
				if(names[j].Contains("Muon")==0 && names[j].Contains("plj")==0 )
					met="(n_bjets_nom==0 && PuppiMET_T1Smear_pt > 20 && mT>40 && mT2>20 && "+jet_cut+")";
				else
					met="(n_bjets_nom==0 && PuppiMET_T1_pt > 20 && mT>40 && mT2>20 && "+jet_cut+")";
				Reco= LEP+"&&"+photon+"&&"+met;
				for(int k=1;k<tags.size();k++){
					if(types[ik].Contains("l1pref") && tags[k].Contains("18"))
						continue;
					for(int i=0;i<vars.size();i++){
						run(dir1,names[j],Reco,tags[k],njets[ij],vars[i],mT_bins,drll_bins,types[ik],num);
					}
				}
			}
		}
