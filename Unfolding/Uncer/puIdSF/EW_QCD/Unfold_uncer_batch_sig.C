#define num 3
void run(TFile*file, TString vec_branchname,TString reco,vector<double> bins,TString cut1,TString tag,int kk,TString type){
     ...
     for(Int_t j=0;j<kk;j++){//Loop the binning cuts
	     for(Int_t i=0;i<num;i++){
		     th1name[i][j]=vec_branchname+Form("_%d_",i)+Form("recobin%d",j+1);
     ...
     TFile*fout[kk];
     for(int j=0;j<kk;j++){
//	     if(vec_branchname.Contains("Mjj")==0) fout[j]= new TFile("unfold_"+vec_branchname+Form("_recobin%d",j+1)+"_qcd_"+type+tag+".root","recreate");
//	     else fout[j]= new TFile("unfold_2d"+vec_branchname+Form("_recobin%d",j+1)+"_qcd_"+type+tag+".root","recreate");
	     if(vec_branchname.Contains("Mjj")==0) fout[j]= new TFile("unfold_"+vec_branchname+Form("_recobin%d",j+1)+"_interf_"+type+tag+".root","recreate");
	     else fout[j]= new TFile("unfold_2d"+vec_branchname+Form("_recobin%d",j+1)+"_interf_"+type+tag+".root","recreate");
     }
     for(int j=0;j<kk;j++){
	     fout[j]->cd();
	     for(Int_t i=0;i<num;i++){
...
//		file[i]=new TFile(dir[i]+"unfold_GenCutla-ZA"+tag[i]+".root");
		file[i]=new TFile(dir[i]+"unfold_GenCutla-ZA_interf"+tag[i]+".root");
//		if(tag[i].Contains("17")==0) continue;
		for(int j=0;j<genvars.size();j++){
			cout<<tag[i]<<" "<<genvars[j]<<endl;
			if(genvars[j].Contains("Mjj")==0){
				run(file[i],genvars[j], recovars[j], bins[j],cut1,tag[i],bins[j].size()-1,"eff");
				run(file[i],genvars[j], recovars[j], bins[j],cut1,tag[i],bins[j].size()-1,"mis");
			}
			else{
