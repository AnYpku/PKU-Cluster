double get_ele_ID(double etalep, double ptlep, TH2F* ID){
	...
	lep_ID_scale=ID->GetBinError(ID->FindBin(etalep,ptlep_tmp));
...
double get_ele_Reco(double etalep, double ptlep, TH2F* Reco){
	...
	lep_Reco_scale=Reco->GetBinError(Reco->FindBin(etalep,ptlep_tmp));
...
double get_photon_ID(double photoneta, double photonet, TH2F* ID_photon){
	...
	photon_ID_scale=ID_photon->GetBinError(ID_photon->FindBin(photoneta,photonet));
