...
//   Double_t pt[11]={20,25,30,35,40,45,50,60,80,120,400};
   Double_t pt[7] = {20,30,40,50,60,100,400};
...
//   nentries=100;
  ...
  // if (Cut(ientry) < 0) continue;
      ...
      JETS=!(fabs(jet2eta)<3&&fabs(jet2eta)>2.7) && !(fabs(jet1eta)<3&&fabs(jet1eta)>2.7) ;
//jet1pt> 30 && jet2pt > 30 && fabs(jet1eta)< 4.7 && fabs(jet2eta)<4.7;
      ijet=0;
      if(name.Contains("outplj")||name.Contains("Data")) scalef =1;
      if(name.Contains("ZA"))  scalef = scalef*photon_id_scale*pileupWeight*prefWeight;
      for(Int_t j=0;j<6;j++)
      {
      ...
      if( (LEPmu || LEPele)&&PHOTON_endcap==true && JETS){ 
