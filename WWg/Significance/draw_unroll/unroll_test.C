...
void unroll_run(TString tag){
	...
	TFile* f_ZA_sig=TFile::Open("../root/hist_WWG_"+tag+".root");
	...
	TFile* f_fakeL=TFile::Open("../root/hist_fakeLepton_"+tag+".root");
        TH1D* th2_fakeL16=(TH1D*)f_plj->Get("hist_bkg");
        TH1D* th2_fakeL= unroll(th2_plj16,"t_fakeL");
...
//        TLine* line = new TLine(th2_ZA->GetXaxis()->GetXmin(),0,th2_ZA->GetXaxis()->GetXmax(),0);
	...
	hs->GetYaxis()->SetTitle("Events /bin");
        ...
        TLegend* leg = new TLegend(0.2, 0.6, 0.55, 0.9);
...
//	leg->AddEntry(th2_plj, Form("Nonprompt #gamma [%.2f]",th2_plj->GetSum() ));
//	leg->AddEntry(th2_fakeL, Form("Nonprompt lepton [%.2f]",th2_fakeL->GetSum() ));
//	leg1->AddEntry(th2_VV, Form("VV [%.2f]", th2_VV->GetSum() ) );
//	leg1->AddEntry(th2_TTA, Form("TTA [%.2f]",th2_TTA->GetSum()) );
//	leg1->AddEntry(th2_ST, Form("ST  [%.2f]",th2_ST->GetSum() ) );
...
////
        ...
        latex.SetTextSize(0.06);
        latex.SetLineWidth(2);
        latex.DrawLatex(0.5,1.1*max,"20<m_{#font[12]{ll}}<80");
        latex.DrawLatex(4.5,1.1*max,"80<m_{#font[12]{ll}}<120");
        latex.DrawLatex(8.5,1.1*max,"120<m_{#font[12]{ll}}<160");
        latex.DrawLatex(13, 1.1*max,"m_{#font[12]{ll}}>160");
