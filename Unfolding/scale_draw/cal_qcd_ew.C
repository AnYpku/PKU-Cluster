...
void run(TString var,int num,TString tag);
int cal_qcd_ew(){
	run("genMjj",9,"16");
//	run_full("genMjj",9);
return 0;
...
void run(TString var,int num,TString tag){
	TFile*f1=new TFile("./"+var+"_unfold_qcd_scale"+tag+".root");
	TFile*ff1=new TFile("./"+var+"_unfold_ewk_scale"+tag+".root");
        TH1D*h1[num];TH1D*h12[num];TH1D*h13[num];
        TH1D*hh1[num];TH1D*hh12[num];TH1D*hh13[num];
	for(int i=0;i<3;i++){
		hh1[i] =(TH1D*)ff1->Get(var+Form("_%i",i));
	...
	for (int i=0;i<num;i++){
		h1[i] =(TH1D*)f1->Get(var+Form("_%i",i));
		if(tag.Contains("16")&&i!=0) h1[i]->Scale(2);
	...
	cout<<"the "<<tag<<" scale unc. "<<error<<" in "<<endl;
	vec_content.clear();
}
