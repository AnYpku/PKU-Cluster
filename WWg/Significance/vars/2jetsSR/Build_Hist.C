...
void run(TString dir,TString name,TString cut,TString vec_branchname,vector<double> bins1,vector<double> bins2,TString tag){
     ...
     TH1D*hist=unroll(h2);
     TFile*fout=new TFile("./root/hist_"+name+"_"+vec_branchname+"_2jets_"+tag+".root","recreate");
     fout->cd();
     hist->Write();
     h2->Write();
     fout->Close();
	...
	for(int k=0;k<tags.size();k++){
		for(int j=0;j<names.size();j++){     
			...
			for(int i=1;i<vars.size();i++){     
				...
				run(dir1,names[j],Reco,vars[i],mT_bins,bins2[i],tags[k]);
			}
		}
	}
	return 1;
