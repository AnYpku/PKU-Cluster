...
//        double mur[num]={0.5,0.5,0.5,1,1,1,2,2,2};
	...
	for(Int_t i=1;i<num;i++){
		t_ZA[i]->SetLineColor(2);
		t_ZA[i]->SetLineWidth(2);
		t_ZA[i]->SetLineStyle(1);
		if(i==1) l2->AddEntry(t_ZA[1],"pdf weight");
        ...
        latex.DrawLatex(4.5,1.3*max,"80<m_{#font[12]{ll}}<140");
        latex.DrawLatex(8.5,1.3*max,"m_{#font[12]{ll}}>140");
	...
	TH1D*h_up=(TH1D*)t_ZA[0]->Clone("upper");
	TH1D*h_down=(TH1D*)t_ZA[0]->Clone("lower");
	for(int j=1;j<=nominal->GetNbinsX();j++){
		double n = nominal->GetBinContent(j);
		double err2Up = 0, err2Dn = 0;
//		double err2Up = pow(nominal->GetBinError(j),2);
//		double err2Dn = pow(nominal->GetBinError(j),2);
		for(int k=0;k<num;k++){
			double content= t_ZA[k]->GetBinContent(j);
			double err = content - n;
			err2Up+=pow(err,2);
			err2Dn+=pow(err,2);
		}
		h_up->SetBinContent(j,n+sqrt(err2Up/(num-1)+pow(nominal->GetBinError(j),2)));
		h_down->SetBinContent(j,n-sqrt(err2Dn/(num-1)+pow(nominal->GetBinError(j),2)));
		cout<<n<<" "<<sqrt(err2Up/(num-1))<<" "<<sqrt(err2Dn/(num-1))<<endl;
	}
	h_up->Divide(nominal);
	...
	nominal->GetYaxis()->SetRangeUser(0.2,2.1);
...
int unroll_test(){
	...
	for(int i=0;i<names.size();i++){
		if(names[i].Contains("ST") || names[i].Contains("WWG")) n=101;
		else n=103;
		unroll_name(names[i],"17",n);
		unroll_name(names[i],"18",n);
	}
	return 0;
}
