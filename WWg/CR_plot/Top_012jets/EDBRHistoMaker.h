...
class EDBRHistoMaker {
	public:
		EDBRHistoMaker(TTree *tree = 0, TFile *fileTMP = 0, TH1F* hR1 = 0, std::string out_name_="", RoccoR* rc_in=0,TString isChannel="",int n=0);
		...
		void Loop(std::string outFileName,double luminosity,int isBarrel,TString isChannel,int n);
		void Loop_SFs_mc(std::string outFileNamei,double luminosity,int isBarrel,TString isChannel, int n);
		// Our added functions
		void createAllHistos(TString isChannel,int n);
		void printAllHistos();
...
EDBRHistoMaker::EDBRHistoMaker(TTree* tree, TFile* fileTMP, TH1F* hR1, std::string out_name_, RoccoR* rc_in,TString isChannel, int n) {
	fChain = 0;
...
void EDBRHistoMaker::createAllHistos(TString isChannel, int n) {
	...
	for (int i = 0; i != nVars; ++i) {
		sprintf(buffer, "%s_%djets_"+isChannel, hs.vars[i].c_str(),n);
...
///----------------------------------------------------------------
void EDBRHistoMaker::Loop(std::string outFileName,double luminosity,int isBarrel,TString isChannel,int n) {
...
//        nentries=100;
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		actualWeight=1;
                ...
                if(isChannel=="emu") 
			lepton_channel= ((HLT_emu1||HLT_emu2||HLT_emu3||HLT_emu4) && channel==1 && lep1_pid==13 && lep2_pid==11 && lep1pt>20 && lep2pt>25 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.5);
                else if(isChannel=="ee") 
			lepton_channel= ((HLT_Ele1||HLT_Ele2) && channel==2 && lep1_pid==11 && lep2_pid==11 && lep1pt>25 && lep2pt>25 && fabs(lep1eta) < 2.5 && fabs(lep1eta) < 2.5);
                else if(isChannel=="mm") 
			lepton_channel= ((HLT_Mu1||HLT_Mu2) && channel==3 && lep1_pid==13 && lep2_pid==13 && lep1pt>20 && lep2pt>20 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.4);
	        ...
	        if( lepton_channel && (n_loose_ele + n_loose_mu) == 2 && n_photon>0  && photonet > 20. && photon_channel && puppimet > 20 && n_bjets >=1 && mll >50 && ptll > 30 && mT2>30 && njets==n){
...
void EDBRHistoMaker::Loop_SFs_mc(std::string outFileName,double luminosity,int isBarrel,TString isChannel,int n){
	if (fChain == 0)
		return;
	int numbe_out = 0;
...
//	nentries=100;
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if(filename.Contains("DY")||filename.Contains("TTJets")||filename.Contains("TTWJets")||filename.Contains("WJets")){
                     ...
                     if(lepton1_isprompt==1 && lepton2_isprompt==1) lepton_flag=1;
		}
                else if(filename.Contains("plj")||filename.Contains("fake")){
                       photon_flag=1;
		       lepton_flag=1;
		}
		else{
		...
		if( lepton_channel && (n_loose_ele + n_loose_mu) == 2 /*&& lepton_flag*/ && n_photon>0  && photonet > 20. && photon_channel /*&& photon_flag==1*/ && puppimet > 20 && n_bjets>=1 && mll >50 && ptll > 30 && mT2>30 && njets==n){
			if(gen_weight>0) npp++;
			if(gen_weight<0) nmm++;
			numbe_out++;
			treename->Fill();
