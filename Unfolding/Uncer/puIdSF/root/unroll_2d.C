...
int unroll_2d(){
	...
	for(int i=0;i<tag.size();i++){
		for(int ik=0;ik<2;ik++){
			for(int j=0;j<6;j++){
				file[j][i]=new TFile(Form("unfold_2dgenMjj_recobin%i_ewk_",j+1)+type[ik]+tag[i]+".root");
				fout[j][i]=new TFile(Form("unfold_genMjj_recobin%i_ewk_",j+1)+type[ik]+tag[i]+".root","recreate");
			...
			TFile*f1=new TFile("unfold_2dgenMjj_recobin7_ewk_"+type[ik]+tag[i]+".root");
			TFile*f2=new TFile("unfold_2dgenMjj_recobin8_ewk_"+type[ik]+tag[i]+".root");
			TFile*f3=new TFile("unfold_2dgenMjj_recobin9_ewk_"+type[ik]+tag[i]+".root");

			fout[6][i]=new TFile("unfold_genMjj_recobin7_ewk_"+type[ik]+tag[i]+".root","recreate");
			for(int k=0;k<num;k++){
