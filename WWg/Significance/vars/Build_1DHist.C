	...
	for(int ij=0;ij<njets.size();ij++){
		...
		for(int i=0;i<vars.size();i++){     
			if(vars[i].Contains("mllg"))
				vars_bins={30,120,160,200,300,400};
			else if(vars[i].Contains("ptllg"))
				vars_bins={0,40,55,65,80,105,400};
			else if(vars[i].Contains("mT"))
				vars_bins={50,80,100,120,140,160,200};
			for(int k=0;k<tags.size();k++){
				for(int j=0;j<names.size();j++){     
					if(names[j].Contains("Muon")==0&&names[j].Contains("Ele")==0)
						met="(n_bjets20_medium_deepFlavB_pc==0 && MET_T1Smear_pt > 20 && mT_pf>50  && "+jet_cut+")";
					else
						met="(n_bjets20_medium_deepFlavB_pc==0 && MET_T1_pt > 20 && mT_pf>50 && "+jet_cut+")";
					Reco= LEP+"&&"+photon+"&&"+met;
					//Reco= met;
