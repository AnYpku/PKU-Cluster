...
// set mcsig information
	...
	for (int i = 0; i < nDATA; i++) {
		...
		sprintf(out_buffer, "./output-slimmed-rootfiles/optimal_2018SR_%s.root", dataLabels[i].c_str());
...
//					hisRatio, out_buffer);
	...
	for (int i = 0; i < nMC; i++) {
		...
		sprintf(out_buffer, "./output-slimmed-rootfiles/optimal_2018SR_%s.root", mcLabels[i].c_str());
	...
	for (int  i = 0; i < nMCSig; i++) {
		...
		sprintf(out_buffer, "./output-slimmed-rootfiles/optimal_2018SR_%s.root", mcLabelsSig[i].c_str());
