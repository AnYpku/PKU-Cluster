...
def merge_year(fin,th1):
    fname=fin.GetName().split('.root')
    fname=fname[0]
    fout=TFile.Open(fname[0]+'pre.root')
    if 'WWG_emu' in fname[0]:
        th2=fout.Get('hist_sig')
    else:
        th2=fout.Get('hist_bkg')
    th1.Add(th2)
    return th1
    
print '-----begin to transfer TH2D to txt for Higgs-combine tool----- \n'
#sys.argv[1] year; sys.argv[2] njets, sys.argv[3] var
fdir = '/home/pku/anying/cms/PKU-Cluster/WWg/Significance/vars/CR/root/'
if 'mT' in sys.argv[3]:
    name='hist_1D_'
else:
    name='hist_'
f_Data=TFile.Open(fdir+name+'MuonEG_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_Data1=TFile.Open(fdir+name+'Muon_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_Data2=TFile.Open(fdir+name+'Ele_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_WWg = TFile.Open(fdir+name+'WWG_emu_tot_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_ZA = TFile.Open(fdir+name+'ZGJets_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_plj = TFile.Open(fdir+name+'plj_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_fakeL = TFile.Open(fdir+name+'fakeL_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_TTA = TFile.Open(fdir+name+'TTGJets_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_TT = TFile.Open(fdir+name+'TTJets_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_WA = TFile.Open(fdir+name+'WGJets_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_VV = TFile.Open(fdir+name+'VV_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_ST = TFile.Open(fdir+name+'ST_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_tZq = TFile.Open(fdir+name+'tZq_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')

th1_Data16=f_Data.Get('hist_bkg')
th2_Data16=f_Data1.Get('hist_bkg')
th3_Data16=f_Data2.Get('hist_bkg')
th1_WWg16=f_WWg.Get('hist_sig')
th1_ZA16=f_ZA.Get('hist_bkg')
th1_non_prompt16=f_plj.Get('hist_bkg')
th1_fakeL16=f_fakeL.Get('hist_bkg')
th1_TTA16=f_TTA.Get('hist_bkg')
th1_TT16=f_TTA.Get('hist_bkg')
th1_WA16=f_WA.Get('hist_bkg')
th1_VV16=f_VV.Get('hist_bkg')
th1_ST16=f_ST.Get('hist_bkg')
th1_tZq16=f_tZq.Get('hist_bkg')

th1_Data=merge_year(fData,th1_Data16)
th2_Data=merge_year(fData,th2_Data16)
th3_Data=merge_year(fData,th3_Data16)
th1_WWg =merge_year(f_WWg,th1_WWg16)
th1_ZA =merge_year(f_ZA,th1_ZA16)
th1_non_prompt =merge_year(f_plj,th1_non_prompt16)
th1_fakeL =merge_year(f_fakeL,th1_fakeL16)
th1_TTA =merge_year(f_TTA,th1_TTA16)
th1_TT =merge_year(f_TT,th1_TT16)
th1_WA =merge_year(f_WA,th1_WA16)
th1_VV =merge_year(f_VV,th1_VV16)
th1_ST =merge_year(f_ST,th1_ST16)
th1_tZq =merge_year(f_tZq,th1_tZq16)
...
genbinerror=[]
arr={}
f=open('/home/pku/anying/cms/PKU-Cluster/WWg/Significance/vars/Uncer/Top_CR/summary_uncer_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
# bin error
   ...
   f.write('WWG_Stat_CR_bin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('%0.3f\t-\t-\t-\t-\t-\n'%(WWg_binerror))
   f.write('VA_Stat_CR_bin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t%0.3f\t-\t-\t-\t-\n'%(ZA_binerror))

   f.write('Nonprompt_photon_Stat_CR_bin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t-\t%0.3f\t-\t-\t-\n'%(non_prompt_binerror))

   f.write('Nonprompt_lepton_Stat_CR_bin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t-\t-\t%0.3f\t-\t-\t-\n'%(fakeL_binerror))

   f.write('VV_Stat_CR_bin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t-\t-\t-\t%0.3f\t-\n'%(VV_binerror))

   f.write('Top_Stat_CR_bin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t-\t-\t-\t-\t%0.3f\n'%(ST_binerror))
