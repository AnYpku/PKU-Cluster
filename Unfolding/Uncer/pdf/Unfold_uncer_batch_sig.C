...
void run(TFile*file, TString vec_branchname,TString reco,vector<double> bins,TString cut1,TString tag,int kk){
     ...
     TFile*fout[kk];
     for(int j=0;j<kk;j++){
	     if(vec_branchname.Contains("Mjj")==0) fout[j]= new TFile("unfold_"+vec_branchname+Form("_recobin%d",j+1)+"_ewk_pdf"+tag+".root","recreate");
	     else fout[j]= new TFile("unfold_2d"+vec_branchname+Form("_recobin%d",j+1)+"_ewk_pdf"+tag+".root","recreate");
     }
     for(int j=0;j<kk;j++){
	     fout[j]->cd();
	     for(Int_t i=0;i<num;i++){
