     ...
     for(int j=0;j<kk;j++){
	     if(name.Contains("EWK")==0){
		     if(name.Contains("interf")==0)	     
			     ...
			     fout[j]= new TFile("unfold_"+vec_branchname[j]+"_interf_pdf"+tag+".root","recreate");
	     }
	     else
...
//		file1[i]=new TFile(dir[i]+"unfold_GenCutla-ZA"+tag[i]+".root");
		file1[i]=new TFile(dir[i]+"unfold_GenCutla-ZA_interf"+tag[i]+".root");
		file2[i]=new TFile(dir[i]+"cutla-outZA"+tag[i]+".root");
		run(file1[i],recovars, bins,cut1,tag[i]);
//		if(tag[i].Contains("16"))run(file1[i],recovars, bins,cut1,tag[i]);
	}
	return 1;
}
