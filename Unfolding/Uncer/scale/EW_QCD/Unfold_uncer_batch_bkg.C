...
//     tree->SetBranchStatus("*",0);
     ...
     for(int j=0;j<kk;j++){
	     if(name.Contains("EWK"))
		     ...
		     fout[j]= new TFile("./unfold_"+vec_branchname[j]+"_interfOut_scale"+tag+".root","recreate");
	     else
...
//		if(tag[i].Contains("17")==0) continue;
		...
		f3[i]=new TFile(dir[i]+"unfold_GenCutla-ZA_interf"+tag[i]+".root");
		run(f3[i], recovars, bins,cut2,tag[i],3);
