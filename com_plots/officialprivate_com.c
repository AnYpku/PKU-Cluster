...
//  en=10000;
  ...
  TString cut="(("+LEPmu+")||("+LEPele+"))&&("+photon+")&&"+jet+"&&"+SignalRegion;
...
//      h2[i]->DrawNormalized("HIST same");
      h2[i]->Draw("HIST same");
      l1[i]->AddEntry(h2[i],"Without");
      l1[i]->AddEntry(h1[i],"PU JetID");
//      l1[i]->Draw();
      ...
      h3[i]->SetMarkerColor(kBlack);
      h3[i]->Draw("PE");
      h3[i]->SetTitle("");
      h3[i]->GetXaxis()->SetTitle( title[i] +" [GeV] ");
      h3[i]->GetYaxis()->SetTitle("ratio");
...
//      open("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2017/step1_add_weight/","outZA-EWK_sherpa17",h1);
//      open("/eos/user/y/yian/2017cutla/","outZA-EWK",h2);
      open("/eos/user/y/yian/2017cutla/","outZA-EWK_noPU",h2);
      open("/eos/user/y/yian/2017cutla/","outZA-EWK_PU",h1);
      
      TH1D *h3[14];
      for(Int_t i=0;i<14;i++){
      double s1=1/h1[i]->Integral();
//      h1[i]->Scale(s1);
      double s2=1/h2[i]->Integral();
//      h2[i]->Scale(s2);
