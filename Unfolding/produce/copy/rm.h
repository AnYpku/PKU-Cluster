...
// Tue Oct 13 11:29:26 2020 by ROOT version 6.12/07
// from TTree ZPKUCandidates/ZPKU Candidates
// found on file: ZA17.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   TString m_dataset,tag;
   rm(TTree *tree=0, TString dataset="",TString year="");
   virtual ~rm();
...
#ifdef rm_cxx
rm::rm(TTree *tree,TString dataset,TString year) : fChain(0) 
{
	// if parameter tree is not specified (or zero), connect the file
	// used to generate this class and read the Tree.
	if (tree == 0) {
		TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ZA17.root");
		if (!f || !f->IsOpen()) {
			f = new TFile("ZA17.root");
		}
		TDirectory * dir = (TDirectory*)f->Get("ZA17.root:/treeDumper");
