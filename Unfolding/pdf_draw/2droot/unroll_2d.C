...
//	vector<TString> sample={"ewk","qcd"};
//	vector<TString> sample={"ewk"};
	vector<TString> sample={"interf"};
	for(int i=0;i<tag.size();i++){
		for(int j=0;j<sample.size();j++){
			cout<<tag[i]<<" "<<sample[j]<<endl;
			TFile*file=new TFile("./unfold_genMjj_"+sample[j]+tag[i]+".root");
			TFile*fout;
			fout=new TFile("../unfold_genMjj_"+sample[j]+tag[i]+".root","recreate");
			for(int k=0;k<num;k++){
