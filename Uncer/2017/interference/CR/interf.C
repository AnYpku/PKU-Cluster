...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   Bool_t JET=false;
   th2 = new TH2D(name,"Mjj vs deltaeta",1, mjj_bins, 1, deltaeta_bins);
   th2->Sumw2();
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	   ...
	   JET = jet1pt> 30 && jet2pt > 30 && fabs(jet1eta)< 4.7 && fabs(jet2eta)<4.7 && Mjj > 150 && drla > 0.7 && drla2 > 0.7 && drj1a > 0.5 && drj2a > 0.5 && drjj> 0.5 && drj1l > 0.5 && drj2l > 0.5 && drj1l2 > 0.5 && drj2l2 > 0.5 &&Mjj<500 && Mva>100 ;
		   ...
		   continue;
           if(deltaetajj > 6.5) deltaetajj = 6.1;
	   ...
	   if(JET && Mjj>150&& Mjj<500 && deltaetajj<6.5){ 
		   th2->Fill(Mjj, deltaetajj, actualWeight);
	   }
                
   }
   TString hname=name+"_hist";
   TH1D*h1=unroll(th2, mjj_bins, deltaeta_bins, 1,1,hname);
   file->cd();
   h1->Write();
   th2->Write();
   file->Close();
