...
void run(TString dir, TString sample,TString cut1[num],int kk,TString tag,bool turn){
     ...
     Double_t scalef,pileupWeight,prefWeight,muon1_id_scale,muon2_id_scale,muon1_iso_scale,muon2_iso_scale,ele1_id_scale,ele2_id_scale,ele1_reco_scale,ele2_reco_scale,photon_id_scale,photon_veto_scale,ele_hlt_scale,muon_hlt_scale,puIdweight_M_new,puIdweight_M_JER_up,puIdweight_M_JER_down;
     int lep;
     ...
     for(int k=0;k<tree->GetEntries();k++){
	     ...
	     if (  tformula2->EvalInstance() ){
		     th1[1]->Fill(Mva,actualWeight*puIdweight_M_JER_up);
	     }
	     if (  tformula3->EvalInstance() ){
		     th1[2]->Fill(Mva,actualWeight*puIdweight_M_JER_down);
	     }
