...
TFile* f1 = TFile::Open(dir1 + "hist_copy_outZA.root");
TFile* f2 = TFile::Open(dir1 + "hist_copy_outZJets_FX.root");
//TFile* ftrue = TFile::Open(dir1 + "ZA.root");

	TH1F* hdata1_ = (TH1F*)f1->Get(Form("h11_pt%0.f_%0.f",lowpt,highpt));
	TH1F* hdata2_ = (TH1F*)f2->Get(Form("h11_pt%0.f_%0.f",lowpt,highpt));
	TH1F* hfake_ = (TH1F*)f2->Get(Form("h12_pt%0.f_%0.f_chiso%0.f_%0.f",lowpt,highpt,lowchiso,highchiso));
	TH1F* htrue_ = (TH1F*)f1->Get(Form("h13_pt%0.f_%0.f",lowpt,highpt));
	TH1F* hist_ = (TH1F*) f2->Get(Form("hfake_pt%0.f_%0.f",lowpt,highpt));
	TH1F* hist1_ = (TH1F*)f1->Get(Form("htrue_pt%0.f_%0.f",lowpt,highpt));
	TH1D*hdata_=(TH1D*)hdata1_->Clone();
	hdata_->Add(hdata2_);
	Int_t nBins = 8;
	Int_t sieie_bin=4;
	Double_t bins[9];
	for (Int_t i=0;i<9;i++){
		bins[i] = 0.00015 + 0.0025*i;
	}
	...
	RooRealVar sieie("sieie", "sieie", 0.00015, 0.02015);
	...
	strChi += ".";
	strChi += (chiInt % 100);
	TLatex *textChi2 = new TLatex(0.7, 0.5, strChi);
	textChi2->SetNDC();
	...
	sieie.setRange("window",0.00015,0.01015);
	...
	TString strFR = "FR = (";
        ...
        strFR += (FRErrInt%10);
        strFR += ")%";
        TLatex* textFR = new TLatex(0.7,0.4,strFR);
        textFR->SetNDC();
    ...
    char buffer[256];
	sprintf(buffer, "pt%0.f-%0.f_chiso%0.f-%0.f.pdf",lowpt,highpt,lowchiso,highchiso);
