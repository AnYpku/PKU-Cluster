...
//  hs->GetYaxis()->CenterTitle();
	...
	latex.SetTextFont(42);
        ...
        latex.DrawLatex(poi_X,maximumMC*0.9,"#font[132]{e}#mu#gamma_{#font[132]{barrel}}");
	if (isDataPresent_) {
	...
	TLegend* leg1 = new TLegend(0.58, 0.5, 0.94, 0.9);
	TLegend* leg2 = new TLegend(0.2, 0.5, 0.57, 0.9);
...
//						TString LabelSig = samplesMCSig;
	...
	cout<<"test"<<endl;
	for (size_t i = 0; i != histosMC.size(); ++i) {
		mcTotalLabels.push_back("plj");mcTotalLabels.push_back("ZZ");mcTotalLabels.push_back("WZ");
		mcTotalLabels.push_back("WW");mcTotalLabels.push_back("ZGJets");mcTotalLabels.push_back("TGJets");
...
//		TString LabelMC = samples ;
	...
	//-- resize y axis --
	hs->SetMaximum(35 * maximumForStack);
	//
	cv->SetLogy(true);
	cv->Update();
	cv->Modified(true);
