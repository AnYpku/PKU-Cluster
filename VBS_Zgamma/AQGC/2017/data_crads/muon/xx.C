...
void xx::Loop(TString name)
{
  ...
  double ZGbin[6] = {150,400,600,800,1000,2e4};
   TH1D*h1=new TH1D(name,"Mva",5,ZGbin);
   TH1D*h2=new TH1D("h2","ZGamma",5,ZGbin);
   double lumi=41.52;
   TFile*file =new TFile("./full/"+name+"_hist.root","recreate");
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if( !(lep == 13 && (HLT_Mu2 >0 || HLT_Mu1 >0) && ptlep1 > 20. && ptlep2 > 20. && abs(etalep1) < 2.4 && abs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep > 70. && massVlep < 110. && ( ( (fabs(jet1eta)<3.14&&fabs(jet1eta)>2.65&&jet1pt>30&&jet1pt<50&&jet1puIdTight==1) || (!(fabs(jet1eta)<3.14&&fabs(jet1eta)>2.65) && fabs(jet1eta)<4.7 && jet1pt>30 && jet1pt<50)||(fabs(jet1eta)<4.7&& jet1pt>50) ) && ( (fabs(jet2eta)<3.14&&fabs(jet2eta)>2.65&&jet2pt>30&&jet2pt<50&&jet2puIdTight==1)||(!(fabs(jet2eta)<3.14&&fabs(jet2eta)>2.65)&&fabs(jet2eta)<4.7&&jet2pt>30&&jet2pt<50) ||(fabs(jet2eta)<4.7 && jet2pt>50) ) ) /*(jet1pt>30 && jet2pt>30 && fabs(jet1eta)< 4.7 && fabs(jet2eta)<4.7)*/ && Mjj>500. &&detajj>2.5 && photonet>100.&&(abs(photoneta)<1.4442||(abs(photoneta)>1.566&&abs(photoneta)<2.5)) ) )
            continue;
