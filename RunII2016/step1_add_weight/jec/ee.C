...
//		if(m_dataset=="outZA.root"){ scalef=1000.*47.46/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
		if(m_dataset.Contains("QCDZA")==1){ scalef=1000.*47.46/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
...
//                cout<<"scalef = "<<scalef<<endl;
      ...
      LEPmu = lep==13 && (HLT_Mu2>0||HLT_Mu5>0) && ptlep1 > 20. && ptlep2 > 20.&& fabs(etalep1) < 2.4 &&abs(etalep2) < 2.4 && nlooseeles==0 && nloosemus <3  && massVlep >70.&& massVlep<110 ;//&& ngoodmus>1 ;
