...
void run(TFile*file, TString vec_branchname,TString reco,vector<double> bins,TString cut1,TString tag,int kk){
     ...
     for(Int_t j=0;j<kk;j++){//Loop the binning cuts
	     for(Int_t i=0;i<num;i++){
		     th1name[i][j]=vec_branchname+Form("_pu%d_",i)+Form("recobin%d",j+1);
     ...
     for(int k=0;k<tree->GetEntries();k++){
	     ...
	     weight=scalef*photon_id_scale*photon_veto_scale*puIdweight*prefWeight;
	     if(lep==11)
     ...
     TFile*fout[kk];
     for(int j=0;j<kk;j++){
//	     if(vec_branchname.Contains("Mjj")==0) fout[j]= new TFile("unfold_"+vec_branchname+Form("_recobin%d",j+1)+"_qcd_pu"+tag+".root","recreate");
//	     else fout[j]= new TFile("unfold_2d"+vec_branchname+Form("_recobin%d",j+1)+"_qcd_pu"+tag+".root","recreate");
	     if(vec_branchname.Contains("Mjj")==0) fout[j]= new TFile("unfold_"+vec_branchname+Form("_recobin%d",j+1)+"_interf_pu"+tag+".root","recreate");
	     else fout[j]= new TFile("unfold_2d"+vec_branchname+Form("_recobin%d",j+1)+"_interf_pu"+tag+".root","recreate");
     }
     for(int j=0;j<kk;j++){
	     fout[j]->cd();
	     for(Int_t i=0;i<num;i++){
