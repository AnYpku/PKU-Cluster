...
input_filename_e = '/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/combine/muon/bins/paramsets_'+sys.argv[1]+'.txt'
input_filename_m = '/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/combine/ele/bins/paramsets_'+sys.argv[1]+'.txt'
...
for l_num, l in enumerate(input_file_m):
	...
	hists[l_num].Write()#the first six histos are muon channel ZGmass bins
...
for l_num, l in enumerate(input_file_e):
	...
	a1_e = float(ll[1])
	for bin_x in range(1,nGridPointsForNewF+1):
	        par1_value=hists[l_num+7].GetXaxis().GetBinCenter(bin_x)
        	yield_bin=1+a1_e*par1_value+a2_e*par1_value*par1_value
	        hists[l_num+7].SetBinContent(bin_x,yield_bin)
	hists[l_num+7].Write()#the last six histos are electron channel ZGmass bins
