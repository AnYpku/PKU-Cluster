...
//   TH1D* hist1[5]; 
   ...
   double uf[3]={1,2,0.5};
   for(int i=0;i<3;i++){
         	  hist1[i] = new TH1D(Form("hist1_%i",i),Form("\t\t %0.f<leading p_{T}<%0.f  reco  #mu_{f}=%2.1f;leding p_{T}^{reco};yields",ptBins[0],ptBins[3],uf[i]),3,ptBins);
		  hist2[i] = new TH1D(Form("hist2_%i",i),Form("\t\t %0.f<leading p_{T}<%0.f  reco && gen  #mu_{f}=%2.1f ;leding p_{T}^{gen};yields",ptBins[0],ptBins[3],uf[i]),3,ptBins);
		  hist3[i] = new TH1D(Form("hist3_%i",i),Form("\t\t %0.f<leading p_{T}<%0.f reco && gen  #mu_{f}=%2.1f ;leding p_{T}^{reco};yields",ptBins[0],ptBins[3],uf[i]),3,ptBins);
		  hist4[i] = new TH1D(Form("hist4_%i",i),Form("\t\t %0.f<leading p_{T}<%0.f  gen #mu_{f}=%2.2f;leding p_{T}^{gen};yields",ptBins[0],ptBins[3],uf[i]),3,ptBins);
   }
   for(int i=0;i<3;i++)
        {
...
//      Gen =  GenLEPele  && GenPhoton && GenJet && GenDr && GenSignalRegion;
      ...
      if(name.Contains("EWK") ){ 
              if(Reco){
                      for(int i=0;i<3;i++){
                              int k = 15*i;
			      hist1[i]->Fill(Mjj,scalef*lumi*pweight[k]);
		      }
		      i1++;
	      }
              if(Gen){
		      ...
		      for(int i=0;i<3;i++){
                              int k = 15*i;
			      hist4[i]->Fill(genMjj,scalef*lumi*pweight[k]);
		      }
	      }
              if(Reco && Gen){ 
		      i3++;
		      for(int i=0;i<3;i++){
                              int k = 15*i;
			      hist2[i]->Fill(genMjj,scalef*lumi*pweight[k]);
			      hist3[i]->Fill(genMjj,scalef*lumi*pweight[k]);
		      }
