...
void run(TString dir,TString name,TString cut,TString tag,TString vec_branchname,vector<Double_t> bins1,vector<Double_t> bins2,TString type,int num){
     TString fname=name+tag;
     TFile*file;
     file=new TFile(dir+fname+".root") ;
     cout<<tag<<" "<<name<<" "<<" "<<vec_branchname<<" "<<type<<endl;
     TTree*tree;
     ...
     TFile*fout=new TFile("./root/hist_"+name+"_"+vec_branchname+"_2jets_"+type+tag+".root","recreate");
     fout->cd();
     for(int i=0;i<num;i++){
	     hist[i]->Write();
	     h2[i]->Write();
...
int Build_Hist(){
	...
	vector<Double_t> mT_bins={50,95,130,200};
	vector<Double_t> ml1g_bins={10,80,140,200};
	vector<Double_t> ml2g_bins={10,50,90,200};
	vector<Double_t> mllg_bins={15,155,255,500};
	...
	for(int ik=0;ik<types.size();ik++){
		if(types[ik].Contains("fakephoton")){
			continue;
			dir1="../fakephoton/cutla-out";
			names={"plj_unc"};
		} 
		else if(types[ik].Contains("scale")||types[ik].Contains("pdf")){
			names={"TTGJets","ST","WWG_emu"};
			dir1="/home/pku/anying/cms/PKU-Cluster/WWg/CR_plot/SR_CR/output-slimmed-rootfiles/optimal_emua_";
		}
		else{
			names={"ZGJets","TTGJets","VV","ST","tZq","WGJets","WWG_emu"};
			dir1="/home/pku/anying/cms/PKU-Cluster/WWg/CR_plot/SR_CR/output-slimmed-rootfiles/optimal_emua_";
		}
		...
		for(int j=0;j<names.size();j++){     
			...
			if(names[j].Contains("Muon")==0 && names[j].Contains("plj")==0 && names[j].Contains("Ele")==0 )
				met="(n_bjets20_medium==0 && PuppiMET_T1Smear_pt > 20 && "+jet_cut+")";
			else
				met="(n_bjets20_medium==0 && PuppiMET_T1_pt > 20 && "+jet_cut+")";
			Reco= LEP+"&&"+photon+"&&"+met;
			for(int k=0;k<tags.size();k++){
				if(types[ik].Contains("l1pref") && tags[k].Contains("18"))
					continue;
				for(int i=2;i<vars.size();i++){
					run(dir1,names[j],Reco,tags[k],vars[i],mT_bins,bins2[i],types[ik],num);
				}
			}
		}
	}
