	...
	for(int i=0;i<tag.size();i++){
		...
		for(int j=0;j<kk;j++){
			...
			if(channels[j].Contains("Mu"))
                             hist[i][j]=run("DMuon17",tag[i],cut1,channels[j],vec_pt);
			else hist[i][j]=run("DEle17",tag[i],cut1,channels[j],vec_pt);
			hist_up[i][j]=run("plj17",tag[i],cut1,channels[j],vec_pt);
			hist_down[i][j]=run("ZA17_plj",tag[i],cut1,channels[j],vec_pt);
