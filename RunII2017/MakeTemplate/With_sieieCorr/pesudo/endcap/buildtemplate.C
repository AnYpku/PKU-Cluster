...
TString dir_file="./rootfiles/";
//TString dir_file="/eos/user/y/yian/2017cutla/";
...
double delta_R(Double_t eta1, Double_t phi1, Double_t eta2, Double_t phi2);
 ...
 Double_t scalef;
 Int_t nlooseeles,nloosemus,HLT_Mu1,HLT_Mu2,HLT_Mu3,ngoodmus;//,ngoodeles;
 Int_t HLT_Ele1,HLT_Ele2;
 Bool_t photon_pevnew[6],photon_pev[6],ewk=false,tta=false,za=false;
 ...
 bool LEPmu,LEPele,Leading_photon[6],medium_endcap[6],medium_barrel[6],loose_cut[6],JET[6],jet_flag;
 double ele1_id_scale,ele2_id_scale,ele1_reco_scale,ele2_reco_scale;
 double muon1_id_scale,muon2_id_scale,muon1_iso_scale,muon2_iso_scale,muon1_track_scale,muon2_track_scale;
...
//Double_t lowpt[ptnumber] ={20};
//Double_t highpt[ptnumber]={400};
 TH1D* h11[ptnumber];
 TH1D* h21[ptnumber];
...
// tree->SetBranchAddress("ngoodeles",&ngoodeles);
 ...
 tree->SetBranchAddress("HLT_Mu3",&HLT_Mu3);
...
// nentries=100000;
 ...
 for(Int_t i=0;i<nentries;i++){
    tree->GetEntry(i);
    LEPmu = lep==13 && (HLT_Mu1>0 || HLT_Mu2>0 || HLT_Mu3>0) && ptlep1 > 20. && ptlep2 > 20.&& abs(etalep1) < 2.4 &&abs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep >70. && massVlep < 110 ;
    ...
    for(Int_t j=0;j<6;j++){
         ...
         medium_endcap[j]= photon_pev[j]==1 /* photon_pevnew[j]==1*/ && photon_hoe[j]<0.0326
	 ...
	 medium_barrel[j]=  photon_pev[j]==1 &&
	...
	//Muon Endcap		 
	 ...
	 if(( LEPmu || LEPele) && Leading_photon[j]&& (medium_endcap[j]) /*&& (jet_flag==true)*/ ){
    ...
    newtree->Fill();
    za = false;
    if(fabs(scalef)<0.08 && fabs(scalef)>0.04) scalef=scalef*6077.22/6259.0;
    if(fabs(scalef)<0.009&&fabs(scalef)>0.002)  za=true;
    if(lep==13) scalef=scalef/(muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale*muon1_track_scale*muon2_track_scale);
    if(lep==11) scalef=scalef/(ele1_id_scale*ele2_id_scale*ele1_reco_scale*ele2_reco_scale);
...
//style();
histo();
TFile *f1 = new TFile(dir_file+rootfiles+".root");     
