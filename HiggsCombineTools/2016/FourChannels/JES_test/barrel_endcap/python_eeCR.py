...
f_in = TFile.Open("/afs/cern.ch/user/y/yian/work/PKU-Cluster/HiggsCombineTools/2016/FourChannels/2016eleendcap/CR/aa.root")
tstack = f_in.Get("hs")
...
jer_VV   =[1.27]
for i in range(1,2):
   f = open('%s_%s_control.txt'%('ele16', 'endcap'),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t-\t-\t-\t#0. uncertainty on ele16_endcap\n'%('ele16','endcap'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\t#0. uncertainty on ele16_endcap\n'%('ele16','endcap',fakephoton[i-1]))
   f.write('VBS_stat_control_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\t-\t-\t-\n'%('ele16','endcap',i,ZA_sig_binerror))
   f.write('QCD_stat_control_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\t-\t-\t-\n'%('ele16','endcap',i,ZA_binerror))
   f.write('non_prompt_stat_control_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\n'%('ele16','endcap',i,non_prompt_binerror))
   f.write('TTA_stat_control_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\n'%('ele16','endcap',i,TTA_binerror))
   f.write('VV_stat_control_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t%0.2f\t-\t-\n'%('ele16','endcap',i,VV_binerror))
   f.write('ST_stat_control_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t%0.2f\t-\n'%('ele16','endcap',i,ST_binerror))
   f.write('WA_stat_control_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t%0.2f\n'%('ele16','endcap',i,WA_binerror))
