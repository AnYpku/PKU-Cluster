#define num 3
#define pi 3.1415926
void run(TString dir, TString sample,TString vec_branchname,vector<double> bins,TString cut1[num],int kk,TString tag,bool turn){
		...
		for(int i=0;i<recovars.size();i++){
			for(int j=0;j<sample.size();j++){
				...
				if(sample[j].Contains("EWK"))
					run(dir,sample[j],recovars[i], bins[i],cut,3,tags[k],1);
				run(dir,sample[j],recovars[i], bins[i],Reco,3,tags[k],0);
			}
		}
	}
	return 1;
