void histo_gen(){
	...
	for(int i=0;i<3;i++){
       ...
       hist2[0]->GetYaxis()->SetRangeUser(0,hist2[0]->GetMaximum()*1.3);
       ...
       l1->AddEntry(hist3[0],"M_{jj}^{reco}");
       for(int i=0;i<3;i++){
...
//          hist3[i]->Draw("same hist");
          l1->AddEntry(hist2[i],Form("M_{jj}^{gen} #mu_{f}=%1.2f",uf[i]));
       }
      l1->Draw();  
      fPads1->Update();
      fPads2->cd();
      ...
      nominal->GetXaxis()->SetTitle("M_{jj}");
