...
void cmsLumi(bool channel) 
{
        ...
        latex.DrawLatex(0.76,0.92,Form("41.522 fb^{-1} (%d TeV)", (beamcomenergytev)));
}
...
TH1D* unroll(TH2D* th2_in,Double_t* xbin, Double_t* ybin,  Int_t xbins_in, Int_t ybins_in)
{
	...
	TH1D* h1= new TH1D("merged", "", nbins, 0, nbins);
...
//        TH1D* t_ZA_sig=unroll(th2_ZA_sig, mjj_bins, mlla_bins, 4,4);
        TH1D* t_ZA_sig=unroll(th2_ZA_sig, mjj_bins, detajj_bins, 1,1);
...
//        TH1D* t_WA=unroll(th2_WA, mjj_bins, mlla_bins, 4,4);
        TH1D* t_WA=unroll(th2_WA, mjj_bins, detajj_bins, 1,1);
...
//        TH1D* t_VV=unroll(th2_VV, mjj_bins, mlla_bins, 4,4);
        TH1D* t_VV=unroll(th2_VV, mjj_bins, detajj_bins, 1,1);
...
//        TH1D* t_TTA=unroll(th2_TTA, mjj_bins, mlla_bins, 4,4);
        TH1D* t_TTA=unroll(th2_TTA, mjj_bins, detajj_bins, 1,1);
...
//        TH1D* t_ST=unroll(th2_ST, mjj_bins, mlla_bins, 4,4);
        TH1D* t_ST=unroll(th2_ST, mjj_bins, detajj_bins, 1,1);
	...
	TFile* f_plj=TFile::Open("./2d_plj.root");
        ...
        TH1D* t_plj=unroll(th2_plj, mjj_bins, detajj_bins, 1,1);
	...
	hs->GetYaxis()->SetTitle("Events /bin");
        ...
        TLegend* leg = new TLegend(0.65, 0.45, 0.85, 0.9);
        leg->SetTextSize(0.04);
        leg->SetMargin(0.4);
        leg->AddEntry(t_ZA_sig,Form("EWK [%.2f]",t_ZA_sig->GetSum()) );
