...
//   mjj_JEC_up = new TH1D("h_up","mjj_JEC_up",9,0,9);
   ...
   for(int i=0;i<3;i++){
	   th2[i] = new TH1D(Form("th2_%d",i),"Mjj",3, mjj_bins);
	   th2[i]->Sumw2();
   }
...
//	   cout<<"scalef = "<<scalef<<"; pileupWeight = "<<pileupWeight<<"; actualWeight = "<<actualWeight<<endl;
	   for(int i=0;i<3;i++){
		   if(JET_JEC[i] && deltaeta_JEC[i]>2.5){ 
			   th2[i]->Fill(Mjj_JEC[i],actualWeight);
		   }
	   }
	   if(JET_down==1) {
