...
// Setup names of data files for trees.
	...
	for (int j = 0; j< pathToTrees.size(); j++) {
		for (int ii = 0; ii < nDATA; ii++) {
			...
			fData.push_back(pathToTrees[j] +"cutlep-out"+ data[ii] + year+"_plj.root");
		}
	}
	const int kk=fData.size();
	std::string dataLabels[kk] = { "MuonEG_plj"};
	cout<<"Data size "<<fData.size()<<endl;
	// set mc imformation
	const int nMC = 8;//
	std::cout << "set data imformation, we have " << nMC << "mc file"
		<< std::endl;
//        std::string mc[nMC] ={"plj_Jets","plj","plj","WW","VV","ST","DYJets","TTJets","tZq","ZGJets","TTGJets","TGJets","WGJets","WJets"};
        std::string mc[nMC] ={"WW","VV","ST","tZq","ZGJets","TTGJets","TGJets","WGJets"};
	std::vector< TString > fMC;
	for (int ii = 0; ii < nMC; ii++) {
		fMC.push_back(pathToTrees[0] +"cutlep-out"+ mc[ii] + year+"_plj.root");
		cout<<pathToTrees[0]<<"cutlep-out"<<mc[ii] <<year<<"_plj.root"<<endl;
	}
	const int nmc=fMC.size();
	std::string bkgLabels[nmc] ={"WW","VV","tW","tZq","QCD Z#gamma","t#bar{t}#gamma","T#gamma","QCD W#gamma"};
	std::string mcLabels[nmc] = {"WW_plj","VV_plj","ST_plj","tZq_plj","ZGJets_plj","TTGJets_plj","TGJets_plj","WGJets_plj"};
	double kFactorsMC_array[nmc] ={lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1,lumiValue1};
