...
# bin error
   ...
   f.write('process\t')
   f.write('QCD\tEWK\tnon_prompt\tTTA\tVV\tST\tWA\tSig_out\n')
   f.write('process\t0\t1\t2\t3\t4\t5\t6\t7\n')
   f.write('rate\t')
   f.write('%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n'%(ZA_bincontent,ZA_sig_bincontent, non_prompt_bincontent, TTA_bincontent, VV_bincontent, ST_bincontent, WA_bincontent,ZA_sig_out_bincontent))
   f.write('------------\n')
...
#
   ...
   f.write('-\t-\t-\t-\t-\t-\t-\t%0.2f/%0.2f\n'%(arr['SigOut_scale_up'][i-1],arr['SigOut_scale_down'][i-1]))
#
   f.write('interf\tlnN\t')
