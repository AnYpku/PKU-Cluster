void run(TString tag,TString sample,TString type){
	...
	TFile* file = new TFile("./root/hist_"+sample+"_"+type+tag+".root");
        TString mc;
        if(sample.Contains("WWG"))mc="sig";else mc="bkg";
	cout<<type<<endl;
	TH1D* h1 = (TH1D*)file->Get(""+mc+"_0");
	TH1D* h2 = (TH1D*)file->Get(""+mc+"_1");
	TH1D* h3 = (TH1D*)file->Get(""+mc+"_2");
...
int uncer_jesr(){
	...
	vector<TString> sample={"ZGJets","TTGJets","VV","ST","tZq","TGJets","WGJets","WWG"};
