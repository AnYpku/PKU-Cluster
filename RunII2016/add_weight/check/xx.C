...
//                scalef=1;
		...
		// if (Cut(ientry) < 0) continue;
			...
			if(m_dataset.Contains("outZA_pweight.root")){ scalef=1000.*47.46/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
			...
			if(m_dataset.Contains("outWW.root")){ scalef=1000.*118.7/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
			if(m_dataset.Contains("outWZ.root")){ scalef=1000.*47.13/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
			if(m_dataset.Contains("outZZ.root")){ scalef=1000.*16.523/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
			if(m_dataset.Contains("outZA-EWK")){ scalef=1000.*0.1097/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
			if(m_dataset.Contains("aQGC")){ scalef=1000.*1.097/float(npp-nmm)*fabs(theWeight)/theWeight; run_period=8;}
		}
...
//			muon_hlt_scale=muon_HLT_scale(etalep1,etalep2,di_lep_trigger);
		...
		if(jentry%100000==0) cout<<" "<<HLT_Ele1<<" "<<HLT_Mu1<<" "<<fabs(theWeight)/theWeight<<" "<<m_dataset<<" "<<"scalef "<<scalef<<" "<<jentry<<" "<<fChain->GetEntries()<<endl;
                ...
                LEPmu = lep==13 && ptlep1 > 20. && ptlep2 > 20.&& fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && nlooseeles==0 && nloosemus <3  && massVlep >70. ;
                LEPele = lep==11 &&  ptlep1 > 20. && ptlep2 > 20.&& fabs(etalep1) < 2.5 && fabs(etalep2) < 2.5 && nlooseeles < 3 && nloosemus == 0  && massVlep >70.;
