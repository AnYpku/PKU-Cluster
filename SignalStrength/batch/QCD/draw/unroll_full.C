...
void unroll_run(TString channel){
	...
	TH1D* th2_ZA16=(TH1D*)f_ZA->Get("hist_sig");
	TH1D* th2_ZA17=(TH1D*)f_ZA17->Get("hist_sig");
	TH1D* th2_ZA18=(TH1D*)f_ZA18->Get("hist_sig");
	TH1D* th2_ZA16out=(TH1D*)f_ZA->Get("hist_sigout");
	TH1D* th2_ZA17out=(TH1D*)f_ZA17->Get("hist_sigout");
	TH1D* th2_ZA18out=(TH1D*)f_ZA18->Get("hist_sigout");
        th2_ZA16->Add(th2_ZA17,1);th2_ZA16out->Add(th2_ZA17out,1);
        th2_ZA16->Add(th2_ZA18,1);th2_ZA16out->Add(th2_ZA18out,1);
        TH1D* th2_ZA= unroll(th2_ZA16,"t_ZA");
        TH1D* th2_ZAout= unroll(th2_ZA16out,"t_ZA");
...
//	htot->Add(th2_ZA,1);
	...
	for(int i=0;i<htot->GetNbinsX();i++){
		htot->SetBinError(i+1,sqrt( th2_TTA->GetBinError(i+1)*th2_TTA->GetBinError(i+1)+th2_VV->GetBinError(i+1)*th2_VV->GetBinError(i+1)+th2_plj->GetBinError(i+1)*th2_plj->GetBinError(i+1)+th2_ZA_sig->GetBinError(i+1)*th2_ZA_sig->GetBinError(i+1)+th2_ZA_sigout->GetBinError(i+1)*th2_ZA_sigout->GetBinError(i+1) ));
	...
	leg1->AddEntry(th2_ZA_sig,Form("ZA [%.1f]",th2_ZA_sig->GetSum()),"f" );
	leg1->AddEntry(th2_ZA_sigout,Form("ZA_{out} [%.1f]",th2_ZA_sigout->GetSum()),"f" );
