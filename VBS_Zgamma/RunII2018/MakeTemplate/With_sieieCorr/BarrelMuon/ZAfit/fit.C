...
//	RooRealVar nTrue("true number", "true number",800,0, nData);
	...
	RooPlot* xframe = sieie.frame(Title(" "), Bins(nBins));
	data_hist.plotOn(xframe);
	fullpdf.plotOn(xframe, Name("sum"), LineColor(kRed),FillColor(kRed));
	fullpdf.plotOn(xframe, Components("ntrue"), Name("true"),LineColor(kGreen),FillColor(2), LineStyle(9));
	...
	leg->AddEntry(htrue, "True #gamma", "L");
	leg->AddEntry(hfake, "Fake #gamma", "L");
	leg->Draw("same");
	...
	strChi += ".";
	strChi += (chiInt % 100);
	TLatex *textChi2 = new TLatex(0.6, 0.4, strChi);
	textChi2->SetNDC();
	...
	TString ptR=Form("%0.f<p_{T}^{#gamma}<%0.f GeV ",lowpt, highpt);
        TLatex *textPtR = new TLatex(0.6, 0.5, ptR);
        textPtR->SetNDC();
        textPtR->SetTextSize(0.05);
        textPtR->SetLineWidth(1);
        textPtR->Draw();
	...
	TString strFR = "FR = (";
        ...
        strFR += (FRErrInt%10);
        strFR += ")%";
        TLatex* textFR = new TLatex(0.6,0.3,strFR);
        textFR->SetNDC();
