	...
	for(int i=0;i<tag.size();i++){
		...
		for(int j=0;j<kk;j++){
			...
			if(channels[j].Contains("Mu"))
                             hist[i][j]=run("DMuon16",tag[i],cut1,channels[j],vec_pt);
			else hist[i][j]=run("DEle16",tag[i],cut1,channels[j],vec_pt);
			hist_up[i][j]=run("plj16",tag[i],cut1,channels[j],vec_pt);
			hist_down[i][j]=run("ZA16_plj",tag[i],cut1,channels[j],vec_pt);
