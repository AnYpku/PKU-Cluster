...
//        ftxt<<index<<" "<<endl;
	TFile*file1=new TFile("./sig_root/unfold_"+var+"_"+index+"_ewk_pdf16.root");
	TFile*file2=new TFile("./sig_root/unfold_"+var+"_"+index+"_ewk_pdf17.root");
	TFile*file3=new TFile("./sig_root/unfold_"+var+"_"+index+"_ewk_pdf18.root");
        TH1D*h1[num];TH1D*h2[num];;TH1D*h3[num];
	for(int j=0;j<num;j++){
		h1[j]=(TH1D*)file1->Get(Form(var+"_pdf%i_recobin%i",j,i));
		h2[j]=(TH1D*)file2->Get(Form(var+"_pdf%i_recobin%i",j,i));
		h3[j]=(TH1D*)file3->Get(Form(var+"_pdf%i_recobin%i",j,i));
                h1[j]->Scale(35.86);
                h2[j]->Scale(41.52);
                h3[j]->Scale(58.7);
                h1[j]->Add(h2[j],1);
