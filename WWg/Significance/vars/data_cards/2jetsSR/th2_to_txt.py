...
fdir = '/home/pku/anying/cms/PKU-Cluster/WWg/Significance/vars/2jetsSR/root/'
...
genbinerror=[]
arr={}
f=open('/home/pku/anying/cms/PKU-Cluster/WWg/Significance/vars/Uncer/2jetsSR/summary_uncer_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
# bin error
   ...
   f.write('WWG_Stat_CRbin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('%0.3f\t-\t-\t-\t-\t-\n'%(WWg_binerror))
   f.write('VA_Stat_CRbin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t%0.3f\t-\t-\t-\t-\n'%(ZA_binerror))

   f.write('Nonprompt_photon_Stat_CRbin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t-\t%0.3f\t-\t-\t-\n'%(non_prompt_binerror))

   f.write('Nonprompt_lepton_Stat_CRbin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t-\t-\t%0.3f\t-\t-\t-\n'%(fakeL_binerror))

   f.write('VV_Stat_CRbin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t-\t-\t-\t%0.3f\t-\n'%(VV_binerror))

   f.write('Top_Stat_CRbin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t-\t-\t-\t-\t%0.3f\n'%(ST_binerror))
