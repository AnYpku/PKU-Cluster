...
void unroll_name(TString filename,TString tag,TString type){
	...
	for(Int_t i=0;i<num;i++){
		if(filename.Contains("WWG")==0)
			t_ZA[i]=(TH1D*)f_ZA->Get(Form("bkg_%d",i));
		else
			t_ZA[i]=(TH1D*)f_ZA->Get(Form("sig_%d",i));
		t_ZA[i]->SetTitle("");
	}
...
//        latex.DrawLatex(0.5,1.3*max,"50<m_{#font[12]{ll}}<100");
//        latex.DrawLatex(4.5,1.3*max,"100<m_{#font[12]{ll}}<150");
//        latex.DrawLatex(8.5,1.3*max,"m_{#font[12]{ll}}>150");
