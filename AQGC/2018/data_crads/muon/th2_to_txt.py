...
jer_EWK=[1.006,1.01,1.02,1.02,1.03]
jer_QCD=[1.20,1.28,1.09,1.36,3]
jes_EWK=[1.04,1.04,1.06,1.08,1.03]
jes_QCD=[1.39,1.36,1.32,1.27,4]
fake=[1.22,1.23,0,1.19,1.23]
pdf_QCD=[1.01,1.01,1.02,1.02,1.03]
pdf_ewk=[1.01,1.01,1.02,1.02,1.03]
scale_QCD=[1.16,1.21,1.15,1.164,1.39]
scale_ewk=[1.07,1.09,1.10,1.12,1.14]
QCDZA_scale_extra_down = [0.962,0.900,0.975,0.951,1.279];
QCDZA_scale_extra_up = [1.04,1.12,1.064,1.106,0.638];
for i in range(1,6):
   f = open('./txt/%s_bin_%d.txt'%('muon18', i),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t#0. uncertainty on muon_18\n'%('muon','18'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t%0.2f\t-\t-\t#0. uncertainty on muon_18\n'%('muon','18',fake[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\n'%('muon','18',i,ZA_sig_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\n'%('muon','18',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\n'%('muon','18',i,non_prompt_binerror))
   f.write('others_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\n'%('muon','18',i,others_binerror))
   f.write('JES_%s\tlnN\t%0.2f\t%0.2f\t-\t%0.2f\n'%('18',jes_EWK[i-1],jes_QCD[i-1],jes_EWK[i-1]))
   f.write('JER_%s\tlnN\t%0.2f\t%0.2f\t-\t%0.2f\n'%('18',jer_EWK[i-1],jer_QCD[i-1],jer_EWK[i-1]))
