...
fakephoton=[1.11,1.17,1.22,1.17,1.13,1.15,1.13]
l1pref    =[1.02,1.02,1.02,1.00,1.00,1.00,1.00]
...
for i in range(1,8):
   f = open('%s_%s_bin_%d.txt'%('17', 'mubarrel', i),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t-\t-\t-\t-\t#0. uncertainty on 17_mubarrel\n'%('17','mubarrel'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\t-\t#0. uncertainty on 17_mubarrel\n'%('17','mubarrel',fakephoton[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\t-\t-\t-\t-\n'%('17','mubarrel',i,ZA_sig_binerror))
   f.write('VBSout_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\t-\t-\t-\t-\n'%('17','mubarrel',i,ZA_sigout_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\t-\n'%('17','mubarrel',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\t-\n'%('17','mubarrel',i,non_prompt_binerror))
   f.write('TTA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t%0.2f\t-\t-\t-\n'%('17','mubarrel',i,TTA_binerror))
   f.write('VV_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t%0.2f\t-\t-\n'%('17','mubarrel',i,VV_binerror))
   f.write('ST_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t%0.2f\t-\n'%('17','mubarrel',i,ST_binerror))
   f.write('WA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t-\t%0.2f\n'%('17','mubarrel',i,WA_binerror))
