...
fakephoton=[1.11,1.13,1.00,1.09,1.07,1.12,1.07]
l1pref    =[1.03,1.03,1.06,1.01,1.02,1.05,1.03]
...
jes_QCD  =[1.24,1.12,1.34,1.35,1.74,1.69,1.58]
jes      =[1.02,1.04,1.07,1.22,1.11,1.10,1.11]
jer      =[1.02,1.01,1.03,1.36,1.12,1.04,1.10]
jer_QCD  =[1.14,1.04,1.22,1.32,1.35,1.35,1.60]
...
for i in range(1,8):
   f = open('%s_%s_bin_%d.txt'%('17', 'elebarrel', i),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t-\t-\t-\t-\t#0. uncertainty on 17_elebarrel\n'%('17','elebarrel'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\t-\t#0. uncertainty on 17_elebarrel\n'%('17','elebarrel',fakephoton[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\t-\t-\t-\t-\n'%('17','elebarrel',i,ZA_sig_binerror))
   f.write('VBSout_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\t-\t-\t-\t-\n'%('17','elebarrel',i,ZA_sigout_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\t-\t-\t-\t-\n'%('17','elebarrel',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\t-\t-\t-\t-\n'%('17','elebarrel',i,non_prompt_binerror))
   f.write('TTA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t%0.2f\t-\t-\t-\n'%('17','elebarrel',i,TTA_binerror))
   f.write('VV_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t%0.2f\t-\t-\n'%('17','elebarrel',i,VV_binerror))
   f.write('ST_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t%0.2f\t-\n'%('17','elebarrel',i,ST_binerror))
   f.write('WA_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t-\t-\t-\t-\t%0.2f\n'%('17','elebarrel',i,WA_binerror))
