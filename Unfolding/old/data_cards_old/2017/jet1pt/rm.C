...
//   TH1D* hist1[5]; 
   TH1D* hist5[4];
   TH1D* hist2= new TH1D("hist",name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f  reco && gen;leding p_{T}^{reco};yields",ptBins[0],ptBins[4]),4,ptBins);
   TH1D* hist1= new TH1D("hist1",name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f  reco;leding p_{T}^{reco};yields",ptBins[0],ptBins[4]),4,ptBins);
   TH1D* hist4= new TH1D("hist4",name+Form("\t\t %0.f<leading p_{T}^{gen}<%0.f  gen;leding p_{T}^{gen};yields",ptBins[0],ptBins[4]),4,ptBins);
   TH1D* hist3= new TH1D("hist3",name+Form("\t\t %0.f<leading p_{T}^{reco}<%0.f reco out of gen;leding p_{T}^{gen};yields",ptBins[0],ptBins[4]),4,ptBins);
   for(int i=0;i<4;i++)
        {
...
//      if(genlep1pt>400) genlep1pt=399;if(ptlep1>400) ptlep1=399;
      if(genjet1pt>800) genjet1pt=799;if(jet1pt>800) jet1pt=799;
      if(name.Contains("EWK") ){ 
              if(Reco){
		      hist1->Fill(jet1pt,scalef*lumi);
		      i1++;
	      }
              if(Gen){
		      i2++;
                      h2->Fill(jet1pt,genjet1pt,scalef*lumi);
		      hist4->Fill(genjet1pt,scalef*lumi);
	      }
              if(Reco && Gen){ 
		      i3++;
		      hist2->Fill(jet1pt,scalef*lumi);
                      h3->Fill(jet1pt,genjet1pt,scalef*lumi);
	      }
              if(Reco&& !Gen) {
                      i4++;
		      hist3->Fill(jet1pt,scalef*lumi);
	      }
      }//signal out gen bincontent
      if(name.Contains("EWK")==0 && !(name.Contains("Muon")==1 || name.Contains("Data")==1) && Reco){
	      i1++;
	      if(name.Contains("plj")){
		      hist2->Fill(jet1pt,scalef);
	      }
	      else{ 
		      hist2->Fill(jet1pt,scalef*lumi);}
      }//bkg bincontent
      if( (name.Contains("Muon")==1 || name.Contains("Data")==1) && Reco){
	      hist2->Fill(jet1pt);}//data bincontent
      for(int kk=0;kk<4;kk++){
	      if(name.Contains("EWK") && Reco && Gen ){ 
		      if(jet1pt>ptBins[kk] && jet1pt<ptBins[kk+1]){
			      hist5[kk]->Fill(genjet1pt,scalef*lumi);
		      }
