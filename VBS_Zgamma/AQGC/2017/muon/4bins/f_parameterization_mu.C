...
using namespace std;
void fX0_parameterization_mu(int index,vector<double>ZGbin){

	const TString InData_New = "/eos/user/y/yian/2017cutla/cutla-";

	// Specify event selection cuts:
	TCut the_cut("(lep == 13 &&  ptlep1 > 20. && ptlep2 > 20. && abs(etalep1) < 2.4 && abs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus <3 && massVlep > 70. && massVlep < 110. && jet1pt> 30 && jet2pt > 30 && fabs(jet1eta)< 4.7 && fabs(jet2eta)<4.7 && Mjj>500. &&deltaetajj>2.5 && photonet>100.&&(abs(photoneta)<1.4442||(abs(photoneta)>1.566&&abs(photoneta)<2.5)))");
	...
	fout = new TFile("signal_proc_mu__"+name+".root", "RECREATE");
	// The input tree
	TFile *f_file;
...
//	const double xf[17] = {-20, -10, -6, -5, -3, -2, -1, -0.6, 0, 0.6, 1, 2, 3, 5, 6, 10, 20};
	...
	for(int abin=0;abin<ZGbin.size()-1;abin++){
		...
		for(int count=0; count < numberOfEntries; count++){
			...
			Double_t weight= lumiWeight * pileupWeight * scalef*muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale*muon1_track_scale*muon2_track_scale*photon_id_scale*photon_veto_scale*prefWeight;
...
void f_parameterization_mu(){
	...
	fX0_parameterization_mu(1,ZGbin);
	fX0_parameterization_mu(2,ZGbin);
	fX0_parameterization_mu(3,ZGbin);
	fX0_parameterization_mu(4,ZGbin);
	fX0_parameterization_mu(5,ZGbin);
	fX0_parameterization_mu(6,ZGbin);
	fX0_parameterization_mu(7,ZGbin);
	fX0_parameterization_mu(8,ZGbin);
	fX0_parameterization_mu(9,ZGbin);
	fX0_parameterization_mu(10,ZGbin);
	fX0_parameterization_mu(11,ZGbin);
	fX0_parameterization_mu(12,ZGbin);
	fX0_parameterization_mu(13,ZGbin);
	fX0_parameterization_mu(14,ZGbin);
	fX0_parameterization_mu(15,ZGbin);
	fX0_parameterization_mu(16,ZGbin);
}
