...
do
  ...
  combine -M Significance full${y}_test.txt > result${y}_test.txt 
  combine -M Significance full${y}_test.txt --freezeParameters all > result${y}_freezeAll.txt
done

combineCards.py emu_*.txt >& full.txt
text2workspace.py full.txt -m 125
combine -M Significance full.txt > result_full.txt
combine -M Significance full.txt --freezeParameters all > result_full_freezeAll.txt
