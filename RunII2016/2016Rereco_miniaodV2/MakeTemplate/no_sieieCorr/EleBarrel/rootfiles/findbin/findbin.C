...
//      if(jentry%2000==0) cout<<jentry<<";"<<nentries<<endl;
      LEP = lep==11 && (HLT_Ele1>0)  && ptlep1 > 25. && ptlep2 > 25.&& abs(etalep1) < 2.5 &&abs(etalep2) < 2.5 && nlooseeles < 3 && nloosemus == 0 && massVlep >70. && massVlep < 110;// && drla>0.7;
      for(Int_t j=0;j<6;j++){
    ...
    for(Int_t k=0;k<num;k++){
	    ...
	    if(photon_chiso[position]<12& photon_chiso[position]>5 && photon_pt[position]<highpt[k] && photon_pt[position]>lowpt[k]){
    ...
    f1->cd();
    file<<"pt range"<<"\t"<<"total"<<"\t"<<"half"<<"\t"<<"binnumber"<<"\t"<<"sum"<<"\t"<<"chiso"<<endl;
    for(Int_t k=0;k<num;k++){
	    h1[k]->Write();
	    for(Int_t i=0;i<h1[k]->GetNbinsX();i++){
		    ...
		    if(   abs(sum[k]- (h1[k]->GetSumOfWeights()/2) )<=1  )  
		    {
			    cout<<lowpt[k]<<"~"<<highpt[k]<<"; the total:"<<h1[k]->GetSumOfWeights()<<"; the half:"<<h1[k]->GetSumOfWeights()/2<<"; sum of the first "<<i+1<<"bin:"<<sum[k]<<"; the chiso value in "<<i+1<<" bin = "<<h1[k]->GetBinCenter(i+1)+0.5*h1[k]->GetBinWidth(i+1)<<endl;
                           file<<lowpt[k]<<"~"<<highpt[k]<<"\t\t"<<h1[k]->GetSumOfWeights()<<"\t"<<h1[k]->GetSumOfWeights()/2<<"\t"<<i+1<<"\t\t"<<sum[k]<<"\t"<<h1[k]->GetBinCenter(i+1)+0.5*h1[k]->GetBinWidth(i+1)<<endl;
//		    break;
		    }
	    }
    } 
