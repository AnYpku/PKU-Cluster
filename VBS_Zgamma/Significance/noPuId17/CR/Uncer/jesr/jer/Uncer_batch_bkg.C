...
void run(TString dir, TString sample,TString cut1[num],int kk,TString tag,bool turn){
     ...
     int lep;
     double muon1_id_scale,muon2_id_scale,muon1_iso_scale,muon2_iso_scale,actualWeight;
...
//	vector<TString> sample={"WA"};
	...
	for(int i=0;i<tags.size();i++){
		if(tags[i].Contains("17")){
			JET_new="( (  ( (!(fabs(jet1eta_new)<3.14&&fabs(jet1eta_new)>2.65)&&fabs(jet1eta_new)<4.7&&jet1pt_new<50&&jet1pt_new>30) || (jet1pt_new>50&&fabs(jet1eta_new)< 4.7) )   &&   ( (!(fabs(jet2eta_new)<3.14 && fabs(jet2eta_new)>2.65) && fabs(jet2eta_new)<4.7 && jet2pt_new<50 && jet2pt_new>30) || (jet2pt_new>50&&fabs(jet2eta_new)< 4.7) )   ) && Mjj_new > 150 && Mjj_new<400 && drla > 0.7 && drla2 > 0.7 && drj1a_new > 0.5 && drj2a_new > 0.5 && ("+drjj_new+") && drj1l_new > 0.5 && drj2l_new > 0.5 && drj1l2_new > 0.5 && drj2l2_new > 0.5 )";
			JET_up="( (  ( (!(fabs(jet1eta_JER_up)<3.14&&fabs(jet1eta_JER_up)>2.65)&&fabs(jet1eta_JER_up)<4.7&&jet1pt_JER_up<50&&jet1pt_JER_up>30) || (jet1pt_JER_up>50&&fabs(jet1eta_JER_up)< 4.7) )   &&   ( (!(fabs(jet2eta_JER_up)<3.14 && fabs(jet2eta_JER_up)>2.65) && fabs(jet2eta_JER_up)<4.7 && jet2pt_JER_up<50 && jet2pt_JER_up>30) || (jet2pt_JER_up>50&&fabs(jet2eta_JER_up)< 4.7) )   ) && Mjj_JER_up > 150 && Mjj_JER_up <400 && drla > 0.7 && drla2 > 0.7 && drj1a_JER_up > 0.5 && drj2a_JER_up > 0.5  && ("+drjj_JER_up+")&& drj1l_JER_up > 0.5 && drj2l_JER_up > 0.5 && drj1l2_JER_up > 0.5 && drj2l2_JER_up > 0.5 )";
			JET_down="( (  ( (!(fabs(jet1eta_JER_down)<3.14&&fabs(jet1eta_JER_down)>2.65)&&fabs(jet1eta_JER_down)<4.7&&jet1pt_JER_down<50&&jet1pt_JER_down>30) || (jet1pt_JER_down>50&&fabs(jet1eta_JER_down)< 4.7) )   &&   ( (!(fabs(jet2eta_JER_down)<3.14 && fabs(jet2eta_JER_down)>2.65) && fabs(jet2eta_JER_down)<4.7 && jet2pt_JER_down<50 && jet2pt_JER_down>30) || (jet2pt_JER_down>50&&fabs(jet2eta_JER_down)< 4.7) )   ) && Mjj_JER_down > 150 && Mjj_JER_down<400 && drla > 0.7 && drla2 > 0.7 && drj1a_JER_down > 0.5 && drj2a_JER_down > 0.5  && ("+drjj_JER_down+")&& drj1l_JER_down > 0.5 && drj2l_JER_down > 0.5 && drj1l2_JER_down > 0.5 && drj2l2_JER_down > 0.5)";
		}
		else{
			JET_new ="jet1pt_new> 30 && jet2pt_new > 30 && fabs(jet1eta_new)< 4.7 && fabs(jet2eta_new)<4.7 && drla > 0.7 && drla2 > 0.7 && drj1a_new > 0.5 && drj2a_new > 0.5 && ("+drjj_new+") && drj1l_new > 0.5 && drj2l_new > 0.5 && drj1l2_new > 0.5 && drj2l2_new > 0.5 && Mjj_new > 150 && Mjj_new<400";
			JET_down="jet1pt_JER_down> 30 && jet2pt_JER_down > 30 && fabs(jet1eta_JER_down)< 4.7 && fabs(jet2eta_JER_down)<4.7 && drla > 0.7 && drla2 > 0.7 && drj1a_JER_down > 0.5 && drj2a_JER_down > 0.5  && ("+drjj_JER_down+")&& drj1l_JER_down > 0.5 && drj2l_JER_down > 0.5 && drj1l2_JER_down > 0.5 && drj2l2_JER_down > 0.5 && Mjj_JER_down > 150 && Mjj_JER_down<400";
			JET_up="jet1pt_JER_up> 30 && jet2pt_JER_up > 30 && fabs(jet1eta_JER_up)< 4.7 && fabs(jet2eta_JER_up)<4.7 && drla > 0.7 && drla2 > 0.7 && drj1a_JER_up > 0.5 && drj2a_JER_up > 0.5  && ("+drjj_JER_up+") && drj1l_JER_up > 0.5 && drj2l_JER_up > 0.5 && drj1l2_JER_up > 0.5 && drj2l2_JER_up > 0.5 && Mjj_JER_up > 150 && Mjj_JER_up<400";
		}
		...
		for(int j=0;j<sample.size();j++){
			...
			if(sample[j].Contains("EWK")){
				...
				//run(dir,tags[i]+"outJER_ZA-EWK",Reco,3,tags[i],0);
			}
			else{
				run(dir1[i],sample[j],Reco,3,tags[i],0);
			}
