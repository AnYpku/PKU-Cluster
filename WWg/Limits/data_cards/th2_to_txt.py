...
fdir = '/home/pku/anying/cms/PKU-Cluster/WWg/Limits/root/'
f_Data=TFile.Open(fdir+'hist_MuonEG_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_HGC = TFile.Open(fdir+'hist_'+sys.argv[4]+'_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
...
th1_WWg=f_WWg.Get('hist_bkg')
th1_HGC=f_HGC.Get('hist_sig')
...
th1_ST.Add(th1_TTA)
th1_ZA.Add(th1_WA)
th1_HGC.Scale(1/(100.*100.))
# the bkg histo and signal histo have already contain the overflow bin in the last bin when creat the histograms 
genbincontent=[]
genbinerror=[]
arr={}
f=open('/home/pku/anying/cms/PKU-Cluster/WWg/Limits/Uncer/summary_uncer_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
for i in range(1,nbins):
   f = open('./txt/emu_%s_%s_%s_%s_bin%i.txt'%(sys.argv[4],sys.argv[1],sys.argv[2],sys.argv[3],i),'w')
...
# bincontent of each precess
   ...
   HGC_bincontent = th1_HGC.GetBinContent(i)  if th1_HGC.GetBinContent(i)>0 else 0
...
# bin error
   ...
   HGC_binerror = th1_HGC.GetBinError(i)/HGC_bincontent if HGC_bincontent>0 else 0
   HGC_binerror = HGC_binerror if HGC_binerror<1 else 1
   HGC_binerror = HGC_binerror+1

   data= HGC_bincontent +  WWg_bincontent + ZA_bincontent+non_prompt_bincontent+fakeL_bincontent+TTA_bincontent+VV_bincontent+ST_bincontent
   ...
   f.write('process\t')
   f.write('sig\tWWg\tVA\tNonprompt_photon\tNonprompt_lepton\tVV\tTop\n')
   f.write('process\t0\t1\t2\t3\t4\t5\t6\n')
   f.write('rate\t')
   f.write('%0.9f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(HGC_bincontent,WWg_bincontent,ZA_bincontent, non_prompt_bincontent,fakeL_bincontent, VV_bincontent, ST_bincontent))
   f.write('------------\n')

   f.write('lumi\tlnN\t')
   f.write('%0.6f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(1.016,1.016,1.016,1.016,1.016))

   f.write('HGC_Stat_bin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('%0.3f\t-\t-\t-\t-\t-\t-\n'%(HGC_binerror))
   ...
   f.write('%0.3f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['jesTotal'+'_'+sys.argv[4]+'_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jesTotal'+'_WWG_emu_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jesTotal'+'_ZGJets_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jesTotal'+'_VV_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jesTotal'+'_Top_'+sys.argv[3]+'_'+sys.argv[2]][i-1]))

   f.write('JER_%s\tlnN\t'%(sys.argv[1]))
   f.write('%0.3f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['jer1'+'_'+sys.argv[4]+'_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jer1'+'_WWG_emu_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jer1'+'_ZGJets_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jer1'+'_VV_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jer1'+'_Top_'+sys.argv[3]+'_'+sys.argv[2]][i-1]))

   f.write('pileup\tlnN\t')
   f.write('%0.3f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['pileup'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[4]][i-1],arr['pileup'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['pileup'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['pileup'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['pileup'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1]))

   f.write('btag\tlnN\t')
   f.write('%0.3f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['btag'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[4]][i-1],arr['btag'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['btag'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['btag'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['btag'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1]))

   if sys.argv[1].find("18") == -1:
      f.write('l1pref\tlnN\t')
      f.write('%0.3f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['l1pref'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[4]][i-1],arr['l1pref'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['l1pref'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['l1pref'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['l1pref'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1]))
   ...
   f.write('pdf_Sig\tlnN\t')
   f.write('%0.3f\t-\t-\t-\t-\t-\t-\n'%(arr['pdf_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[4]][i-1]))

   f.write('scale_sig\tlnN\t')
   f.write('%0.3f\t-\t-\t-\t-\t-\t-\n'%(arr['scale_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[4]][i-1]))

   f.write('mu_id\tlnN\t')
   f.write('%0.3f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['muon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[4]][i-1],arr['muon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['muon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['muon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['muon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1]))

   f.write('mu_iso\tlnN\t')
   f.write('%0.3f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['muon_iso_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[4]][i-1],arr['muon_iso_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['muon_iso_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['muon_iso_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['muon_iso_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1]))

   f.write('ele_reco\tlnN\t')
   f.write('%0.3f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['ele_reco_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[4]][i-1],arr['ele_reco_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['ele_reco_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['ele_reco_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['ele_reco_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1]))

   f.write('ele_ID\tlnN\t')
   f.write('%0.3f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['ele_id_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[4]][i-1],arr['ele_id_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['ele_id_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['ele_id_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['ele_id_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1]))

   f.write('photon_ID\tlnN\t')
   f.write('%0.3f\t%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\n'%(arr['photon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[4]][i-1],arr['photon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['photon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['photon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['photon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1]))
