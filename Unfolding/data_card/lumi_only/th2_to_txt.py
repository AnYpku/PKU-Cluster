...
# bin error
   ...
   genbinerror[i-1]=genbinerror[i-1]/genbincontent[i-1] if genbincontent>0 else 0
   genbinerror[i-1]=genbinerror[i-1]  if genbinerror[i-1]<1 else 1
   genbinerror[i-1]=genbinerror[i-1]+1
...
#   f.write('%0.2f\t-\t-\t-\t-\t-\t%0.2f\n'%(arr['QCD_pdf'][i-1],arr['SigOut_pdf'][i-1]))
##
#   f.write('Scale\tlnN\t')
#   for j in range(1,nbins):
#	 f.write('%0.2f\t'%(arr['genbin{}_scale'.format(j)][i-1]))
#   f.write('%0.2f\t-\t-\t-\t-\t-\t%0.2f\n'%(arr['QCD_scale'][i-1],arr['SigOut_scale'][i-1]))
##
