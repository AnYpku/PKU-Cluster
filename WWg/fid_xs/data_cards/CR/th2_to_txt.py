...
def get_unc(directory):
    arr={}
    f=open(directory+'Uncer/Top_CR/summary_uncer_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.txt')
    for line in f:
            if not line.strip():
                continue
            print line
...
dir1='/home/pku/anying/cms/PKU-Cluster/WWg/Significance/vars/'
dir2='/home/pku/anying/cms/PKU-Cluster/WWg/fid_xs/'
f_Data=TFile.Open(dir1+'CR/root/hist_MuonEG_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_WWg = TFile.Open(dir2+'CR/root/hist_WWG_emu_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_ZA = TFile.Open(dir1+'CR/root/hist_ZGJets_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_plj = TFile.Open(dir1+'CR/root/hist_plj_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_fakeL = TFile.Open(dir1+'CR/root/hist_fakeL_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_TTA = TFile.Open(dir1+'CR/root/hist_TTGJets_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_WA = TFile.Open(dir1+'CR/root/hist_WGJets_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_VV = TFile.Open(dir1+'CR/root/hist_VV_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_ST = TFile.Open(dir1+'CR/root/hist_ST_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')
f_tZq = TFile.Open(dir1+'CR/root/hist_tZq_'+sys.argv[3]+'_'+sys.argv[2]+'_'+sys.argv[1]+'.root')

th1_Data=f_Data.Get('hist_bkg')
th1_WWg=f_WWg.Get('hist_sig')
th1_WWgout=f_WWg.Get('hist_sig_out')
...
for i in range(1,nbins):
   f = open('./txt/emu_%s_%s_%s_CR_bin%i.txt'%(sys.argv[1],sys.argv[2],sys.argv[3],i),'w')
...
# bincontent of each precess
   ...
   WWgout_bincontent = th1_WWgout.GetBinContent(i)  if th1_WWgout.GetBinContent(i)>0 else 0
...
# bin error
   ...
   WWgout_binerror = th1_WWgout.GetBinError(i)/WWgout_bincontent if WWgout_bincontent>0 else 0
   WWgout_binerror = WWgout_binerror if WWgout_binerror<1 else 1
   WWgout_binerror = WWgout_binerror+1

   data= WWg_bincontent + ZA_bincontent+non_prompt_bincontent+fakeL_bincontent+TTA_bincontent+VV_bincontent+ST_bincontent + WWgout_bincontent
   ...
   f.write('process\t')
   f.write('Sig\tVA\tNonprompt_photon\tNonprompt_lepton\tVV\tTop\tSig_out\n')
   f.write('process\t0\t1\t2\t3\t4\t5\t6\n')
   f.write('rate\t')
   f.write('%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(WWg_bincontent,ZA_bincontent, non_prompt_bincontent,fakeL_bincontent, VV_bincontent, ST_bincontent,WWgout_bincontent))
   f.write('------------\n')
   ...
   f.write('Sigout_Stat_bin%d_%s_%s_%s\tlnN\t'%(i,sys.argv[1],sys.argv[2],sys.argv[3]))
   f.write('-\t-\t-\t-\t-\t-\t%0.3f\n'%(WWgout_binerror))
   ...
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr1['jesTotal'+'_WWG_emu_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jesTotal'+'_ZGJets_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jesTotal'+'_VV_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jesTotal'+'_Top_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr1['jesTotal'+'_WWG_emu_out_'+sys.argv[3]+'_'+sys.argv[2]][i-1]))

   f.write('JER_%s\tlnN\t'%(sys.argv[1]))
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr1['jer1'+'_WWG_emu_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jer1'+'_ZGJets_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jer1'+'_VV_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr['jer1'+'_Top_'+sys.argv[3]+'_'+sys.argv[2]][i-1],arr1['jer1'+'_WWG_emu_out_'+sys.argv[3]+'_'+sys.argv[2]][i-1]))

   f.write('pileup\tlnN\t')
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr1['pileup'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['pileup'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['pileup'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['pileup'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1],arr1['pileup'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu_out'][i-1]))

   f.write('btag\tlnN\t')
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr1['btag'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['btag'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['btag'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['btag'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1],arr1['btag'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu_out'][i-1]))

   if sys.argv[1].find("18") == -1:
      f.write('l1pref\tlnN\t')
      f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr1['l1pref'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['l1pref'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['l1pref'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['l1pref'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1],arr1['l1pref'+'_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu_out'][i-1]))
   ...
   f.write('pdf_Sig\tlnN\t')
   f.write('-\t-\t-\t-\t-\t-\t%0.3f\n'%(arr1['pdf_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu_out'][i-1]))

   f.write('scale_sig\tlnN\t')
   f.write('-\t-\t-\t-\t-\t-\t%0.3f\n'%(arr1['scale_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu_out'][i-1]))

   f.write('mu_id\tlnN\t')
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr1['muon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['muon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['muon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['muon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1],arr1['muon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu_out'][i-1]))

   f.write('mu_iso\tlnN\t')
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr1['muon_iso_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['muon_iso_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['muon_iso_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['muon_iso_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1],arr1['muon_iso_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu_out'][i-1]))

   f.write('ele_reco\tlnN\t')
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr1['ele_reco_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['ele_reco_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['ele_reco_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['ele_reco_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1],arr1['ele_reco_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu_out'][i-1]))

   f.write('ele_ID\tlnN\t')
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr1['ele_id_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['ele_id_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['ele_id_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['ele_id_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1],arr1['ele_id_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu_out'][i-1]))

   f.write('photon_ID\tlnN\t')
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr1['photon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu'][i-1],arr['photon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_ZGJets'][i-1],arr['photon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_VV'][i-1],arr['photon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_Top'][i-1],arr1['photon_id_'+sys.argv[3]+'_'+sys.argv[2]+'_WWG_emu_out'][i-1]))
   f.write('ttgamma_xs\tlnN\t')
   f.write('-\t-\t-\t-\t-\t1.1\t-\n')

   f.write('VV_xs\tlnN\t')
   f.write('-\t-\t-\t-\t1.1\t-\t-\n')
   f.write('Top_scale rateParam * Top 1.0 [0.1,10]\n')
