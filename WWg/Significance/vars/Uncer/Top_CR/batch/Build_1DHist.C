...
void run(TString dir,TString name,TString cut,TString tag,TString njets,TString vec_branchname,vector<Double_t> bins1,TString type,int num){
     ...
     for(int i=0;i<tree->GetEntries();i++){
	     ...
	     cout<<weight[0]<<" "<<weight[1]<<" "<<weight[2]<<endl;
		     ...
		     for(int k=0;k<num;k++){
			     h2[k]->Fill(variable[vec_branchname],actualWeight*weight[k]);
		     }
	     }
				...
				for(int j=0;j<names.size();j++){     
					...
					for(int k=0;k<tags.size();k++){
						if(types[ik].Contains("l1pref") && tags[k].Contains("18"))
							continue;
						run(dir1,names[j],Reco,tags[k],njets[ij],vars[i],vars_bins,types[ik],num);
					}
				}
			}
		}
