...
void makeSnapshot(TString wsname, TString mconfig, double true_value, double hypo_tested, TString poi_name_for_hypo, TString workspace_filename,TString outfile_name,TString parameter_var="", double value_parameter_var=0)
{
  ...
  RooSimultaneous *pdf = (RooSimultaneous *)ws->pdf("model_s");
  ...
  if(ws->var("mu_XS_ggF")){
    ws->var("mu_XS_ggF")->setConstant(1);
    ws->var("mu_XS_ggF")->setVal(1);
  }
  if(ws->var("mu_XS_VBF")){
    ws->var("mu_XS_VBF")->setConstant(1);
  ...
  if(ws->var("nbkg_1tag"))
    ws->var("nbkg_1tag")->setMax(10000);
  if(ws->var("nbkg_2tag"))
    ws->var("nbkg_2tag")->setMax(10000);
  ...
  if(parameter_var=="mResonance") {
    ws->var("dNRbkg_1tag")->setVal(10);
    ws->var("dNRbkg_2tag")->setVal(10);
  }
  int  status = robustMinimize(*nll,*minim,robustprintlevel);
  
  if(status!=1&&parameter_var=="mResonance") {
    ws->var("dNRbkg_1tag")->setVal(20);
    ws->var("dNRbkg_2tag")->setVal(20);
    status = robustMinimize(*nll,*minim,robustprintlevel);
  }

  if(parameter_var=="mResonance") {
    if(status!=1) {
      ws->var("dNRbkg_1tag")->setVal(0);
      ws->var("dNRbkg_2tag")->setVal(0);
      status = robustMinimize(*nll,*minim,robustprintlevel);
      if(status!=1) {
	ws->var("dNRbkg_1tag")->setVal(10);
	ws->var("dNRbkg_2tag")->setVal(10);
...
void makeToy(int init_toy, int n_toy, TString wsname, TString mconfig, double true_value, double hypo_tested, TString poi_name_for_hypo, TString workspace_filename, TString outfile_name,int save_np=0,double min_ghost=-999, double max_ghost=-999, double Nghost=100, TString parameter_var="", double value_parameter_var=0){
  ...
  while((parg_nui=(RooRealVar*)iter_nui->Next()) )
      ...
      while((parg_global=(RooRealVar*)it_global->Next()) )
	{
	  ...
	  if (name_of_globs=="RNDM_"+TString(parg_nui->GetName())){
 ...
 if (i%10==0) cout<<"randomization of Gloabal vairalbes!"<<endl; 
	...
	double r=0;
	if (name_of_globs.Contains("bias")) {
	  r = rndm3->Gaus(0,1);
	}
	else {
    ...
    TIterator* iter =  pdf->indexCat().typeIterator() ;
    RooCatType* tt = NULL;
    while((tt=(RooCatType*) iter->Next())) {     
      channellist->defineType(tt->GetName()) ;     
      RooAbsPdf* pdftmp = pdf->getPdf(tt->GetName()) ;
      const RooArgSet* obstmp = pdftmp->getObservables( Observables ); 
      toyDataMap[tt->GetName()] =  pdftmp->generate(*obstmp,Extended());
    }
    RooRealVar wt("wt","wt",1); 
    RooArgSet* args = new RooArgSet();
    args->add(*Observables);
    args->add(wt);
    RooDataSet* toyData = new RooDataSet("toyData", "", *args, Index(*channellist), Import(toyDataMap), WeightVar(wt));
    
    /*Add ghost*/
    if(min_ghost!=max_ghost) {
      RooArgSet *settmp = (RooArgSet*)toyData->get();
      TIterator *it_temp = settmp->createIterator();
      RooCategory* channellist2 = (RooCategory*)it_temp->Next();
      iter->Reset();
      double step = (max_ghost-min_ghost)/Nghost;
      while((tt=(RooCatType*) iter->Next())) {
	channellist2->setLabel(tt->GetName()) ;
	for (float mgg=min_ghost;mgg<max_ghost;mgg+=step) {
	  ws->var("gg_mass")->setVal(mgg);
	  toyData->add(*ws->var("gg_mass"),1e-27);
	}
      }
    }

    /* Float the np and fix the go*/
    ...
    RooCategory *cat = (RooCategory*)ws->cat("channellist");
 ...
 if (i%10==0) cout<<"1st fit"<<endl; 
    ...
    if(parameter_var=="mResonance") {
      if(status_fix!=1) {
        ws->var("bias_2tag")->setVal(0);
        ws->var("bias_1tag")->setVal(0);
        ws->var("dNRbkg_1tag")->setVal(0);
        ws->var("dNRbkg_2tag")->setVal(0);
        status_fix = robustMinimize(*nll_fix,*_minuit,robustprintlevel);
        if(status_fix!=1) {
          ws->var("bias_2tag")->setVal(10);
          ws->var("bias_1tag")->setVal(10);
          ws->var("dNRbkg_1tag")->setVal(10);
          ws->var("dNRbkg_2tag")->setVal(10);
          status_fix = robustMinimize(*nll_fix,*_minuit,robustprintlevel);
          if(status_fix!=1) {
            ws->var("bias_2tag")->setVal(20);
            ws->var("bias_1tag")->setVal(20);
            ws->var("dNRbkg_1tag")->setVal(20);
            ws->var("dNRbkg_2tag")->setVal(20);
            status_fix = robustMinimize(*nll_fix,*_minuit,robustprintlevel);
          }
 ...
 if (i%10==0) cout<<"3rd fit"<<endl;
	...
	if(status_free!=1) {
	  ...
	  ws->var("dNRbkg_1tag")->setVal(10); 
	  ws->var("dNRbkg_2tag")->setVal(10); 
	  PoiStudied->setMin(-10); 
	  PoiStudied->setVal(10); 
	  status_free = robustMinimize(*nll_free,*_minuit2,robustprintlevel); 
	  if(status_free!=1) {
	    ws->var("bias_2tag")->setVal(20);
	    ws->var("bias_1tag")->setVal(20);
    ...
    /*    for(int icat = 0 ; icat< 2 ; icat++) {
      TString temp_dir = outfile_name;
      temp_dir.ReplaceAll("/Toys/","/Toys/PlotAfterFit/");
      TCanvas *c = new TCanvas();
      RooPlot* frame=(*ws->var("gg_mass")).frame();
      frame->SetTitle("");
      ...
      cat->setIndex(icat);
      RooDataSet *obsdata = (RooDataSet*) toyData->reduce(Cut("channellist == "+TString::Itoa(cat->getIndex(),10)));
      obsdata->plotOn(frame, MarkerColor(kBlack));
      ((RooSimultaneous*)pdf)->getPdf(cat->getLabel())->plotOn(frame, LineColor(kRed));
      frame->Draw();
      if(cat->getIndex()==0)
	c->SaveAs(temp_dir+"_2tag_"+TString::Itoa(i,10)+".png");
      else 
	c->SaveAs(temp_dir+"_1tag_"+TString::Itoa(i,10)+".png");
    }
    */
