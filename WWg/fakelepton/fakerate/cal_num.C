...
//		ptbins={20,30,35,40,60};
        ...
        TString dir;
	if(year.Contains("16pre")){dir="../chain_combine/rootfiles_tmp/pre/";year="16";}
	else dir="../chain_combine/rootfiles_tmp/";
...
TFile* run_cal_num(TString channel,TString year){
	...
	if(channel=="mu"){
		...
		cut="( fabs(lepton_pid)==11 && lepton_pt>15 && fabs(lepton_eta)<2.5 && PuppiMET_pt<30 && sqrt(2*(lepton_pt*PuppiMET_pt*(1-cos(lepton_phi-PuppiMET_phi) ) ) )< 20 )";
		cut_mc="( fabs(lepton_pid)==11 && lepton_pt>15 && fabs(lepton_eta)<2.5 && PuppiMET_pt<30 && sqrt(2*(lepton_pt*PuppiMET_pt*(1-cos(lepton_phi-PuppiMET_phi) ) ) )< 20 )";
		file="Ele";
	}
