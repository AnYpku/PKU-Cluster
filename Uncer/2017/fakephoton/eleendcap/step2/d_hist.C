...
//	TTree* t = (TTree*)f1->Get("demo");
	...
	Double_t Mjj,zepp,jet1eta,jet2eta;
	Double_t deltaetajj;
	Double_t theWeight,lumiWeight,scalef,pweight[703],pileupWeight,delta_phi;
...
//	t->SetBranchAddress("deltaetajj", &deltaetajj);
	...
	for(Int_t j=0; j<t->GetEntries();j++)
	{
		...
		if( !(lep==11 && fabs(photoneta)<2.5 && fabs(photoneta)>1.566 && delta_phi>1.9 && zepp<2.4 && Mjj>500 && deltaetajj>2.5) ) continue;
...
int d_hist(){
	TH2D* h0 = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2017/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2017_plj_eleendcap.root",0);
	TH2D* hup = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2017/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2017_plj_eleendcap_up.root",1);
	TH2D* hdown = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2017/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2017_plj_eleendcap_down.root",2);
