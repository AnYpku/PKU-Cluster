...
void unroll_name(TString filename,TString tag,TString type){
	setTDRStyle();
	gStyle->SetOptStat(0);
	TFile* f_ZA=new TFile("./root/hist_"+filename+"_"+type+tag+".root");
	TH1D* t_ZA[num];
	...
	c1->SetFrameFillColor(41);
        ...
        const char *name[16]={"0-50","50-100","100-150","150-#infty","0-50","50-100","100-150","150-#infty","0-50","50-100","100-150","150-#infty"};
	...
	t_ZA[0]->SetMarkerStyle(20);
        ...
        t_ZA[0]->GetYaxis()->SetRangeUser(0,2.5*max);
	t_ZA[0]->Draw("PE");
	...
	for(Int_t i=1;i<num;i++){
        ...
        latex.DrawLatex(0.2,1.3*max,"50<m_{#font[12]{ll}}<100");
        latex.DrawLatex(4.1,1.3*max,"100<m_{#font[12]{ll}}<150");
        latex.DrawLatex(6.5,1.3*max,"m_{#font[12]{ll}}>150");
//        latex.DrawLatex(13, 1.3*max,"m_{#font[12]{ll}}>160");
        ...
        TLine* vline2 = new TLine(t_ZA[0]->GetBinLowEdge(7),0,t_ZA[0]->GetBinLowEdge(7),max*1.5);
        TLine* vline3 = new TLine(t_ZA[0]->GetBinLowEdge(10),0,t_ZA[0]->GetBinLowEdge(10),max*1.5);
