...
//	t->SetBranchAddress("deltaetajj", &deltaetajj);
	...
	{
		t->GetEntry(j);
		if( !(lep==13 && fabs(photoneta)<1.4442 && delta_phi>1.9 && zepp<2.4 && Mjj>500 && deltaetajj>2.5) ) continue;
...
int d_hist(){
	TH2D* h0 = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2018/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2018_plj_mubarrel.root",0);
	TH2D* hup = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2018/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2018_plj_mubarrel_up.root",1);
	TH2D* hdown = run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2018/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2018_plj_mubarrel_down.root",2);
