...
for j in range(1,limit.GetEntries()+1):
    ...
    if param[0] > max_parval:
        max_parval = param[0]
    if param[0]==0:
       print param[0],deltaNLL[0]
    graph.SetPoint(j,param[0],2*deltaNLL[0])
...
if (len(sys.argv) == 8):
    ...
    for j in range(1,limit_2.GetEntries()-1):
        ...
        if param_2[0] > max_parval_2:
            max_parval_2 = param_2[0]
        if param_2[0]==0:
           print param_2[0],2*deltaNLL_2[0]
...
print "95% confidence limits of :"+output_file_name
...
max1=graph.GetMaximum()
print 'max1', max1
graph.GetYaxis().SetRangeUser(0.01,14)
...
c1.SaveAs('./deltaNLL_'+output_file_name+'_observed.pdf')
