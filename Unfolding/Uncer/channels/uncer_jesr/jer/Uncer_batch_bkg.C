...
//             cout<<detajj_new<<" "<<Mjj_new<<"; "<<detajj_JER_up<<" "<<Mjj_JER_up<<"; "<<detajj_JER_down<<" "<<Mjj_JER_down<<endl;
	     ...
	     if (  tformula2->EvalInstance() ){
		     ...
		     if(Mjj_JER_up>=500 &&Mjj_JER_up<2000&&detajj_JER_up>=6)
	     ...
	     if (  tformula3->EvalInstance() ){
		     ...
		     if(Mjj_JER_down>=500 &&Mjj_JER_down<2000&&detajj_JER_down>=6)
     ...
     if(sample.Contains("EWK")&&turn==1)
	     fout=new TFile("./unfold_"+sample+"out_jer"+tag+".root","recreate");
     else 
	     fout=new TFile("./unfold_"+sample+"_Mjj_jer"+tag+".root","recreate");
     fout->cd();
     for(int i=0;i<kk;i++){
	     th1[i]->Write();
     }
