...
void EDBRHistoMaker::Init(TTree *tree) {
	...
	treename->Branch("actualWehgt", &actualWeight, "actualWeight/D");
...
void EDBRHistoMaker::Loop(std::string outFileName) {
	...
	double r2=gRandom->Rndm(jentry*2);
		...
		if (drll>0.3 &&drla>0.7 && drla2>0.7 && lep == 13 &&  ( HLT_Mu1 > 0|| HLT_Mu2 > 0) && ptlep1 > 20. && ptlep2 > 20. && fabs(etalep1) < 2.4 && fabs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep > 70. && massVlep < 110. && photonet > 20. && fabs(photoneta)<2.5&&fabs(photoneta)>1.566 && ZGmass>100 && jet1pt> 30 && jet2pt > 30 && fabs(jet1eta)< 4.7 && fabs(jet2eta)<4.7 && drj1a>0.5 &&drj2a>0.5 && drj1l>0.5 && drj2l>0.5 && drjj>0.5 && /*Mjj>500 && detajj>2.5 &&*/ Mjj > 150 && Mjj<400) {
			numbe_out++;
			treename->Fill();
		}
		else
