...
void run(TString dir,TString name,TString cut,TString tag,TString type,TString njets,TString vec_branchname,vector<Double_t> bins1,vector<Double_t> bins2){
     ...
     for(int i=0;i<tree->GetEntries();i++){
	     ...
	     if(PuppiMET_pt>20 && mT>50 )    
		     h2[0]->Fill(mT,variables[vec_branchname],actualWeight);
	     if(variables["PuppiMET_pt"+type+"Up"]>20 && mT_Up>50 )
		     h2[1]->Fill(mT_Up,variables[vec_branchname],actualWeight);
	     if(variables["PuppiMET_pt"+type+"Down"]>20 && mT_Down>50 )
...
int Build_Hist(){
	...
	TString dir1;
	TString Reco;
	vector<TString> names={"ZGJets","TTGJets","VV","ST","tZq","WGJets","WWG_emu_tot"};
	...
	for(int ij=0;ij<njets.size();ij++){
		...
		else jet_cut="(njets30_pc<=1"+njets[ij]+")";
		for(int i=0;i<vars.size();i++){
			...
			met="(n_bjets20_medium_deepFlavB_pc==0 && "+jet_cut+")";
			Reco= LEP+"&&"+photon+"&&"+met;
