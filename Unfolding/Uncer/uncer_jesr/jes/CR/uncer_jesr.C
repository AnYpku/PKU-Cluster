void run(TString var,TString sample,TString tag){
        fstream ftxt("./"+var+"_jes_uncer"+tag+"CR.txt", ios::app );
	ofstream f1("./"+var+"_"+sample+"_jes_uncer"+tag+"CR.txt");
	TFile*file;
	if(sample.Contains("EWK"))
		file = new TFile("./root/unfold_"+tag+"JESR_ZA-EWK"+tag+"_"+var+"_jes"+tag+".root");
        else file=new TFile("./root/unfold_"+sample+"_"+var+"_jes"+tag+".root");
