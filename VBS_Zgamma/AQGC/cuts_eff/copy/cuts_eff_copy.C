...
void run(/*double medium_barrel,double medium_endcap,*/TString cut1,TString tag, int TotalEntries=0, double lumi=0, bool isEW=1){
	...
	TString filename = isEW? dir+"outZA-EWK.root": dir+"outZA.root";
	...
	vector<TString> vec_options = {"photon_eta", "photon_pev", "photon_hoe", "photon_pt", "photon_chiso", "photon_nhiso", "photon_phoiso", "jeteta", "jetpt", "djeta","mjj"} ;
	...
	temp = {0}; vec_cuts.push_back(temp); vec_eff.push_back(0); vec_yields.push_back(0); vec_error.push_back(0);
        //temp = {1}; vec_cuts.push_back(temp); vec_eff.push_back(0); vec_yields.push_back(0); vec_error.push_back(0);
        //temp = {2}; vec_cuts.push_back(temp); vec_eff.push_back(0); vec_yields.push_back(0); vec_error.push_back(0);
...
*/
                        //PHOTON_barrel[j]=  photon_pev[j]==1 && photon_hoe[j]<0.02197 && fabs(photon_eta[j])<1.4442&&photon_pt[j]>20&&photon_pt[j]<400;
                        //PHOTON_endcap[j]=  photon_pev[j]==1 && photon_hoe[j]<0.0326 && ( fabs(photon_eta[j])<2.5&&fabs(photon_eta[j])>1.566 )&& photon_pt[j]>20&&photon_pt[j]<400;
		...
		for (int index=0; index<vec_cuts.size(); index++ ){
			...
			for (int arr_i=0; arr_i<6; arr_i++){
				tmp_photon_barrel[arr_i] = PHOTON_barrel[arr_i];
				tmp_photon_endcap[arr_i] = PHOTON_endcap[arr_i];	 
					
				for ( int icut=0; icut<vec_cuts[index].size(); icut++ ){
					//cout<<vec_options[vec_cuts[index][icut]]<<endl;
					if ( vec_cuts[index][icut] == 0 ) {
						//cout<<"pass 0";
