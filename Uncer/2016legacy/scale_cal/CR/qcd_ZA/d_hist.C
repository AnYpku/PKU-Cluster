...
//    Double_t pileupWeight;
	...
	for(Int_t j=0; j<t->GetEntries();j++)
	{
		p=0;
		t->GetEntry(j);
		if( !(Mjj>150 && Mjj<500 && Mva>100/*&& detajj>2.5*/) ) continue;
		for(Int_t i=104;i<113;i++){
			if( flag && (i==109 || i==111) ) continue;
			if(p==0)  actualWeight[p]=scalef*pweight[i];
			else      actualWeight[p]=2*scalef*pweight[i];
...
//	run("/home/pku/anying/cms/file_in_cms/2017RunIIrootfiles/2017cutla/cutla-outZA_pweight.root");
	run("/eos/user/y/yian/2016legacy/cutla-outZA_pweight.root");
