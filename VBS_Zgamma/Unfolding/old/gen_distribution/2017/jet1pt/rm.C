...
//      Gen =  GenLEPele  && GenPhoton && GenJet && GenDr && GenSignalRegion;
      ...
      if(name.Contains("EWK") ){ 
              if(Reco){
                      for(int i=0;i<3;i++){
                              int k = 15*i;
			      hist1[i]->Fill(jet1pt,scalef*lumi*pweight[k]);
		      }
		      i1++;
	      }
              if(Gen){
		      ...
		      for(int i=0;i<3;i++){
                              int k = 15*i;
			      hist4[i]->Fill(genjet1pt,scalef*lumi*pweight[k]);
		      }
	      }
              if(Reco && Gen){ 
		      i3++;
		      for(int i=0;i<3;i++){
                              int k = 15*i;
			      hist2[i]->Fill(genjet1pt,scalef*lumi*pweight[k]);
			      hist3[i]->Fill(genjet1pt,scalef*lumi*pweight[k]);
		      }
