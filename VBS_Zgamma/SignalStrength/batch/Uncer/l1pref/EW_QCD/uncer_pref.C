void run(TString channel,TString tag,TString sample){
	ofstream f1("./txt/l1pref_uncer_"+channel+"_"+tag+".txt");
        ofstream ftxt("./l1pref_uncer_"+channel+"_"+tag+".txt",ios::app);
	TFile* file1;
	if(sample.Contains("ZA")==0)
		file1 = new TFile("../root/hist_"+sample+"_pref_"+tag+channel+".root");
	else file1 = new TFile("../root/hist_ewk_pref_"+tag+channel+".root");
	TH1D* h1 = (TH1D*)file1->Get("hist_0");
	TH1D* h2 = (TH1D*)file1->Get("hist_1");
	TH1D* h3 = (TH1D*)file1->Get("hist_2");
	TFile* file;
	if(sample.Contains("ZA")){
		file= new TFile("./hist_qcd_pref_"+tag+channel+".root");
		TH1D* hh1 = (TH1D*)file->Get("hist_0");
		TH1D* hh2 = (TH1D*)file->Get("hist_1");
		TH1D* hh3 = (TH1D*)file->Get("hist_2");
