...
//     cout<<fixed<<setprecision(1)<<100*fabs(biggest_band1[j]-smallest_band1[j])/(2*central_value[j])<<"%,";
}
cout<<endl;
cout<<"uncertaninty band2: ";
for(Int_t j=0;j<p;j++){
     cout<<fixed<<setprecision(3)<<1+fabs(biggest_band2[j]-smallest_band2[j])/(2*central_value[j])<<",";
//     cout<<fixed<<setprecision(1)<<100*fabs(biggest_band2[j]-smallest_band2[j])/(2*central_value[j])<<"%,";
}
cout<<endl;
cout<<"uncertaninty band3: ";
for(Int_t j=0;j<p;j++){
     cout<<fixed<<setprecision(3)<<1+fabs(biggest_band3[j]-smallest_band3[j])/(2*central_value[j])<<",";
//     cout<<fixed<<setprecision(1)<<100*fabs(biggest_band3[j]-smallest_band3[j])/(2*central_value[j])<<"%,";
}
cout<<endl;
