...
double get_ele_Reco_up(double etalep, double ptlep, TH2F* Reco){
	double ptlep_tmp = ptlep;
	if(ptlep>500) ptlep_tmp=499.;
	double lep_Reco_scale=-1;
...
double get_ele_Reco_down(double etalep, double ptlep, TH2F* Reco){
	double ptlep_tmp = ptlep;
	if(ptlep>500) ptlep_tmp=499.;
	double lep_Reco_scale=-1;
	lep_Reco_scale=Reco->GetBinContent(Reco->FindBin(etalep,ptlep_tmp))-Reco->GetBinError(Reco->FindBin(etalep,ptlep_tmp));
...
double get_photon_veto_up(double photoneta,TH1F*SF,TH1F*Stat,TH1F*PU,TH1F*Model){
       double photon_veto_scale=-1;
       if(fabs(photoneta)<1.4442)
	       photon_veto_scale=SF->GetBinContent(1)+sqrt(pow(Stat->GetBinContent(1),2)+pow(PU->GetBinContent(1),2)+pow(Model->GetBinContent(1),2));
       if(fabs(photoneta)<2.5 && fabs(photoneta)>1.566)
	       photon_veto_scale=SF->GetBinContent(4)+sqrt(pow(Stat->GetBinContent(4),2)+pow(PU->GetBinContent(4),2)+pow(Model->GetBinContent(4),2));
       return photon_veto_scale;
}

double get_photon_veto_down(double photoneta,TH1F*SF,TH1F*Stat,TH1F*PU,TH1F*Model){
       double photon_veto_scale=-1;
       if(fabs(photoneta)<1.4442)
	       photon_veto_scale=SF->GetBinContent(1)-sqrt(pow(Stat->GetBinContent(1),2)+pow(PU->GetBinContent(1),2)+pow(Model->GetBinContent(1),2));
       if(fabs(photoneta)<2.5 && fabs(photoneta)>1.566)
