...
//     tree->SetBranchStatus("*",0);
     ...
     for(int k=0;k<tree->GetEntries();k++){
	     ...
	     int p=0;
             ...
             if(tag.Contains("17")==1) puIdweight=puIdweight_T;
             if(tag.Contains("18")==1) {prefWeight=1;puIdweight=puIdweight_L;}
             if(tag.Contains("16")==1) puIdweight=puIdweight_M;
     ...
     TFile*fout[kk];
     for(int j=0;j<kk;j++){
	     fout[j]= new TFile("hist_"+vec_branchname[j]+"_"+sample+"_pu"+tag+".root","recreate");
     }
     for(int j=0;j<kk;j++){
	     fout[j]->cd();
	     for(Int_t i=0;i<num;i++){
