...
//        ftxt<<index<<" "<<endl;
	TFile*f1=new TFile("./root/unfold_"+var+"_"+index+"_EWK"+tag+".root");
	TFile*f2=new TFile("./root/unfold_"+var+"_"+index+"_interf"+tag+".root");
	double lumi;
	if(tag.Contains("16"))
		lumi=35.86;
		...
		lumi=59.7;
        TH1D*h1[num];
        cout<<"./root/unfold_"+var+"_"+index+"_EWK"+tag+".root"<<endl;
        cout<<"./root/unfold_"+var+"_"+index+"_interf"+tag+".root"<<endl;
	h1[0]=(TH1D*)f1->Get(Form(var+"_EWK_recobin%i",i));
	h1[1]=(TH1D*)f2->Get(Form(var+"_interf_recobin%i",i));
	h1[0]->Scale(lumi);
	h1[1]->Scale(lumi);
	...
	for(int k=0;k<kk;k++){
            double error=0;
	    if(h1[0]->GetBinContent(k+1)>0 && h1[1]->GetBinContent(k+1)>0) 
		    error=h1[1]->GetBinContent(k+1)/h1[0]->GetBinContent(k+1);
	    else    error=0;
...
//			cout<<var<<" "<<uncer[k][j]<<endl;
	...
	for(int i=0;i<kk-1;i++){
            for(int j=0;j<kk-1;j++){
               if(j==0)fout<<"genbin"<<i+1<<"_interf=[";
