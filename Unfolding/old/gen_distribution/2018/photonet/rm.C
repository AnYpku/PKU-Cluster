...
//      Gen =  GenLEPele  && GenPhoton && GenJet && GenDr && GenSignalRegion;
      ...
      if(name.Contains("EWK") ){ 
              if(Reco){
                      for(int i=0;i<3;i++){
                              int k = 15*i;
			      hist1[i]->Fill(photonet,scalef*lumi*pweight[k]);
		      }
		      i1++;
	      }
              if(Gen){
		      i2++;
                      h2->Fill(photonet,genphotonet,scalef*lumi);
		      for(int i=0;i<3;i++){
                              int k = 15*i;
			      hist4[i]->Fill(genphotonet,scalef*lumi*pweight[k]);
		      }
	      }
              if(Reco && Gen){ 
		      i3++;
		      for(int i=0;i<3;i++){
                              int k = 15*i;
			      hist2[i]->Fill(genphotonet,scalef*lumi*pweight[k]);
			      hist3[i]->Fill(genphotonet,scalef*lumi*pweight[k]);
		      }
                      h3->Fill(photonet,genphotonet,scalef*lumi);
	      }
              if(Reco&& !Gen) {
                      i4++;
	      }
