...
void run(TString var,int i,TString tag,TString type){
	ofstream ftxt(Form("./txt/"+var+"_recobin%i_uncer_"+type+tag+"CR.txt",i));//,ios::app);
...
void open(TString var,vector<double> genbins,TString tag,int kk,TString type){
	...
	ofstream fout("./"+var+"_uncer_"+type+tag+"CR.txt");
	for(int k=0;k<kk-1;k++){
		ifstream f1(Form("./txt/"+var+"_recobin%i_uncer_"+type+tag+"CR.txt",k+1));
