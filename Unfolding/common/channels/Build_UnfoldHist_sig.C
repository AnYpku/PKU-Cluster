void run(TFile*file,TString var1, TString var2, vector<double> bins,TString cut1,TString cut2,TString cut3,TString tag,TString channel){
     ...
     for(int i=0;i<tag.size();i++){
	     ...
	     for(int j=0;j<genvars.size();j++){     
		     for(int k=0;k<channels.size();k++){
			     run(file[i],genvars[j], recovars[j], bins[j],cut1,cut2,cut3,tag[i],channels[k]);
		     }
	     }
     }
     return 1;
