...
void run( TFile*file,TString cut1,TString tag,bool turn){
	...
	for(int k=0;k<tree->GetEntries();k++){
		...
		if (  tformula->EvalInstance() ){
			for(Int_t i=init;i<(num+init);i++){
				...
				if(Mjj>=500&&Mjj<2000&&detajj>=6)th1[p]->Fill(6.5,actualWeight[p]);//6~7 6~infi 800~1200
				p++;
			}
		}
	}
	...
	if(name.Contains("EWK")==0)
		fout= new TFile("unfold_Mjj_qcd_pdf"+tag+".root","recreate");
	else{
		if(turn==1)	fout= new TFile("unfold_Mjj_Sigout_pdf"+tag+".root","recreate");
