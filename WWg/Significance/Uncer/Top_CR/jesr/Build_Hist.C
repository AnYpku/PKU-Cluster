...
TH1D*unroll(TH2D*h2){
	...
	TH1D*h1_out=new TH1D("h_"+histname,"",h2->GetNbinsX()*h2->GetNbinsY()-3,0,h2->GetNbinsX()*h2->GetNbinsY()-3);
	...
	for(int i=0;i<h1->GetNbinsX();i++){
            if( !(i==8 || i==9 || i==7) ){
		    j++;
		    h1_out->SetBinContent(j,h1->GetBinContent(i+1));
	    }
	}
	h1_out->SetBinContent(6,h1->GetBinContent(6)+h1->GetBinContent(7)+h1->GetBinContent(8));
	h1_out->SetBinContent(7,h1->GetBinContent(9)+h1->GetBinContent(10));
//	return h1_out;
	return h1;
}
void run(TString dir,TString name,TString cut,TString tag,TString type){
     ...
     vector<Double_t> mT_bins    ={0,50,100,150,200};
     vector<Double_t> mll_bins   ={50,100,150,200};
//   vector<Double_t> mllg_bins   ={40,100,150,200,300};
     ...
     for(int i=0;i<tree->GetEntries();i++){
	     ...
	     if(PuppiMET_T1Smear_pt>20 && mT2>30)    
		     h2[0]->Fill(mT,mll,actualWeight);
	     if(variables["PuppiMET_T1Smear_pt_"+type+"Up"]>20 && mT2_Up>30)
		     h2[1]->Fill(mT_Up,mll,actualWeight);
	     if(variables["PuppiMET_T1Smear_pt_"+type+"Down"]>20 && mT2_Down>30)
...
int Build_Hist(){
	TString LEP = "( ( (HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_DZ) || (HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL) || (HLT_Mu12_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ) || (HLT_Mu12_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL) ) && channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && lep1pt>20 && lep2pt>25 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.5 && n_loose_ele==1 && n_loose_mu==1 && ptll>30 && mll>50 && lep1_charge*lep2_charge<0 && drll>0.5 && lep1_is_tight==1 && lep2_is_tight==1 && lep1_isprompt==1 && lep2_isprompt==1)";
