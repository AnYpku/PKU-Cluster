...
void run(TString var,TString sample, TString tag){
	...
	TFile*file=new TFile("./root/hist_"+sample+"_"+var+"_jes"+tag+".root");
	TFile*file1=new TFile("./root/hist_"+sample+"_"+var+"_jes"+tag+".root");
...
int Print_uncer_bkg(){
	...
	vector<TString> sample={"ZA-EWK","ZA","TTA","VV","ST"};
