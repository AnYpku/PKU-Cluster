...
void run(TString cut,TString tag,TString vec_branchname,vector<double> bins1,vector<double> bins2){
     ...
     for(int i=0;i<tree->GetEntries();i++){
		...
		h2[0]->Fill(mT,variable[vec_branchname],scalef);
		h2[1]->Fill(mT,variable[vec_branchname],scalef_up);
		h2[2]->Fill(mT,variable[vec_branchname],scalef_down);
	     }
     ...
     TFile*fout=new TFile("./root/hist_plj_"+vec_branchname+"_"+tag+".root","recreate");
     fout->cd();
     for(int i=0;i<3;i++){
	     hist[i]->Write();
	     h2[i]->Write();
...
int Build_Hist(){
	...
	vector<Double_t> mT_bins={60,90,120,160,200};
        ...
        vector<Double_t> ml1g_bins   ={10,50,120,200};
        vector<Double_t> ml2g_bins   ={10,50,120,200};
        vector<Double_t> mllg_bins   ={15,115,215,315,500};
	...
	for(int k=0;k<tags.size();k++){
		for(int i=0;i<vars.size();i++){
			Reco= LEP+"&&"+photon+"&&"+met;
			run(Reco,tags[k],vars[i],mT_bins,bins2[i]);
		}
	}
	return 1;
}
