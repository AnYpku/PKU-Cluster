...
TH1D* unroll(TH2D* th2_in,Double_t* xbin, Double_t* ybin,  Int_t xbins_in, Int_t ybins_in,char *hname)//,char* htitle)
{
    ...
    TH1D* h1_out= new TH1D(hname, "", 3, 0, 3);
...
//		    cout<<"ix = "<<ix<<", iy = "<<iy<<"; bin = "<<ix+(iy-1)*nbinsx<<", BinContent"<<41.52*th2->GetBinContent(th2->FindBin(x_temp, y_temp))<<endl;
    ...
    h1_out->SetBinContent(1,h1->GetBinContent(1)+h1->GetBinContent(4)+h1->GetBinContent(7));
    h1_out->SetBinContent(2,h1->GetBinContent(2)+h1->GetBinContent(5)+h1->GetBinContent(8));
    h1_out->SetBinContent(3,h1->GetBinContent(3)+h1->GetBinContent(6)+h1->GetBinContent(9));
    delete h1;
    return h1_out;
}
