...
void run(TString var, TString recovar,TString title,TString tag){
        ...
        if(tag.Contains("16")){
          num17=0;num18=0;
          xs16=114.3+55.49*1000;xs17=0;xs18=0;
	  num16=884668+30040390;
	}
        if(tag.Contains("17")){
          num16=0;num18=0;
          xs16=0;xs17=114.3+55.49*1000;xs18=0;
	  num17=799717+30482963;
	}
        if(tag.Contains("18")){
          num16=0;num17=0;
          xs16=0;xs17=0;xs18=114.3+55.49*1000;
	  num18=879399+33805459;
	}
        ...
        TFile*f1=new TFile("../../../pdf_draw/unfold_"+var+"_qcd_pdf16.root");
        TFile*f11=new TFile("../../../scale_draw/"+var+"_unfold_qcd_scale"+tag+".root");
        TH1D*h1[num];TH1D*h2[num];TH1D*h3[num];//ew pdf
        TH1D*h11[9];TH1D*h22[9];TH1D*h33[9];//qcd scale
        TH1D*h4[num];TH1D*h5[num];TH1D*h6[num];//qcd pdf
        TH1D*h44[3];TH1D*h55[3];TH1D*h66[3];//ew scale
        TFile*fout=new TFile("histo_"+var+"_df"+tag+".root","recreate");       
        for(int i=0;i<num;i++){
                if(i<9){
                        h11[i]=(TH1D*)f11->Get(var+Form("_%i",i));
                        if(tag.Contains("16")&&i!=0) h11[i]->Scale(2);
                        if(i<3){
                                h44[i]=(TH1D*)file11->Get(var+Form("_%i",i));
                        }
                ...
                h4[i]=(TH1D*)f1->Get(var+Form("_%i",i));
                h1[i]->Add(h4[i]);
	}
	...
	for(int i=0;i<kk;i++){
        ...
        TFile*fin1=new TFile("../../../pdf_draw/unfold_"+var+"_qcd"+tag+".root");
        TFile*fin2=new TFile("../../../pdf_draw/unfold_"+var+"_interf"+tag+".root");
        TH1D*hist_clone=(TH1D*)fin->Get(var+"_0");
        TH1D*hist_clone1=(TH1D*)fin1->Get(var+"_0");
        TH1D*hist_clone2=(TH1D*)fin2->Get(var+"_0");
        hist_clone->Add(hist_clone1,1);
        hist_clone->Add(hist_clone2,1);
	...
	double Err_up[kk],Err_down[kk],r[kk];
        ...
        ifstream f_in;
	f_in.open("/home/pku/anying/cms/PKU-Cluster/Unfolding/data_card/EW_QCD/combine/unblind/r_"+recovar+"_"+tag+".txt");  
        ifstream f_sys;
	f_sys.open("/home/pku/anying/cms/PKU-Cluster/Unfolding/data_card/EW_QCD/combine/breakdown/unblind/r_sys_"+recovar+"_"+tag+".txt");  
...
//		cout<<Err_down[i]<<" "<<Err_up[i]<<endl;
		...
		hist_clone->SetBinError(i+1,Err_up[i]*hist_clone->GetBinContent(i+1)/(num16+num17+num18)*(xs16+xs17+xs18));//handle the bin error
                cout<<"bin"<<i+1<<"; number of events "<<hist_clone->GetBinContent(i+1)<<"; number of events processed "<<(num16+num17+num18)<<"; cross section "<<(xs16+xs17+xs18)<<"; xsec in this bin "<<hist_clone->GetBinContent(i+1)/(num16+num17+num18)*(xs16+xs17+xs18)<<" bin error"<<hist_clone->GetBinError(i+1)<<endl;
...
//        cout<<"minimum "<<hist_clone->GetMinimum()<<endl; 
...
//		cout<<hist_nominal->GetBinError(i+1)<<endl;
                 ...
                 cout<<"bin"<<i+1<<" "<<y_ratio[i]<<" "<< yerror_down_ratio[i]<<" "<<yerror_up_ratio[i]<<" "<<hist_nominal->GetBinError(i+1)<<endl;
	...
	TGraphAsymmErrors* gr1 = new TGraphAsymmErrors(kk, xbin, y_ratio, xerror_down,xerror_up, yerror_down_ratio, yerror_up_ratio);
        ...
        nominal->GetYaxis()->SetRangeUser(ye_h[pos_l]-ye_l[pos_l]-0.3,ye_h[pos_h]+0.3);
...
//	else if(tag.Contains("18"))latex.DrawLatex(0.76,0.92,Form("58.7 fb^{-1} (%d TeV)", (beamcomenergytev)));
        ...
        latex.SetLineWidth(2);
        latex.DrawLatex(0.17,0.5,"2.5<#Delta#eta_{jj}<4.5");
        latex.DrawLatex(0.5 ,0.5,"4.5<#Delta#eta_{jj}<6");
        latex.DrawLatex(0.8 ,0.5,"#Delta#eta_{jj}>6");
}
