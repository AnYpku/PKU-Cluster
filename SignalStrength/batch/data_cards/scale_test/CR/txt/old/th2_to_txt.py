...
genbinerror=[]
arr={}
f=open('../../../../../CR/Uncer/old/summary_uncer_'+sys.argv[2]+sys.argv[1]+'CR.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
for i in range(1,nbins):
   f = open('./%s_%s_control_bin%i.txt'%(sys.argv[2],sys.argv[1],i),'w')
...
#   f.write('%0.3f\t-\t-\t-\t-\t-\t%0.2f\n'%(arr['Sig_scale'][i-1],arr['SigOut_scale'][i-1]))
   ...
   f.write('Scale_QCD_band1\tlnN\t')
   f.write('-\t%0.2f\t-\t-\t-\t-\t-\n'%(arr['scale_band1'][i-1]))
#
   f.write('Scale_QCD_band2\tlnN\t')
   f.write('-\t%0.2f\t-\t-\t-\t-\t-\n'%(arr['scale_band2'][i-1]))

   f.write('Scale_QCD_band3\tlnN\t')
   f.write('-\t%0.2f\t-\t-\t-\t-\t-\n'%(arr['scale_band3'][i-1]))
#
   f.write('interf\tlnN\t')
...
#   print 'bin ',i,' ',ZA_binerror,' ',non_prompt_binerror,' ',TTA_binerror,' ',VV_binerror,' ',ST_binerror,' ',WA_binerror,' ',ZA_sig_out_binerror
   ...
   f.write('theory group = pdf_EW pdf_QCD Scale_EW Scale_QCD_band1 Scale_QCD_band2 Scale_QCD_band3 interf \n')
