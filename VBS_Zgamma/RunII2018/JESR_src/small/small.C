...
//   for (Long64_t jentry=0; jentry<1000;jentry++){
      ...
      scalef = 1000.*55.49/(np-nm)*theWeight/fabs(theWeight);
...
//      cout<<"fill zepp end"<<endl;
      ...
      for(int i=0;i<jet1pt_unc_up->size();i++){
          JET_up=jet1pt_unc_up->at(i)>30 && jet2pt_unc_up->at(i)>30 && fabs(jet1eta_unc_up->at(i))<4.7 && fabs(jet2eta_unc_up->at(i))<4.7 && drj1a_up->at(i)>0.5 && drj1l1_up->at(i)>0.5 && drj1l2_up->at(i)>0 && drj2l1_up->at(i)>0 && drj2l2_up->at(i)>0.5 && drjj_up->at(i)>0.5 && detajj_up->at(i)>2.5 && Mjj_up->at(i)>500 && zepp_up[i]<2.4 && delta_phi_up[i]>1.9 && Mva>100;
          JET_down=jet1pt_unc_down->at(i)>30 && jet2pt_unc_down->at(i)>30 && fabs(jet1eta_unc_down->at(i))<4.7 && fabs(jet2eta_unc_down->at(i))<4.7 && drj1a_down->at(i)>0.5 && drj1l1_down->at(i)>0.5 && drj1l2_down->at(i)>0 && drj2l1_down->at(i)>0 && drj2l2_down->at(i)>0.5 && drjj_down->at(i)>0.5 && detajj_down->at(i)>2.5 && Mjj_down->at(i)>500 && zepp_down[i]<2.4 && delta_phi_down[i]>1.9 && Mva>100; 
...
//          if(jentry%100==0)cout<<"Fill histograms"<<endl;
      ...
      JET=jet1pt_new>30 && jet2pt_new>30 && fabs(jet1eta_new)<4.7 && fabs(jet2eta)<4.7&&Mjj_new>500&&deltaeta_new>2.5 && drj1a_new>0.5 && drj1l_new>0.5 && drj2l_new>0.5 && drj1l2_new>0.5 && drj2l2_new>0.5 && zepp<2.4 && delta_phi>1.9 && Mva>100 ;
