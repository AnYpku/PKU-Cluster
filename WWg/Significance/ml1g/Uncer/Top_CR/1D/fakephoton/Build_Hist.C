...
//	     h2[i]=new TH2D(th2name+Form("_%d",i),"",mT_bins.size()-1,&mT_bins[0],mll_bins.size()-1,&mll_bins[0]);
     ...
     for(int i=0;i<tree->GetEntries();i++){
	     ...
	     if (  tformula1->EvalInstance() && mT2>30 ){ 
		if (mT>=200) mT=199;
		if (mll>=200) mll=199;
                if(mllg>300) mllg=299;
                cout<<scalef<<" "<<scalef_up<<" "<<scalef_down<<endl;
		h2[0]->Fill(mT,scalef);
		h2[1]->Fill(mT,scalef_up);
		h2[2]->Fill(mT,scalef_down);
	     }
...
int Build_Hist(){
	TString LEP = "(( (HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_DZ) || (HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL) || (HLT_Mu12_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ) || (HLT_Mu12_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL) ) && channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && lep1pt>20 && lep2pt>25 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.5 && n_loose_ele==1 && n_loose_mu==1 && ptll>30 && mll>50 && lep1_charge*lep2_charge<0 && drll>0.5 && lep1_is_tight==1 && lep2_is_tight==1)";
	...
	TString met="(n_bjets>=1 && PuppiMET_T1_pt > 20  )";
