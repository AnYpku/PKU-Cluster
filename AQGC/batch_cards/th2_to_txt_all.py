...
fdir = '/home/pku/anying/cms/PKU-Cluster/AQGC/batch_cards/root/'
f_EW1 = TFile.Open(fdir+'hist_ZA-EWK16'+sys.argv[2]+'.root')
f_ZA1 = TFile.Open(fdir+'hist_ZA16'+sys.argv[2]+'.root')
f_plj1 = TFile.Open(fdir+'hist_plj16'+sys.argv[2]+'.root')
f_TTA1 = TFile.Open(fdir+'hist_TTA16'+sys.argv[2]+'.root')
f_VV1 = TFile.Open(fdir+'hist_VV16'+sys.argv[2]+'.root')
f_ST1 = TFile.Open(fdir+'hist_ST16'+sys.argv[2]+'.root')

th1_ZA_sig=f_EW1.Get('hist')
th1_ZA=f_ZA1.Get('hist')
th1_non_prompt=f_plj1.Get('hist')
th1_TTA=f_TTA1.Get('hist')
th1_VV=f_VV1.Get('hist')
th1_ST=f_ST1.Get('hist')

merge_year(th1_ZA_sig,'ZA-EWK')
merge_year(th1_ZA,'ZA')
merge_year(th1_non_prompt,'plj')
merge_year(th1_TTA,'TTA')
merge_year(th1_VV,'VV')
merge_year(th1_ST,'ST')
genbincontent=[]
genbinerror=[]
arr={}
f=open('/home/pku/anying/cms/PKU-Cluster/AQGC/Uncer_aQGC/summary_uncer_'+sys.argv[2]+sys.argv[1]+'.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
for i in range(1,nbins):
   f = open('./txt/%s_all_bin%i.txt'%(sys.argv[2],i),'w')
...
#   WA_binerror = WA_binerror+1
   ...
   f.write('------------\n')
   print ZA_sig_bincontent,ZA_bincontent, non_prompt_bincontent, TTA_bincontent, VV_bincontent, ST_bincontent
...
#       f.write('%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\n'%(1.015,1.015,1.015,1.015,1.015))
   if sys.argv[1].find("16")==-1 and sys.argv[1].find("17") and sys.argv[1].find("18")==-1:
       f.write('lumi_16\tlnN\t%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr['lumi16'][0],arr['lumi16'][0],arr['lumi16'][0],arr['lumi16'][0],arr['lumi16'][0]))
       f.write('lumi_17\tlnN\t%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr['lumi17'][0],arr['lumi17'][0],arr['lumi17'][0],arr['lumi17'][0],arr['lumi17'][0]))
       f.write('lumi_18\tlnN\t%0.3f\t%0.3f\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr['lumi18'][0],arr['lumi18'][0],arr['lumi18'][0],arr['lumi18'][0],arr['lumi18'][0]))
