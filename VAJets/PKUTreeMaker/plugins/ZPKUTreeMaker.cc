...
//	iEvent.getByToken(phoPhotonIsolationToken_, phoPhotonIsolationMap);
	...
	for (size_t ip=0; ip<photons->size();ip++)
	{
		...
		double pho_ieie=(*photons)[ip].full5x5_sigmaIetaIeta();//(*full5x5SigmaIEtaIEtaMap)[pho];
                ...
                double chiso = std::max(0.0, chIso2 - rhoVal_*EAch(fabs(phosc_eta)));
                double nhiso = std::max(0.0, nhIso2 - rhoVal_*EAnh(fabs(phosc_eta)));
                double phoiso= std::max(0.0, phIso2 - rhoVal_*EApho(fabs(phosc_eta)));
		...
		if(ip<6)  {
			...
			photon_chiso[ip]=chiso1;
			photon_nhiso[ip]=nhiso1;
			photon_phoiso[ip]=phoiso1;
