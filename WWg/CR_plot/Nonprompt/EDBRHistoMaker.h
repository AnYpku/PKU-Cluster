...
void EDBRHistoMaker::createAllHistos(TString isChannel) {
	...
	hs.setHisto("njets", 15, 0, 15);
	hs.setHisto("njets30", 10, 0, 10);
	hs.setHisto("njets40", 8, 0, 8);
	hs.setHisto("njets50", 7, 0, 7);
...
//        nentries=100;
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		actualWeight=1;
	        ...
	        if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && lep1_is_tight==1 && lep2_is_tight==1 && mll >20 && ptll > 30 && n_photon>0  && photonet > 20. && drl1a>0.5 && drl2a>0.5 && photon_channel && photon_selection==1 && PuppiMET_T1_pt > 20 && n_bjets>=1 && mT>60 ){
...
//	nentries=100;
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && mll >20 && ptll > 30 && lepton_flag && n_photon>0  && photonet > 20. && drl1a>0.5 && drl2a>0.5 && photon_channel && photon_flag==1 && PuppiMET_T1Smear_pt > 20 && n_bjets>=1 && mT>60 ){
			if(gen_weight>0) npp++;
			if(gen_weight<0) nmm++;
			numbe_out++;
			treename->Fill();
