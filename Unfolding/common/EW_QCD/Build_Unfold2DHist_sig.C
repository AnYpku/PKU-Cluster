void run(TString sample,TString tag, TString cut1,TString cut2,TString cut3,TString Reco){
     ...
     TFile*file;
     file=new TFile(dir+"unfold_GenCutla-ZA"+tag+".root");
     const int nbins=9; 
     ...
     double muon1_id_scale,muon2_id_scale,muon1_iso_scale,muon2_iso_scale,ele1_id_scale,ele2_id_scale,ele1_reco_scale,ele2_reco_scale,photon_id_scale,photon_veto_scale,pileupWeight,prefWeight,puIdweight_T,ele_hlt_scale,muon_hlt_scale;
     ...
     for(int i=0;i<tree->GetEntries();i++){
	     ...
	     actualWeight=scalef*pileupWeight*prefWeight*lumi*photon_id_scale*photon_veto_scale*puIdweight_T;
	     if(lep==11)
...
//             if(i%1000==0) cout<<"actualWeight "<<actualWeight<<endl;
	     if (  tformula3->EvalInstance()&&sample.Contains("ZA") ){ 
		     count++;
	     ...
	     if (  tformula1->EvalInstance()&&sample.Contains("ZA") ) {//reco && gen
	     ...
	     if (  tformula2->EvalInstance()&&sample.Contains("ZA") ){//reco && !gen
	     ...
	     if (  tformula->EvalInstance() &&sample.Contains("ZA")==0 ){//reco
		     if(Mjj<2000&deltaetajj<6.5)
