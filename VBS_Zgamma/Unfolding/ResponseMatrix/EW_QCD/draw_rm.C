...
TH2D*run(TString var,TString tag,int num){
	...
	TFile*file1=new TFile("/home/pku/anying/cms/PKU-Cluster/Unfolding/common/EW_QCD/root/hist_ZA_"+var+tag+".root");
	TH1D*h2=(TH1D*)file->Get("histGen_"+var);
	TH1D*hh2=(TH1D*)file1->Get("histGen_"+var);
        h2->Add(hh2,1);
        const int binx=h2->GetNbinsX();
        TH1D*h1[num];TH1D*hh1[num];
	for(int i=0;i<binx;i++){
		h1[i]=(TH1D*)file->Get(var+Form("_%i",i));
		hh1[i]=(TH1D*)file1->Get(var+Form("_%i",i));
		h1[i]->Add(hh1[i]);
	}
...
int draw_rm(){
	...
	for(int i=0;i<genvars.size();i++){
		for(int j=0;j<tag.size();j++){
			...
			c1->Print("figs/rm_"+genvars[i]+tag[j]+".pdf");
		}
	}
	return 0;
}
