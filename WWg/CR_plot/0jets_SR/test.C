...
void EDBRHistoMaker::Init(TTree *tree) {
	...
	treename->Branch("ele_id_scale", &ele_id_scale, "ele_id_scale/D");
	treename->Branch("ele_reco_scale", &ele_reco_scale, "ele_reco_scale/D");
	treename->Branch("muon_id_scale", &muon_id_scale, "muon_id_scale/D");
	treename->Branch("muon_iso_scale", &muon_iso_scale, "muon_iso_scale/D");
...
//        nentries=100;
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		actualWeight=0;
	        ...
	        if( lepton_channel && (n_loose_ele + n_loose_mu) == 2 && n_photon>0  && photonet > 20. && photon_channel && puppimet > 20 && ptll>30 && mll>20 && n_bjets==0 && mT2>30 && mT>60){
...
//	nentries=100;
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if( lepton_channel && (n_loose_ele + n_loose_mu) == 2 && n_photon>0  && photonet > 20. && photon_channel /*&& photon_flag==1*/ && puppimet > 20 && ptll>30 && mll>20 && n_bjets==0 && mT2>30 && mT>60 ){
			if(gen_weight>0) npp++;
			if(gen_weight<0) nmm++;
			numbe_out++;
			treename->Fill();
