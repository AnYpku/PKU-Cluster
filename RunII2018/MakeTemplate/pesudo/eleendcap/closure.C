...
Double_t closureFake_test,closureFake_uncer;
...
TString dir = "./txt/";
TString dir1 = "../fit_eleendcap/fractionfitResult_za/";
...
//    cout<<"selectFake_number"<<selectFake_number;
    ...
    cout<<"fitFake_number "<<fitFake_number<<endl; 
    closureFake_test = TMath::Abs((FakeNumber - fitFake_number)/fitFake_error);
    closureFake_uncer = TMath::Abs((FakeNumber - fitFake_number)/FakeNumber);
...
//  myfile<<"*"<<"\t"<<"3"<<"\t"<<"4"<<"\t"<<"5"<<"\t"<<"6"<<"\t"<<"7"<<"\t"<<"8"<<endl;
    ...
    ofstream myfile3(dir2 + Form("closure-uncer_pt%0.f-%0.f.txt",lowpt[i],highpt[i]));
      for(Int_t j=0;j<21;j++){
         run_calculate(i,j);
         myfile2<<closureFake_test<<endl;
         myfile3<<closureFake_uncer<<endl;
       }
  file<<"#######################################################################################"<<endl;    
  file1<<"#######################################################################################"<<endl;    
  file2<<"#######################################################################################"<<endl;    
