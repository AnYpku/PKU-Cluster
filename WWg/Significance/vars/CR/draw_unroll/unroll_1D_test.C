...
void unroll_run(TString tag,TString var,TString title,TString njets){
	...
	TFile* f_TT=TFile::Open("../root/hist_1D_TTJets_"+var+"_"+njets+"_"+tag+".root");
        TH1D* th2_TT16=(TH1D*)f_TT->Get("hist_bkg");
        TH1D* th2_TT= unroll(th2_TT16,"t_TT");

	th2_TT->SetFillColor(kBlue-7);
	th2_TT->SetMarkerColor(kBlue-7);
        th2_TT->SetLineColor(kBlue-7);
	th2_TTA->Add(th2_TT);
...
//        h_up->Draw("EP same");
	...
	c1->Print("aa_CR_1D_"+var+"_"+njets+"_"+tag+".pdf");
}
int unroll_1D_test(){
