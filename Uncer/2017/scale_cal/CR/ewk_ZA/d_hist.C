...
//	Double_t mjj_bins[4]={500, 750, 1000, 2000};
	...
	for(Int_t j=0; j<t->GetEntries();j++)
	{
        p=0;
		t->GetEntry(j);
        if( !(Mjj>150 && Mjj<500 && ZGmass>100/*&& detajj>2.5*/) ) continue;
        for(Int_t i=0;i<num;i++){
            int k = 15*i;
...
int d_hist(){
	run("/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2017/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2017_ZA-EWK-pweight.root");
