#define pi 3.1415926
TH1D*run(TString sample,TString cut1,TString tag,TString channel){
	TFile*file;
...
//	vector<double> mjj_bins={800,1000,1300,1600,2000};
	vector<double> mjj_bins={500,600,700,800};
	/*for(int i=0;i<=10;i++){
		double mjj_tmp=init+step*i;
		mjj_bins.push_back(mjj_tmp);
	}*/
	...
	TH1D*h1=new TH1D(hist_name1,"",mjj_bins.size()-1,&mjj_bins[0]);
        ...
        if(channel.Contains("mubarrel")) cut1="("+cut1+")"+"&&(lep==13&&(fabs(photoneta)<1.4442))";
        else if(channel.Contains("mubarrel")) cut1="("+cut1+")"+"&&(lep==11&&(fabs(photoneta)<1.4442))";
        else if(channel.Contains("muendcap")) cut1="("+cut1+")"+"&&(lep==13&&(fabs(photoneta)<2.5 && fabs(photoneta)>1.566))";
	else cut1="("+cut1+")"+"&&(lep==11&&(fabs(photoneta)<2.5 && fabs(photoneta)>1.566))";
	TTreeFormula *tformula1=new TTreeFormula("formula1", "(("+cut1+"))", tree);
        int lep;double Mjj; double actualWeight;
	double puIdweight_M,puIdweight_T;
	...
	for(int i=0;i<tree->GetEntries();i++){
		...
		if(Mjj>2000) Mjj=1999;
                if(sample.Contains("Muon")||sample.Contains("Ele")){actualWeight=1;puIdweight_T=1;puIdweight_M=1;}
                if(sample.Contains("plj")) {puIdweight_T=1;puIdweight_M=1;}
                if(tag.Contains("17")) actualWeight=actualWeight*puIdweight_T;
                else actualWeight=actualWeight*puIdweight_M;
...
int Find_mjj_ch(){
	...
	for(int k=0;k<tags.size();k++){
		...
		for(int i =0;i<channel.size();i++){
			...
			h_ST[k]=run("ST",Reco,tags[k],channel[i]);
			h_VV[k]=run("VV",Reco,tags[k],channel[i]);
			h_TTA[k]=run("TTA",Reco,tags[k],channel[i]);
			h_ZA[k]=run("ZA",Reco,tags[k],channel[i]);
			h_plj[k]=run("plj",Reco,tags[k],channel[i]);
			h_EWK[k]=run("ZA-EWK",Reco,tags[k],channel[i]);
			TH1D*h1;
			if(channel[i].Contains("mu")){
				h_Muon[k]=run("Muon",Reco,tags[k],channel[i]);
				h1=(TH1D*) h_Muon[k]->Clone();
			}
			else{
				h_Ele[k]=run("Ele",Reco,tags[k],channel[i]);
				h1=(TH1D*) h_Ele[k]->Clone();
			}
			...
			leg->Draw();
			leg1->Draw();
			c1->SaveAs("hist_"+channel[i]+"_"+tags[k]+".pdf");
		}
	}
	return 0;
}
