...
int Uncer_batch_bkg(){
	...
	for(int i=0;i<tag.size();i++){
		...
		for(int jk=0;jk<kk;jk++){
			hist[i][jk]=run("",tag[i],cut1,channels[jk]);
			hist_up[i][jk]=run("_up",tag[i],cut1,channels[jk]);
			hist_down[i][jk]=run("_down",tag[i],cut1,channels[jk]);
		}
	}
	...
	for(int i=0;i<3;i++){
		for(int jk=0;jk<kk;jk++){
			fout[i][jk]= new TFile("unfold_Mjj_plj_"+tag[i]+channels[jk]+".root","recreate");
			cout<<hist[i][jk]->GetBinContent(1)<<endl;
			fout[i][jk]->cd();
			hist[i][jk]->Write();
			hist_up[i][jk]->Write();
			hist_down[i][jk]->Write();
			fout[i][jk]->Close();
		}
	}//fout[year][vars]
	/*     for(int j=0;j<kk;j++){
