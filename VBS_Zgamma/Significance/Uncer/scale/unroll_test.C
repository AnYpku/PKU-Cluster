...
void unroll_name(TString sample,TString tag,int num){
	...
	for(Int_t i=0;i<num;i++){
		...
		cc[p] = new TCanvas(Form("cc_%d",i),Form("Mjj vs deltajj %d",i+1),800,600);
		...
		if(i==1) ll[p]->AddEntry(t_ZA[p],Form("%d nominal scale variation",i+1));
		else     ll[p]->AddEntry(t_ZA[p],lable_scale[p]);
		ll[p]->Draw();
	...
	TLegend *l2 = new TLegend(0.7,0.5,0.82,0.82);
	TLegend *l1 = new TLegend(0.41,0.5,0.7,0.82);
        l2->SetLineColor(10);
        l1->SetLineColor(10);
	fPads1->SetTopMargin(0.15);
	fPads1->SetBottomMargin(0.05);
	...
	t_ZA[0]->GetYaxis()->SetTitleOffset(0.6);
	t_ZA[0]->GetYaxis()->SetLabelSize(0.065);
	t_ZA[0]->GetXaxis()->SetLabelSize(0.0);
	l2->AddEntry(t_ZA[0]," nominal scale ");
	...
	for(Int_t i=1;i<p;i++){
		...
		t_ZA[i]->GetYaxis()->SetTitleSize(0.1);
		if(i<3) l2->AddEntry(t_ZA[i], lable_scale[i]);
		else if(i==5)   l1->AddEntry(t_ZA[i],"scale(0.5,1) ");
		else if(i==6)   l1->AddEntry(t_ZA[i],"scale(0.5,0.5) ");
		else l1->AddEntry(t_ZA[i], lable_scale[i]);
	}
