...
f_WWg = TFile.Open(fdir+'hist_1D_WWG_'+sys.argv[1]+'.root')
f_ZA = TFile.Open(fdir+'hist_1D_ZGJets_'+sys.argv[1]+'.root')
f_plj = TFile.Open(fdir+'hist_1D_plj_'+sys.argv[1]+'.root')
f_fakeL = TFile.Open(fdir+'hist_1D_fakeLepton_'+sys.argv[1]+'.root')
f_TTA = TFile.Open(fdir+'hist_1D_TTGJets_'+sys.argv[1]+'.root')
f_WA = TFile.Open(fdir+'hist_1D_WGJets_'+sys.argv[1]+'.root')
f_VV = TFile.Open(fdir+'hist_1D_VV_'+sys.argv[1]+'.root')
f_ST = TFile.Open(fdir+'hist_1D_ST_'+sys.argv[1]+'.root')
f_tZq = TFile.Open(fdir+'hist_1D_tZq_'+sys.argv[1]+'.root')

th1_WWg=f_WWg.Get('sig')
th1_ZA=f_ZA.Get('bkg')
th1_non_prompt=f_plj.Get('bkg')
th1_fakeL=f_fakeL.Get('bkg')
th1_TTA=f_TTA.Get('bkg')
th1_WA=f_WA.Get('bkg')
th1_VV=f_VV.Get('bkg')
th1_ST=f_ST.Get('bkg')
th1_tZq=f_tZq.Get('bkg')
th1_ST.Add(th1_tZq)
th1_ZA.Add(th1_WA)
...
genbinerror=[]
arr={}
f=open('/home/pku/anying/cms/PKU-Cluster/WWg/Significance/Uncer/Top_CR/1D/summary_uncer_'+sys.argv[1]+'.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
# bin error
   ...
   f.write('rate\t')
   f.write('%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\t%0.3f\n'%(WWg_bincontent,ZA_bincontent, non_prompt_bincontent,fakeL_bincontent,TTA_bincontent, VV_bincontent, ST_bincontent))
   f.write('------------\n')
   ...
   f.write('TTA_Stat_CR_bin%d_%s\tlnN\t'%(i,sys.argv[1]))
   f.write('-\t-\t-\t-\t%0.3f\t-\t-\n'%(TTA_binerror))

   f.write('VV_Stat_CR_bin%d_%s\tlnN\t'%(i,sys.argv[1]))
   f.write('-\t-\t-\t-\t-\t%0.3f\t-\n'%(VV_binerror))

   f.write('ST_Stat_CR_bin%d_%s\tlnN\t'%(i,sys.argv[1]))
   f.write('-\t-\t-\t-\t-\t-\t%0.3f\n'%(ST_binerror))
   ...
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr['jesTotal'+'_WWG'][i-1],arr['jesTotal'+'_ZGJets'][i-1],arr['jesTotal'+'_TTGJets'][i-1],arr['jesTotal'+'_VV'][i-1],arr['jesTotal'+'_ST'][i-1]))

   f.write('JER_%s\tlnN\t'%(sys.argv[1]))
   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr['jer1'+'_WWG'][i-1],arr['jer1'+'_ZGJets'][i-1],arr['jer1'+'_TTGJets'][i-1],arr['jer1'+'_VV'][i-1],arr['jer1'+'_ST'][i-1]))
...
#   f.write('%0.3f\t%0.3f\t-\t-\t%0.3f\t%0.3f\t%0.3f\n'%(arr['photon_ZA-EWK_ID'][i-1],arr['photon_ZA_ID'][i-1],arr['photon_TTA_ID'][i-1],arr['photon_VV_ID'][i-1],arr['photon_ST_ID'][i-1]))
   ...
   f.write('VV_xs\tlnN\t')
   f.write('-\t-\t-\t-\t-\t1.1\t-\n')
   f.write('TTG_scale rateParam * TTA 1.0 [0.1,4]\n')
   f.write('ST_scale rateParam * ST 1.0 [0.1,4]\n')
