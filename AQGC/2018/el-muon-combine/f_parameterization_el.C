...
//	const double xf[17] = {-20, -10, -6, -5, -3, -2, -1, -0.6, 0, 0.6, 1, 2, 3, 5, 6, 10, 20};
	...
	for(int abin=0;abin<5;abin++){
		...
		for(int count=0; count < numberOfEntries; count++){
			...
			Double_t weight;
			if(lep==11)
				weight=scalef*pileupWeight*photon_id_scale*photon_veto_scale*ele1_id_scale*ele2_id_scale*ele1_reco_scale*ele2_reco_scale;
			else    weight=pileupWeight*scalef*muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale*photon_id_scale*photon_veto_scale;
