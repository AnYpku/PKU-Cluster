	...
	for(int abin=0;abin<ZGbin.size()-1;abin++){
		...
		for(int count=0; count < numberOfEntries; count++){
			...
			if( ! formula->EvalInstance())
				continue;
                        if(Mva>2e4) Mva=1999;
			Double_t weight=pileupWeight * scalef*ele1_id_scale*ele2_id_scale*ele1_reco_scale*ele2_reco_scale*photon_id_scale*photon_veto_scale*prefWeight*puIdweight_M*ele_hlt_scale;
			if(count%100==0){
				cout<<pileupWeight <<" "<< scalef<<" "<<ele1_id_scale<<" "<<ele2_id_scale<<" "<<ele1_reco_scale<<" "<<ele2_reco_scale<<" "<<photon_id_scale<<" "<<photon_veto_scale<<" "<<prefWeight<<" "<<puIdweight_M<<" "<<ele_hlt_scale<<endl;
				cout<<"abin="<<abin<<" count="<<count<<" weight "<<weight<<endl;
			}
...
void f_parameterization_el(){
	...
	fX0_parameterization_el(4,ZGbin);
	fX0_parameterization_el(5,ZGbin);
	fX0_parameterization_el(6,ZGbin);
	fX0_parameterization_el(7,ZGbin);
	fX0_parameterization_el(8,ZGbin);
	fX0_parameterization_el(9,ZGbin);
	fX0_parameterization_el(10,ZGbin);
	fX0_parameterization_el(11,ZGbin);
	fX0_parameterization_el(12,ZGbin);
	fX0_parameterization_el(13,ZGbin);
	fX0_parameterization_el(14,ZGbin);
	fX0_parameterization_el(15,ZGbin);
	fX0_parameterization_el(16,ZGbin);
}
