...
//		file_ID->Close();
	...
	for(int k=0;k<tree->GetEntries();k++){
		...
		if(particle.Contains("muon")&&tag.Contains("16")&&lep==13){
			...
			if(type.Contains("trigger")){
				muon_WeightUp=muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale*scalef*puIdweight_T*prefWeight*photon_id_scale*photon_veto_scale*(muon_hlt_scale+muon_hlt_Uncer);
		...
		if (  tformula->EvalInstance() ){
			if(lep==13){
				...
				for(int i=0;i<3;i++){
					if(Mjj<500)th1[i]->Fill(Mjj,muon_weight[i]);//0~1, 2.5~4.5 and 500~800                
				}
			}
			if(lep==11){
				...
				for(int i=0;i<3;i++){
					if(Mjj<500)th1[i]->Fill(Mjj,ele_weight[i]);//0~1, 2.5~4.5 and 500~800                
				}
			}
			if(photonet>0){
				...
				for(int i=0;i<3;i++){
					if(Mjj<500)th1[i]->Fill(Mjj,photon_weight[i]);//0~1, 2.5~4.5 and 500~800                
				}
			}
		}
	...
	TFile*fout=new TFile("./"+particle+"_"+sample+"_"+type+tag+"CR.root","recreate");
	fout->cd();
	for(int i=0;i<3;i++){
		th1[i]->Write();
	}
...
//	vector<TString> tag={"16","17","18"};
	...
	for(int j=0;j<tag.size();j++){
		...
		TString ControlRegion = "(Mjj<500 && Mjj>150 && ZGmass>100)";
		...
		for(int i=0;i<par.size();i++){
			for(int k=0;k<sample.size();k++){
				...
				if(par[i].Contains("muon")){ 
					cal(par[i],sample[k],tag[j],cut_final,th2[i],"all");
					cal(par[i],sample[k],tag[j],cut_final,th2[i],"trigger");
				}
				if(par[i].Contains("photon")){ 
					cal(par[i],sample[k],tag[j],cut_final,th2[i],"ID");
				}
				if(par[i].Contains("ele")){ 
					cal(par[i],sample[k],tag[j],cut_final,th2[i],"ID");
					cal(par[i],sample[k],tag[j],cut_final,th2[i],"reco");
				}
			}
		}
	}
