...
void run(TString var, TString recovar,TString title,TString tag){
	...
	double max_r=0,max_e=0;
	double min_r=0,min_e=0;
...
//              cout<<Err_down[i]<<" "<<Err_up[i]<<endl;
                ...
                if(ybin[i]>max_r) max_r=ybin[i];
                if(yerror_up[i]>max_e) max_e=yerror_up[i];
                if(ybin[i]<min_r) min_r=ybin[i];
                if(yerror_down[i]<min_e) min_e=yerror_up[i];
	}
...
//        if(var.Contains("Mjj")==1) {for(int i=0;i<h1[0]->GetNbinsX();i++){gr->GetXaxis()->SetBinLabel(i+1,name[i]);}}
	...
	if(var.Contains("Mjj")==1) gr->GetXaxis()->SetTitle(title+"-#Delta#eta_{jj}");
        gr->GetYaxis()->SetRangeUser(min_r-max_e,(max_r+max_e)*2);
	gr->SetMarkerColor(1);
	...
	gr->GetXaxis()->SetLabelSize(0.05);
        ...
        gr_sys->Draw("P SAME");
        TLegend*l1=new TLegend(0.6,0.65,0.88,0.88);
        l1->AddEntry(gr," stat. #oplus syst.");
        l1->AddEntry(gr_sys,"systematic only");
        ...
        TLine*line=new TLine(h1[0]->GetXaxis()->GetXmin(),1.,h1[0]->GetXaxis()->GetXmax(),1.);
        line->SetLineColor(2);
        line->SetLineWidth(1);
        line->SetLineStyle(2);
        line->Draw();
