...
//   if(name.Contains("Muon")||name.Contains("Ele")) h1->SetTitle("data_obs"); 
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if( !(
		      ...
		      (lep == 13 && (HLT_Mu2 >0 || HLT_Mu1 >0 || HLT_Mu3>0) && ptlep1 > 20. && ptlep2 > 20. && abs(etalep1) < 2.4 && abs(etalep2) < 2.4 && nlooseeles == 0 && nloosemus < 3 && massVlep > 70. && massVlep < 110. && jet1pt>30. && jet2pt>30.&& abs(jet1eta)< 4.7 && abs(jet2eta)<4.7 && Mjj>500. &&detajj>2.5 && photonet>100.&&(abs(photoneta)<1.4442||(abs(photoneta)>1.566&&abs(photoneta)<2.5)) ) 
	   ))
            continue;
      ...
      if(name.Contains("plj")==0 && (name.Contains("Muon")==0 && name.Contains("Ele")==0) )
          actualWeight=actualWeight*photon_veto_scale;
