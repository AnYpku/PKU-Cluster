...
//		file_ID->Close();
	...
	TFile*fout=new TFile("./"+particle+"_"+type+tag+"CR.root","recreate");
	fout->cd();
	for(int i=0;i<3;i++){
		th1[i]->Write();
	}
...
int cal(){
	...
	TString GenSignalRegion = "(genMjj<500 && genMjj>150 && gendetajj>2.5)";
	...
	for(int j=0;j<tag.size();j++){
		...
		TString SignalRegion = "(Mjj<500 && Mjj>150 && detajj>2.5 && ZGmass>100)";
