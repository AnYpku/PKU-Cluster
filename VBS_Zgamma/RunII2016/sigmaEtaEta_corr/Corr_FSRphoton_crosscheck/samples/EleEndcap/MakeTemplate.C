...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TFile* file = new TFile("trans_"+name+".root","RECREATE");
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	  ...
	  double tm1=(leading_electron+subleading_electron+photon).M();
          Photon_cut[j]= photon_pevnew[j]==1 &&
                         ...
                         (fabs(photon_eta[j])<2.5&&fabs(photon_eta[j])>1.566) && photon_pt[j]>20 &&
    ...
    if(name.Contains("Jets"))scalef = 0.070591*fabs(theWeight)/theWeight * prefWeight;
    else scalef = 1;
    h1->Fill(photon_sieie[position],scalef);
    vector_pt.clear();
   }
   fout->cd();
   ExTree->Write();
