...
int unroll_2d(){
	...
	for(int ik=0;ik<1;ik++){
		if(var[ik].Contains("gen"))sample={"ZA-EWK"};
		else  sample={"ZA-EWKout","ZA","ST","TTA","VV"};
		for(int kk=0;kk<sample.size();kk++){
			for(int i=0;i<tag.size();i++){
				for(int j=0;j<6;j++){
					file[j][i]=new TFile("unfold_"+sample[kk]+"_2d"+var[ik]+Form("_recobin%i_jer",j+1)+tag[i]+".root");
					fout[j][i]=new TFile("unfold_"+sample[kk]+"_"+var[ik]+Form("_recobin%i_jer",j+1)+tag[i]+".root","recreate");
				...
				TFile*f1=new TFile("unfold_"+sample[kk]+"_2d"+var[ik]+"_recobin7_jer"+tag[i]+".root");
				TFile*f2=new TFile("unfold_"+sample[kk]+"_2d"+var[ik]+"_recobin8_jer"+tag[i]+".root");
				TFile*f3=new TFile("unfold_"+sample[kk]+"_2d"+var[ik]+"_recobin9_jer"+tag[i]+".root");

				fout[6][i]=new TFile("unfold_"+sample[kk]+"_"+var[ik]+"_recobin7_jer"+tag[i]+".root","recreate");
				for(int k=0;k<num;k++){
