TH1D* run(TString cut,TString file,TString channel,TString isBarrel,TString year){
	...
	TFile*f1=new TFile("/home/pku/anying/cms/rootfiles/WWg/20"+y+"/cutlep-"+file+".root");
	TTree*tree=(TTree*)f1->Get("Events");
        TString histname=file+"_"+channel+"_"+isBarrel;
        TString BSL,HLT;
        BSL="( channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && lep1_is_tight==1 && lep2_is_tight==1 && lep1_charge*lep2_charge<0 && drll>0.5 && lep1pt>20 && lep2pt>25 && fabs(lep1eta) < 2.4 && fabs(lep2eta) < 2.5 && n_loose_ele==1 && n_loose_mu==1 && ptll>30 && mll>20 )";
        if(file.Contains("MuonEG")) HLT = "(HLT_emu)";
	else if(file.Contains("Muon")) HLT = "( !(HLT_emu) && HLT_mm )";
	else if(file.Contains("Ele")) HLT = "( !(HLT_emu) && !(HLT_mm) && HLT_ee )";
	else HLT = "( HLT_emu || HLT_ee || HLT_mm  )";
        TH1D*h1 = new TH1D(histname,"",ptbins.size()-1,&ptbins[0]);
	TString cut1 = "("+ BSL + "&&" + HLT +")";
        if(isBarrel.Contains("barrel")) cut="("+ cut + "&&" + cut1+"&& (fabs(photoneta)<1.4442)"+")";
        else cut="(" + cut +"&&" + cut1 + "&& (fabs(photoneta)<2.5&&fabs(photoneta)>1.566)"+")";
...
int cal_weight(){
	...
	for(int ik=2;ik<year.size();ik++){
		...
		for(int k=0;k<isBarrel.size();k++){
			...
			TH1D*h1=run(cut1,"outMuonEG"+year[ik],channel[0],isBarrel[k],year[ik]);
			TH1D*h2=run(cut2,"outMuonEG"+year[ik],channel[0],isBarrel[k],year[ik]);
			TH1D*hh1=run(cut1,"outMuon"+year[ik],channel[0],isBarrel[k],year[ik]);
			TH1D*hh2=run(cut2,"outMuon"+year[ik],channel[0],isBarrel[k],year[ik]);
			TH1D*hhh1=run(cut1,"outEle"+year[ik],channel[0],isBarrel[k],year[ik]);
			TH1D*hhh2=run(cut2,"outEle"+year[ik],channel[0],isBarrel[k],year[ik]);
			TH1D*h3=run(cut3,"outVV"+year[ik],channel[0],isBarrel[k],year[ik]);
			TH1D*h4=run(cut3,"outTTGJets"+year[ik],channel[0],isBarrel[k],year[ik]);
