...
fdir = '/afs/cern.ch/user/y/yian/work/PKU-Cluster/SignalStrength/batch/CR/root/'
...
f_WA = TFile.Open(fdir+'hist_WA_'+sys.argv[1]+"CR_"+sys.argv[2]+'.root')
...
genbinerror=[]
arr={}
f=open('/afs/cern.ch/user/y/yian/work/PKU-Cluster/SignalStrength/batch/CR/Uncer/1d_JESR/summary_uncer_'+sys.argv[2]+sys.argv[1]+'CR.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
# bin error
   ...
   f.write('WA_Stat_control_bin%d_%s%s\tlnN\t'%(i,sys.argv[2],sys.argv[1]))
   f.write('-\t-\t-\t-\t-\t-\t%0.2f\t-\n'%(WA_binerror))
