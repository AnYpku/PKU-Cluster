...
TH1D* unroll(TH2D* th2_in,Double_t* xbin, Double_t* ybin,  Int_t xbins_in, Int_t ybins_in,char *hname)//,char* htitle)
{
	...
	for(Int_t iy=1; iy<=nbinsy; iy++){
		for(Int_t ix=1; ix<=nbinsx; ix++){
			...
			cout<<"ix = "<<ix<<", iy = "<<iy<<"; bin = "<<ix+(iy-1)*nbinsx<<", BinContent"<<58.7*th2->GetBinContent(th2->FindBin(x_temp, y_temp))<<endl;
		}
	}
//    delete h1;
//    return h1_out;
...
int unroll(){
   /* ifstream file1;
    ...
    for(Int_t i=0;i<num;i++){
        ...
        ofstream file2(Form("./uncer-txt/content-hist_no%d",i+1));
...
//        file2<<t_ZA[i]->GetBinContent(k+1)*scale_factor[i]<<endl;}
     ...
     TLegend *l3 = new TLegend(0.5,0.7,0.7,0.9);
     t_ZA[0]->SetTitle("Mjj vs detajj");
     t_ZA[0]->SetLineWidth(3);
     t_ZA[0]->SetLineColor(kRed);
     t_ZA[0]->GetYaxis()->SetRangeUser(t_ZA[0]->GetMaximum()*0.8,t_ZA[0]->GetMaximum()*1.8);
     t_ZA[0]->Draw("HIST");
...
//          t_ZA[i]->Scale(scale_factor[i]);
         ...
         else if(i==5)   l3->AddEntry(t_ZA[i],"scale7 ");
         else if(i==6)   l3->AddEntry(t_ZA[i],"scale9 ");
         // delete t_ZA[i];
         // delete cc[i];
       }
       cmsLumi(0);
       ...
       line->SetLineWidth(1);
       line->Draw();
       c1->SaveAs("./figs/hist-2d.eps");
       c1->SaveAs("./figs/hist-2d.pdf");
