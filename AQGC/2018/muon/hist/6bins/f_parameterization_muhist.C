...
//		h1[abin]=new TH1D(name+Form("_bin%i",abin+1),"yields with every "+name+" value",16,xxf);
		...
		for(int count=0; count < numberOfEntries; count++){
			...
			Double_t weight=lumiWeight * pileupWeight * scalef*muon1_id_scale*muon2_id_scale*muon1_iso_scale*muon2_iso_scale*photon_id_scale;//*photon_veto_scale*prefWeight;
