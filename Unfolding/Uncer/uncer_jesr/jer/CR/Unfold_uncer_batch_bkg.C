     ...
     for(int i=0;i<recovars.size();i++){
	     for(int j=0;j<sample.size();j++){
		     ...
		     if(sample[j].Contains("EWK")){
			     run(dir,"16outJEC_ZA-EWK",recovars[i], bins[i],cut,3,"16");
			     run(dir,"17outJEC_ZA-EWK",recovars[i], bins[i],cut,3,"17");
			     run(dir,"18outJEC_ZA-EWK",recovars[i], bins[i],cut,3,"18");
		     }
		     else{
			     run(dir1,sample[j],recovars[i], bins[i],Reco,3,"16");
			     run(dir2,sample[j],recovars[i], bins[i],Reco,3,"17");
			     run(dir3,sample[j],recovars[i], bins[i],Reco,3,"18");
		     }
	     }
     }
     return 1;
