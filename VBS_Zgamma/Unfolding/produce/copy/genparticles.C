...
void rm::genparticles(){
	...
	if(*biggest_muon1>0){
		...
		for(int iphoton=0;iphoton<6;iphoton++){
			...
			if( lep1p4.DeltaR(photon_temp)<0.1) {lep1p4+=photon_temp;continue;}
		}
		genlep1pt  = lep1p4.Pt();
	...
	if(*biggest_muon2>0){
		...
		for(int iphoton=0;iphoton<6;iphoton++){
			TLorentzVector photon_temp;
			photon_temp.SetPtEtaPhiM(genphoton_pt[iphoton],genphoton_eta[iphoton],genphoton_phi[iphoton],0);
			if( lep2p4.DeltaR(photon_temp)<0.1) {lep2p4+=photon_temp;continue;}
		}
		genlep2pt  = lep2p4.Pt();
	...
	if(*biggest_ele1>0 && fabs(genmuon_pid[ele1_index]!=13)){
		...
		lep1p4.SetPtEtaPhiM(genlep1pt, genlep1eta, genlep1phi, 0.00051);
		for(int iphoton=0;iphoton<6;iphoton++){
			TLorentzVector photon_temp;
			photon_temp.SetPtEtaPhiM(genphoton_pt[iphoton],genphoton_eta[iphoton],genphoton_phi[iphoton],0);
			if( lep1p4.DeltaR(photon_temp)<0.1) {lep1p4+=photon_temp;continue;}
	...
	if(*biggest_ele2>0 && fabs(genmuon_pid[ele2_index]!=13)){ 
		...
		lep2p4.SetPtEtaPhiM(genlep2pt, genlep2eta, genlep2phi, 0.00051);
		for(int iphoton=0;iphoton<6;iphoton++){
			TLorentzVector photon_temp;
			photon_temp.SetPtEtaPhiM(genphoton_pt[iphoton],genphoton_eta[iphoton],genphoton_phi[iphoton],0);
			if( lep2p4.DeltaR(photon_temp)<0.1) {lep2p4+=photon_temp;continue;}
	...
	//pho1_index 
	for(int i=0;i<6;i++){
		if(vector_photon[i]>0){
			double drla1_tmp=delta_R(genphoton_eta[i],genphoton_phi[i],genlep1eta,genlep1phi);
			double drla2_tmp=delta_R(genphoton_eta[i],genphoton_phi[i],genlep2eta,genlep2phi);
			if(drla1_tmp<0.7 || drla2_tmp<0.7) vector_photon[i]=0;
		}
	}
