...
TH2D* hist_2d_mc(TString filename){
	...
	TString file = "/afs/cern.ch/user/y/yian/work/PKU-Cluster/RunII2017/aa2/ScalSeq/output-slimmed-rootfiles/optimal_2017_"+filename+".root";
...
//	TH2D* th2 = new TH2D("th2","th2",4, mjj_bins, 4, mlla_bins);
	...
	for(Int_t i=0; i<t->GetEntries();i++)
	{
		t->GetEntry(i);
                if(Mjj>400) continue;
//                if (! ((fabs(photoneta)>1.566&&fabs(photoneta)<2.5) ) ) continue;
                if (! (fabs(photoneta)<1.4442)  ) continue;
