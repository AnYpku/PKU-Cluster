...
class ZPKUTreeMaker : public edm::EDAnalyzer {
	...
	private:
		...
		int matchToTruth(const reco::Photon &pho,const edm::Handle<edm::View<reco::GenParticle>>  &genParticles, bool &ISRPho, double &dR, int &isprompt);
                int getjetindex1(std::vector<TLorentzVector*> jets, int iphoton,double photoneta,double photonphi);
		int getjetindex2(std::vector<TLorentzVector*> jets, int iphoton,double photoneta,double photonphi);
...
//                edm::EDGetTokenT< double > prefweightdown_token;
		...
		double jet1pt_new, jet1pt_JEC_up, jet1pt_JEC_down, jet1pt_JER_up, jet1pt_JER_down;
                double jet1pt_val,jet1e_val,jet1eta_val,jet1phi_val;
                double jet2pt_val,jet2e_val,jet2eta_val,jet2phi_val;
		...
		std::vector<double> jet1pt_unc_up,   jet1pt_unc_down;
		std::vector<double> jet1eta_unc_up,  jet1eta_unc_down;
		std::vector<double> jet1phi_unc_up,  jet1phi_unc_down;
		std::vector<double> jet1e_unc_up,    jet1e_unc_down;
		std::vector<double> jet2pt_unc_up,   jet2pt_unc_down;
		std::vector<double> jet2eta_unc_up,  jet2eta_unc_down;
		std::vector<double> jet2phi_unc_up,  jet2phi_unc_down;
		std::vector<double> jet2e_unc_up,    jet2e_unc_down;
...
//	outTree_->Branch("passFilter_duplicateMuon"		,&passFilter_duplicateMuon_	,"passFilter_duplicateMuon_/O");
	...
	outTree_->Branch("lep2_sign"      ,&lep2_sign     ,"lep2_sign/I"     );
        ...
        outTree_->Branch("jet1pt_unc_up",  &jet1pt_unc_up   );
        outTree_->Branch("jet1eta_unc_up", &jet1eta_unc_up   );
        outTree_->Branch("jet1phi_unc_up", &jet1phi_unc_up   );
        outTree_->Branch("jet1e_unc_up",   &jet1e_unc_up   );
        outTree_->Branch("jet2pt_unc_up",  &jet2pt_unc_up   );
        outTree_->Branch("jet2eta_unc_up", &jet2eta_unc_up   );
        outTree_->Branch("jet2phi_unc_up", &jet2phi_unc_up   );
        outTree_->Branch("jet2e_unc_up",   &jet2e_unc_up   );

        outTree_->Branch("jet1pt_unc_down",  &jet1pt_unc_down   );
        outTree_->Branch("jet1eta_unc_down", &jet1eta_unc_down   );
        outTree_->Branch("jet1phi_unc_down", &jet1phi_unc_down   );
        outTree_->Branch("jet1e_unc_down",   &jet1e_unc_down   );
        outTree_->Branch("jet2pt_unc_down",  &jet2pt_unc_down   );
        outTree_->Branch("jet2eta_unc_down", &jet2eta_unc_down   );
        outTree_->Branch("jet2phi_unc_down", &jet2phi_unc_down   );
        outTree_->Branch("jet2e_unc_down",   &jet2e_unc_down   );
	outTree_->Branch("jet1pt_val"          ,&jet1pt_val         ,"jet1pt_val/D"         );
	outTree_->Branch("jet1e_val"          ,&jet1e_val         ,"jet1e_val/D"         );
	outTree_->Branch("jet1eta_val"          ,&jet1eta_val         ,"jet1eta_val/D"         );
	outTree_->Branch("jet1phi_val"          ,&jet1phi_val         ,"jet1phi_val/D"         );
	outTree_->Branch("jet2pt_val"          ,&jet2pt_val         ,"jet2pt_val/D"         );
	outTree_->Branch("jet2e_val"           ,&jet2e_val          ,"jet2e_val/D"         );
	outTree_->Branch("jet2eta_val"         ,&jet2eta_val        ,"jet2eta_val/D"         );
	outTree_->Branch("jet2phi_val"         ,&jet2phi_val        ,"jet2phi_val/D"         );
}
...
int ZPKUTreeMaker::getjetindex1(std::vector<TLorentzVector*> jets, int iphoton,double photoneta,double photonphi){
        ...
        int index1=jetindexphoton12[0];
        return index1;
}
int ZPKUTreeMaker::getjetindex2(std::vector<TLorentzVector*> jets, int iphoton,double photoneta,double photonphi){
        Int_t jetindexphoton12[2] = {-1,-1};
        for (size_t i=0;i<jets.size();i++) {
                if(iphoton>-1) {
                        double drtmp1=deltaR(jets.at(i)->Eta(), jets.at(i)->Phi(), photoneta,photonphi);
...
/*	int mtemp7=0;
	...
	edm::Handle<edm::View<reco::Candidate> > leptonicVs;
        ...
        if (leptonicVs->empty()) {  outTree_->Fill();
		jet1pt_unc_up.clear();jet1e_unc_up.clear();jet1eta_unc_up.clear();jet1phi_unc_up.clear();
		jet1pt_unc_down.clear();jet1e_unc_down.clear();jet1eta_unc_down.clear();jet1phi_unc_down.clear();
		return;  }
	...
	if (vertices->empty()) { outTree_->Fill();
		jet1pt_unc_up.clear();jet1e_unc_up.clear();jet1eta_unc_up.clear();jet1phi_unc_up.clear();
		jet1pt_unc_down.clear();jet1e_unc_down.clear();jet1eta_unc_down.clear();jet1phi_unc_down.clear();
	...
	if ( firstGoodVertex==vertices->end() ) {outTree_->Fill();
		jet1pt_unc_up.clear();jet1e_unc_up.clear();jet1eta_unc_up.clear();jet1phi_unc_up.clear();
		jet1pt_unc_down.clear();jet1e_unc_down.clear();jet1eta_unc_down.clear();jet1phi_unc_down.clear();
...
//                double phIso1 = (*photons)[ip].photonIso();//(*phoPhotonIsolationMap)[pho];
		...
		if(passEleVetonew && (*photons)[ip].isEB() && (*photons)[ip].hadTowOverEm()<0.02197 && photon_sieie[ip]<0.01015 && chiso<1.141 && nhiso<(1.189 + (0.01512*(*photons)[ip].pt()+0.00002259*(*photons)[ip].pt()*(*photons)[ip].pt())) && phoiso<(2.08+0.004017*(*photons)[ip].pt())) {ismedium_photon=1;}
		if(passEleVetonew && (*photons)[ip].isEE() && (*photons)[ip].hadTowOverEm()<0.0326 && photon_sieie[ip]<0.0272 && chiso<1.051 && nhiso<(2.718 + (0.0117*(*photons)[ip].pt()+0.000023*(*photons)[ip].pt()*(*photons)[ip].pt())) && phoiso<(3.867+0.0037*(*photons)[ip].pt())) {ismedium_photon=1;}
	...
	// ***********************************************************//
	Int_t jetindexphoton12[2] = {-1,-1}; 
	Int_t jetindexphoton12_val[2] = {-1,-1}; 
	...
	//jet validation
	std::vector<TLorentzVector*> jets_val;
	for (size_t ik=0; ik<ak4jets->size();ik++)
	{
		reco::Candidate::LorentzVector uncorrJet = (*ak4jets)[ik].correctedP4(0);
		if((*ak4jets)[ik].userFloat("jetValidation_Ptup")>tmpjetptcut) {
			TLorentzVector *dummy = new TLorentzVector(0,0,0,0);    
			dummy->SetPtEtaPhiE((*ak4jets)[ik].userFloat("jetValidation_Ptup"), uncorrJet.eta(), uncorrJet.phi(), (*ak4jets)[ik].userFloat("jetValidation_Eup"));
			jets_val.push_back(dummy);
		}   
	}

        const size_t num=jecAK4chs_source_.size();
        Int_t jetindex_unc_up[num][2];
        Int_t jetindex_unc_down[num][2];
        for(size_t i=0;i<jecAK4chs_source_.size();i++){
		for(int j=0;j<2;j++){
			jetindex_unc_up[i][j]=-1;
			jetindex_unc_down[i][j]=-1;
		}
	}
	for(size_t i=0;i<jecAK4chs_source_.size();i++){
		std::vector<TLorentzVector*> jets_unc_down;
		for (size_t ik=0; ik<ak4jets->size();ik++)
		{
			reco::Candidate::LorentzVector uncorrJet = (*ak4jets)[ik].correctedP4(0);
			if((*ak4jets)[ik].userFloat(jecAK4chs_source_[i]+"_Ptdown")>tmpjetptcut) {
				TLorentzVector *dummy = new TLorentzVector(0,0,0,0);
				dummy->SetPtEtaPhiE((*ak4jets)[ik].userFloat(jecAK4chs_source_[i]+"_Ptdown"), uncorrJet.eta(), uncorrJet.phi(), (*ak4jets)[ik].userFloat(jecAK4chs_source_[i]+"_Edown"));
				jets_unc_down.push_back(dummy);
			}
		}
		std::vector<TLorentzVector*> jets_unc_up;
		for (size_t ik=0; ik<ak4jets->size();ik++)
		{
			reco::Candidate::LorentzVector uncorrJet = (*ak4jets)[ik].correctedP4(0);
			if((*ak4jets)[ik].userFloat(jecAK4chs_source_[i]+"_Ptup")>tmpjetptcut) {
				TLorentzVector *dummy = new TLorentzVector(0,0,0,0);
				dummy->SetPtEtaPhiE((*ak4jets)[ik].userFloat(jecAK4chs_source_[i]+"_Ptup"), uncorrJet.eta(), uncorrJet.phi(), (*ak4jets)[ik].userFloat(jecAK4chs_source_[i]+"_Eup"));
				jets_unc_up.push_back(dummy);
			}
		}
             sort (jets_unc_up.begin (), jets_unc_up.end (), ZmysortPt);         
             sort (jets_unc_down.begin (), jets_unc_down.end (), ZmysortPt);         
             jetindex_unc_up[i][0]=getjetindex1(jets_unc_up,iphoton,photoneta,photonphi);
             jetindex_unc_up[i][1]=getjetindex2(jets_unc_up,iphoton,photoneta,photonphi);
             jetindex_unc_down[i][0]=getjetindex1(jets_unc_down,iphoton,photoneta,photonphi);
             jetindex_unc_down[i][1]=getjetindex2(jets_unc_down,iphoton,photoneta,photonphi);
	     if(jetindex_unc_up[i][0]>-1 && jetindex_unc_up[i][1]>-1) {

		     jet1pt_unc_up[i]=(jets_unc_up[jetindex_unc_up[i][0]]->Pt());
		     jet1eta_unc_up[i]=(jets_unc_up[jetindex_unc_up[i][0]]->Eta());
		     jet1phi_unc_up[i]=(jets_unc_up[jetindex_unc_up[i][0]]->Phi());
		     jet1e_unc_up[i]=(jets_unc_up[jetindex_unc_up[i][0]]->E());

		     jet2pt_unc_up[i]=(jets_unc_up[jetindex_unc_up[i][1]]->Pt());
		     jet2eta_unc_up[i]=(jets_unc_up[jetindex_unc_up[i][1]]->Eta());
		     jet2phi_unc_up[i]=(jets_unc_up[jetindex_unc_up[i][1]]->Phi());
		     jet2e_unc_up[i]=(jets_unc_up[jetindex_unc_up[i][1]]->E());
	     }else{
		     jet1pt_unc_up[i]=(-10);
		     jet1eta_unc_up[i]=(-10);
		     jet1phi_unc_up[i]=(-10);
             ...
             if(jetindex_unc_down[i][0]>-1 && jetindex_unc_down[i][1]>-1) {
		     jet1pt_unc_down[i]=(jets_unc_down[jetindex_unc_down[i][0]]->Pt());
		     jet1eta_unc_down[i]=(jets_unc_down[jetindex_unc_down[i][0]]->Eta());
		     jet1phi_unc_down[i]=(jets_unc_down[jetindex_unc_down[i][0]]->Phi());
		     jet1e_unc_down[i]=(jets_unc_down[jetindex_unc_down[i][0]]->E());

		     jet2pt_unc_down[i]=(jets_unc_down[jetindex_unc_down[i][1]]->Pt());
		     jet2eta_unc_down[i]=(jets_unc_down[jetindex_unc_down[i][1]]->Eta());
		     jet2phi_unc_down[i]=(jets_unc_down[jetindex_unc_down[i][1]]->Phi());
		     jet2e_unc_down[i]=(jets_unc_down[jetindex_unc_down[i][1]]->E());
	     }else{
		     jet1pt_unc_down[i]=(-10);
		     jet1eta_unc_down[i]=(-10);
		     jet1phi_unc_down[i]=(-10);
		     jet1e_unc_down[i]=(-10);

		     jet2pt_unc_down[i]=(-10);
		     jet2eta_unc_down[i]=(-10);
		     jet2phi_unc_down[i]=(-10);
		     jet2e_unc_down[i]=(-10);
	     }
	     jets_unc_up.clear();jets_unc_down.clear();
	}
	sort (jets.begin (), jets.end (), ZmysortPt);
	sort (jets_val.begin (), jets_val.end (), ZmysortPt);
	...
	sort (jets_JER_down.begin (), jets_JER_down.end (), ZmysortPt);
        jetindexphoton12_val[0]=getjetindex1(jets_val,iphoton,photoneta,photonphi);
        jetindexphoton12_val[1]=getjetindex2(jets_val,iphoton,photoneta,photonphi);
	if(jetindexphoton12_val[0]>-1 && jetindexphoton12_val[1]>-1) {
            jet1pt_val=jets_val[jetindexphoton12_val[0]]->Pt();
            jet1e_val=jets_val[jetindexphoton12_val[0]]->E();
            jet1eta_val=jets_val[jetindexphoton12_val[0]]->Eta();
            jet1phi_val=jets_val[jetindexphoton12_val[0]]->Phi();
            jet2pt_val=jets_val[jetindexphoton12_val[1]]->Pt();
            jet2e_val=jets_val[jetindexphoton12_val[1]]->E();
            jet2eta_val=jets_val[jetindexphoton12_val[1]]->Eta();
            jet2phi_val=jets_val[jetindexphoton12_val[1]]->Phi();
	}
	//two leading jets 
	for (size_t i=0;i<jets.size();i++) {
		if(iphoton>-1) {
...
void ZPKUTreeMaker::setDummyValues() {
	...
	isTrue_=-1;
	isprompt_=-1; 
	jet1pt_new=-1e1;     jet1pt=-1e1;
	jet1eta_new=-1e1;    jet1eta=-1e1;
	jet1phi_new=-1e1;    jet1phi=-1e1;
	jet1e_new=-1e1;      jet1e=-1e1;
	jet2pt_new=-1e1;     jet2pt=-1e1;
	jet2eta_new=-1e1;    jet2eta=-1e1;
	jet2phi_new=-1e1;    jet2phi=-1e1;
	jet2e_new=-1e1;      jet2e=-1e1;
	...
	zepp_JEC_up=-1e1; zepp_JEC_down=-1e1; zepp_JER_up=-1e1; zepp_JER_down=-1e1;
        jet1pt_val=-10;jet1e_val=-10;jet1eta_val=-10;jet1phi_val=-10;
        jet2pt_val=-10;jet2e_val=-10;jet2eta_val=-10;jet2phi_val=-10;
        for(size_t i=0;i<jecAK4chs_source_.size();i++){
		jet1pt_unc_up.push_back(-10); 
		jet1eta_unc_up.push_back(-10); 
		jet1phi_unc_up.push_back(-10); 
		jet1e_unc_up.push_back(-10); 
		jet2pt_unc_up.push_back(-10); 
		jet2eta_unc_up.push_back(-10); 
		jet2phi_unc_up.push_back(-10); 
		jet2e_unc_up.push_back(-10); 
		jet1pt_unc_down.push_back(-10); 
		jet1eta_unc_down.push_back(-10); 
		jet1phi_unc_down.push_back(-10); 
		jet1e_unc_down.push_back(-10); 
		jet2pt_unc_down.push_back(-10); 
		jet2eta_unc_down.push_back(-10); 
		jet2phi_unc_down.push_back(-10); 
		jet2e_unc_down.push_back(-10); 
	}
