...
void EDBRHistoMaker::Init(TTree *tree) {
	...
	treename->Branch("MET_pt", &MET_pt, "MET_pt/F");
	treename->Branch("mT", &mT, "mT/F");
...
//      nentries=100;
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		//data
		actualWeight=1;
                lep1_isprompt=1;lep2_isprompt=1;photon_isprompt=1;
                MET_pt=PuppiMET_T1_pt;
		...
		if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 /*&& lep1_is_tight==1 && lep2_is_tight==1*/ && mll > 20 && ptll > 30 /*&& n_photon>0  && photonet > 20. && drl1a>0.5 && drl2a>0.5 && photon_channel && photon_selection==1 */ && PuppiMET_T1_pt > 20 /*&& mT<60 && mT2 > 30 && n_bjets==0*/ ){
...
//	nentries=100;
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && mll > 20 && ptll > 30 && lepton_flag /*&& n_photon>0  && photonet > 20. && drl1a>0.5 && drl2a>0.5 && photon_channel && photon_flag==1*/ && PuppiMET_T1Smear_pt > 20 /*&& mT<60 && mT2>30 && n_bjets==0*/ ){
			if(gen_weight>0) npp++;
			if(gen_weight<0) nmm++;
			numbe_out++;
