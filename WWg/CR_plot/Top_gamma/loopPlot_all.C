...
void loopPlot(int isBarrel,TString isChannel) {
	...
	vector<TString> pathToTrees = {"/home/pku/anying/cms/rootfiles/WWg/2016/","/home/pku/anying/cms/rootfiles/WWg/2017/","/home/pku/anying/cms/rootfiles/WWg/2018/"};
...
// Setup names of data files for trees.
	...
	for (int ii = 0; ii < nDATA; ii++) {
		for (int j = 0; j< pathToTrees.size(); j++) {
			if(j==0){
				...
				dataLabels.push_back(data[ii]);
				fData.push_back(pathToTrees[j] +"cutla-out"+ data[ii] + "16.root");
				fData.push_back(pathToTrees[j] +"cutla-out"+ data[ii] + "16pre.root");
			}
			else if(j==1){ 
				dataLabels.push_back(data[ii]);
				fData.push_back(pathToTrees[j] +"cutla-out"+ data[ii] + "17.root");
			}
			else if(j==2){ 
				dataLabels.push_back(data[ii]);
	...
	std::cout << "set data imformation, we have " << nMC << "mc file"
		<< std::endl;
        std::string mc[nMC] ={"plj","fakeL","VV","ST","ZGJets","TTGJets","TTJets","WGJets","tZq"};
	...
	for (int ii = 0; ii < nMC; ii++) {
		for (int j = 0; j< pathToTrees.size(); j++) {
			if(j==0){
				...
				mcLabels.push_back(mc[ii]);
				fMC.push_back(pathToTrees[j] +"cutla-out"+ mc[ii] + "16.root");
				fMC.push_back(pathToTrees[j] +"cutla-out"+ mc[ii] + "16pre.root");
			}
			if(j==1){
				mcLabels.push_back(mc[ii]);
				fMC.push_back(pathToTrees[j] +"cutla-out"+ mc[ii] + "17.root");
			}
			if(j==2){
				mcLabels.push_back(mc[ii]);
				fMC.push_back(pathToTrees[j] +"cutla-out"+ mc[ii] + "18.root");
			}
			cout<<pathToTrees[j]<<"cutla-out"<<mc[ii]<<".root"<<endl;
		}
	}
	const int nmc=nMC;
	std::string bkgLabels[nmc] ={"plj","fakeL","VV","tW","QCD Z#gamma","t#bar{t}#gamma","t#bar{t}(FSR #gamma)","QCD W#gamma","tZq"};
	...
	for (int ii = 0; ii < nMCSig; ii++) {
		for (int j = 0; j< pathToTrees.size(); j++) {
			if(j==0){ 
				...
				mcLabelsSig.push_back(mcSig[ii]);
				fMCSig.push_back(pathToTrees[j] + "cutla-out" + mcSig[ii] +"16.root");
				fMCSig.push_back(pathToTrees[j] + "cutla-out" + mcSig[ii] +"16pre.root");
			}
			else if(j==1){
				mcLabelsSig.push_back(mcSig[ii]);
				fMCSig.push_back(pathToTrees[j] + "cutla-out" + mcSig[ii] + "17.root");
			}
			else if(j==2){
				mcLabelsSig.push_back(mcSig[ii]);
				fMCSig.push_back(pathToTrees[j] + "cutla-out" + mcSig[ii] + "18.root");
			}
		}
	}
