...
sed -i '/theory/d' tmp
sed -i '/ST_Stat_bin1_eleendcap/d' tmp
echo "$theory"
sed -i ':label;N;s/\n/,/;t label' tmp
#sed -i "$ a $theory" tmp
...
combineTool.py -M Impacts -d ${name}.root -m 125 --robustFit 1 --doFits --parallel 8
combineTool.py -M Impacts -d ${name}.root -m 125 -o impacts_QCD_${name}.json
plotImpacts.py -i impacts_QCD_${name}.json -o impacts_QCD_${name}
