...
int unroll_2d(){
	...
	for(int i=0;i<tag.size();i++){
		for(int j=0;j<sample.size();j++){
			...
			TFile*fout=new TFile("hist_"+sample[j]+"_unroll_"+tag[i]+".root","recreate");
			if(sample[j].Contains("EWK")){
				th2_7=merge_hist(file,"hist2d_6","hist2d_7","hist2d_8");
				hist_7=unroll_hist(th2_7,"hist2d_7");
				for(int k=0;k<6;k++){
					th2[k]=(TH2D*)file->Get(Form("hist2d_%i",k));
					hist[k]=unroll_hist(th2[k],Form("hist2d_%i",k+1));
				}
			}
			else{
				h2=(TH2D*)file->Get("hist_2d"+tag[i]);
				h1=unroll_hist(h2,"hist_bkg");
				fout->cd();
				h1->Write();
			}
			if(sample[j].Contains("EWK")){
