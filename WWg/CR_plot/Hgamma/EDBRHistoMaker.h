...
void EDBRHistoMaker::Init(TTree *tree) {
	...
	treename->Branch("ptlep1", &ptlep1,     "ptlep1/F");
	treename->Branch("dphill", &dphill,     "dphill/F");
	treename->Branch("ptlep2", &ptlep2,     "ptlep2/F");
...
void EDBRHistoMaker::createAllHistos(TString isChannel) {
	...
	hs.setHisto("mT", 10, 40, 200);
	hs.setHisto("mT2", 20, 20, 200);
	...
	hs.setHisto("njets50", 14,0, 14);
        ...
        hs.setHisto("drll", 8,0.5, 2.5);
	hs.setHisto("ptlep1", 20, 25, 180);
	hs.setHisto("ptlep2", 20, 15, 180);
	hs.setHisto("dphill", 8, 0, 2.5);
	char buffer[256];
	char buffer2[256];
...
void EDBRHistoMaker::Loop(std::string outFileName,float luminosity,int isBarrel,TString isChannel) {
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if(isChannel=="emu") 
			lepton_channel= ((HLT_emu1||HLT_emu2||HLT_emu3||HLT_emu4) && channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && ptlep1>25 && ptlep2>15 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.5) && lep1_charge*lep2_charge<0 && drll>0.5;
		...
		if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && lep1_is_tight==1 && lep2_is_tight==1 && mll >20 && ptll > 30 && n_photon>0  && photonet > 20. && drl1a>0.5 && drl2a>0.5 && photon_channel && photon_selection==1 && PuppiMET_T1_pt > 20 && n_bjets_nom==0 && njets30<=1 && mT>40 && mT2>20 && drl1a>0.8 && drl2a>0.8 && drll<2.3 && dphill<2.3 ){
...
void EDBRHistoMaker::Loop_SFs_mc(std::string outFileName,float luminosity,int isBarrel,TString isChannel){
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		if(filename.Contains("HGC") ) scalef=scalef*(50*50);
		if(filename.Contains("HGS") ) scalef=scalef*(900*900);
		if(filename.Contains("HGU") ) scalef=scalef*(10000*10000);
		if(filename.Contains("HGD") ) scalef=scalef*(10000*10000);
		...
		if(filename.Contains("DY")||filename.Contains("TTJets")||filename.Contains("TTWJets")||filename.Contains("WJets")){
			...
			if(photon_selection==1)
				photon_flag=1;
			if( ! (lep1_is_tight==1 && lep2_is_tight==1) && njets>0 )
				lepton_flag=1;
		}
		else{//MC
		...
		bool photon_channel=false,lepton_channel=false;
                if(ele_reco_scale==0) ele_reco_scale=1; if(ele_id_scale==0) ele_id_scale=1;
                if(muon_id_scale==0) muon_id_scale=1; if(muon_iso_scale==0) muon_iso_scale=1;
		if(isChannel=="emu") 
			lepton_channel= ((HLT_emu1||HLT_emu2||HLT_emu3||HLT_emu4) && channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && ptlep1>25 && ptlep2>15 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.5) && lep1_charge*lep2_charge<0 && drll>0.5;
		if(isBarrel==1)  photon_channel=( (fabs(photoneta) < 1.4442) );
		else if(isBarrel==0) photon_channel= ( fabs(photoneta) < 2.5 && fabs(photoneta)>1.566 );
		else photon_channel=( (fabs(photoneta) < 1.4442) || ( fabs(photoneta) < 2.5 && fabs(photoneta)>1.566 ));
		if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && mll >20 && ptll > 30 && lepton_flag && n_photon>0  && photonet > 20. && drl1a>0.5 && drl2a>0.5 && photon_channel && photon_flag==1 && PuppiMET_T1Smear_pt > 20 && n_bjets_nom==0 && njets30<=1 && mT>40 && mT2>20 && drl1a>0.8 && drl2a>0.8 && drll<2.3 && dphill<2.3){
			if(gen_weight>0) npp++;
			if(gen_weight<0) nmm++;
			numbe_out++;
