void fX0_parameterization_mu(int index){
        ...
        TString dir6="/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2016/muon/hist/hist_mu_"; 
        TString dir7="/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2017/muon/hist/hist_mu_"; 
        TString dir8="/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2018/muon/hist/hist_mu_"; 
        TString dir66="/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2016/ele/hist/hist_ele_"; 
        TString dir77="/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2017/ele/hist/hist_ele_"; 
        TString dir88="/afs/cern.ch/user/y/yian/work/PKU-Cluster/AQGC/2018/ele/hist/hist_ele_"; 
        TFile*f6=new TFile(dir6+name+".root");
        TFile*f7=new TFile(dir7+name+".root");
        TFile*f8=new TFile(dir8+name+".root");
        TFile*f66=new TFile(dir66+name+".root");
        TFile*f77=new TFile(dir77+name+".root");
        TFile*f88=new TFile(dir88+name+".root");
        std::ofstream ParamSetf ;
//      ParamSetf.open("paramsets_fT0_mu.txt");
        if(index==1) {ParamSetf.open("paramsets_fM0.txt"); }
        if(index==2) {ParamSetf.open("paramsets_fM1.txt"); }
        if(index==3) {ParamSetf.open("paramsets_fM2.txt"); }
        if(index==4) {ParamSetf.open("paramsets_fM3.txt"); }
        if(index==5) {ParamSetf.open("paramsets_fM4.txt"); }
        if(index==6) {ParamSetf.open("paramsets_fM5.txt"); }
        if(index==7) {ParamSetf.open("paramsets_fM6.txt"); }
        if(index==8) {ParamSetf.open("paramsets_fM7.txt"); }
        if(index==9) {ParamSetf.open("paramsets_fT0.txt"); }
        if(index==10){ParamSetf.open("paramsets_fT1.txt");}
        if(index==11){ParamSetf.open("paramsets_fT2.txt");}
        if(index==12){ParamSetf.open("paramsets_fT5.txt");}
        if(index==13){ParamSetf.open("paramsets_fT6.txt");}
        if(index==14){ParamSetf.open("paramsets_fT7.txt");}
        if(index==15){ParamSetf.open("paramsets_fT8.txt");}
        if(index==16){ParamSetf.open("paramsets_fT9.txt");}

        double ZGbin[7] = {150,300,450,600,750,1000,2e4};
        const int num=6;
	TFile* fout = new TFile("signal_proc_"+name+".root", "RECREATE");
        TGraph*g6[num];TGraph*g7[num];TGraph*g8[num];
        TGraph*g66[num];TGraph*g77[num];TGraph*g88[num];
        for(int i=0;i<num;i++){
            g6[i]=(TGraph*)f6->Get(Form("bin%i",i));
            g7[i]=(TGraph*)f7->Get(Form("bin%i",i));
            g8[i]=(TGraph*)f8->Get(Form("bin%i",i));
            g66[i]=(TGraph*)f66->Get(Form("bin%i",i));
            g77[i]=(TGraph*)f77->Get(Form("bin%i",i));
            g88[i]=(TGraph*)f88->Get(Form("bin%i",i));
            double rf6[17]={0};double xf6[17];
            double rf7[17]={0};double xf7[17];
            double rf8[17]={0};double xf8[17];
            double rf66[17]={0};double xf66[17];
            double rf77[17]={0};double xf77[17];
            double rf88[17]={0};double xf88[17];
		    ...
		    g66[i]->GetPoint(j,xf66[j],rf66[j]);
		    g77[i]->GetPoint(j,xf77[j],rf77[j]);
		    g88[i]->GetPoint(j,xf88[j],rf88[j]);
               if( (xf6[j]-xf7[j])>1 || (xf6[j]-xf8[j])>1  || (xf8[j]-xf7[j])>1 ){
                   cout<<"bin "<<i<<endl;
                   cout<<"check the TGraph X-axis binning"<<endl;
                   break;
	         }
	       rf[j]=rf6[j]+rf7[j]+rf8[j]+rf66[j]+rf77[j]+rf88[j];xf[j]=xf6[j];
	    ...
	    leg->Draw("SAME") ;
	    cout <<"x9"<<endl;
	    if(index==1) c1->SaveAs(TString("fit_fM0")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==2) c1->SaveAs(TString("fit_fM1")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==3) c1->SaveAs(TString("fit_fM2")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==4) c1->SaveAs(TString("fit_fM3")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==5) c1->SaveAs(TString("fit_fM4")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==6) c1->SaveAs(TString("fit_fM5")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==7) c1->SaveAs(TString("fit_fM6")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==8) c1->SaveAs(TString("fit_fM7")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==9) c1->SaveAs(TString("fit_fT0")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==10) c1->SaveAs(TString("fit_fT1")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==11) c1->SaveAs(TString("fit_fT2")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==12) c1->SaveAs(TString("fit_fT5")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==13) c1->SaveAs(TString("fit_fT6")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==14) c1->SaveAs(TString("fit_fT7")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==15) c1->SaveAs(TString("fit_fT8")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    if(index==16) c1->SaveAs(TString("fit_fT9")+Form("_ZGbin_%u",i)+TString(".png")) ;
	    //              c1->SaveAs(TString("fit_fT0")+Form("_ZPTbin_%u",i)+TString(".png")) ;
