...
//     tree->SetBranchStatus("*",0);
     ...
     for(int j=0;j<kk;j++){
	     if(name.Contains("EWK"))
		     fout[j]= new TFile("./hist_"+vec_branchname[j]+"_Sig_scale"+tag+".root","recreate");
	     else
		     fout[j]= new TFile("./hist_"+vec_branchname[j]+"_qcd_scale"+tag+".root","recreate");
     }
     for(int j=0;j<kk;j++){
	     fout[j]->cd();
	     for(Int_t i=0;i<num;i++){
...
//		if(tag[i].Contains("17")==0) continue;
		...
		f1[i]=new TFile(dir[i]+"cutla-outZA-EWK"+tag[i]+".root");
		f2[i]=new TFile(dir[i]+"cutla-outZA"+tag[i]+".root");
		run(f2[i], recovars, bins,cut1,tag[i],9);

		run(f1[i], recovars, bins,cut1,tag[i],3);
	}
	return 1;
}
