...
f_ZA = TFile.Open("ZA_total_hist.root")
f_plj = TFile.Open("plj_weight_hist.root")
f_others = TFile.Open("others_hist.root")

th1_ZA_sig = f_sig.Get("ZA-EWK")
th1_ZA = f_ZA.Get("ZA_total")
th1_non_prompt = f_plj.Get("plj_weight")
th1_others = f_others.Get("others")
...
for i in range(1,6):
   f = open('./txt/%s_bin_%d.txt'%('all_2016', i),'w')
...
# bin error
   ...
   if non_prompt_bincontent==0:
       f.write('fake_%s_%s\tlnN\t-\t-\t-\t-\t#0. uncertainty on \n'%('all','16'))
   else: 
       f.write('fake_%s_%s\tlnN\t-\t%0.2f\t-\t-\t#0. uncertainty on all_16\n'%('all','16',fake[i-1]))
   f.write('VBS_stat_%s_%s_bin_%d\tlnN\t%0.2f\t-\t-\t-\n'%('all','16',i,ZA_sig_binerror))
   f.write('QCD_stat_%s_%s_bin_%d\tlnN\t-\t%0.2f\t-\t-\n'%('all','16',i,ZA_binerror))
   f.write('non_prompt_stat_%s_%s_bin_%d\tlnN\t-\t-\t%0.2f\t-\n'%('all','16',i,non_prompt_binerror))
   f.write('others_stat_%s_%s_bin_%d\tlnN\t-\t-\t-\t%0.2f\n'%('all','16',i,others_binerror))
   ...
   f.write('mu16_trigger\tlnN\t1.01\t1.01\t-\t1.01\n')
   f.write('mu16_efficiency\tlnN\t1.006\t1.006\t-\t1.006\n')
   f.write('ele16_ID\tlnN\t1.03\t1.03\t-\t1.03\n')
   f.write('ele16_reco\tlnN\t1.005\t1.005\t-\t1.005\n')
