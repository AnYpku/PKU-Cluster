#define pi 3.1415926
void run(TString sample,TString tag, TString cut1,TString cut2,TString cut3,TString Reco,TString channel){
     ...
     if(channel.Contains("elebarrel")){
	     ...
	     cut0="(genlep==11&&fabs(genphotoneta)<1.4442)";
     }
     if(channel.Contains("mubarrel")){
             cut="(lep==13&&fabs(photoneta)<1.4442)";
	     cut0="(genlep==13&&fabs(genphotoneta)<1.4442)";
     }
     if(channel.Contains("eleendcap")){
	     cut="(lep==11&&fabs(photoneta)<2.5&&fabs(photoneta)>1.566)";
	     cut0="(genlep==11 && (fabs(genphotoneta)<2.5&&fabs(genphotoneta)>1.566) )";
     }
     if(channel.Contains("muendcap")){
             cut="(lep==13&&fabs(photoneta)<2.5&&fabs(photoneta)>1.566)";
	     cut0="(genlep==13 && (fabs(genphotoneta)<2.5&&fabs(genphotoneta)>1.566) )";
     }
     cut1="(("+cut1+")&&("+cut+")&&"+cut0+")";
     cut2="(("+cut2+")&&("+cut+")&&"+cut0+")"; // select ele/muon channel and photon barrel/endcap
     Reco="(("+Reco+")&&("+cut+"))";
     cut3="(("+cut3+")&&("+cut0+"))";

     TTreeFormula *tformula=new TTreeFormula("formula",Reco, tree);  // reco
     TTreeFormula* tformula1=new TTreeFormula("formula1", cut1, tree); //reco && gen
     TTreeFormula* tformula2=new TTreeFormula("formula2", cut2, tree); // reco && !gen
     TTreeFormula* tformula3=new TTreeFormula("formula3", cut3, tree); //gen

     double muon1_id_scale,muon2_id_scale,muon1_iso_scale,muon2_iso_scale,ele1_id_scale,ele2_id_scale,ele1_reco_scale,ele2_reco_scale,photon_id_scale,photon_veto_scale,pileupWeight,prefWeight,muon1_track_scale,muon2_track_scale,ele_hlt_scale,muon_hlt_scale;
...
//             if(i%1000==0) cout<<"actualWeight "<<actualWeight<<endl;
	     ...
	     if (  tformula->EvalInstance() &&sample.Contains("EWK")==0 ){//reco for bkg 
		     if(Mjj<2000&deltaetajj<6.5)
     ...
     int p=0;
     TFile*fout=new TFile("./2droot/hist_"+sample+"_2d_"+tag+channel+".root","recreate");
...
int Build_Unfold2DHist_sig(){
	...
	for(int i=0;i<tag.size();i++){
		...
		if(tag[i].Contains("18")) continue;
		for(int j=0;j<sample.size();j++){
			cout<<tag[i]<<" "<<sample[j]<<endl;
			for(int k=0;k<channels.size();k++){
				if(sample[j].Contains("EWK"))
					run(sample[j],tag[i],cut1,cut2,cut3,Reco,channels[k]);
				else
					run(sample[j],tag[i],Reco,Reco,Reco,Reco,channels[k]);
			}
		}
	}
	return 1;
