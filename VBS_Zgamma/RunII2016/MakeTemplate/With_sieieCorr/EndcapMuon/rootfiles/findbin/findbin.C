...
void findbin::Loop()
{
   ...
   for(Int_t k=0;k<num;k++){
	   h1[k] = new TH1D(Form("h_%i",k+1),"",20000,low,high);
   }
...
//      if(jentry%2000==0) cout<<jentry<<";"<<nentries<<endl;
    ...
    for(Int_t k=0;k<num;k++){
	    ...
	    if(photon_chiso[position]<high && photon_chiso[position]>low && photon_pt[position]<highpt[k] && photon_pt[position]>lowpt[k]){
