...
TFile* fdata = TFile::Open("../root/Data_template1-DEle17.root");
TFile* ftrue = TFile::Open("../root/True_template1-ZA17.root");
//TFile* ftrue = TFile::Open("../root/True_template1-cutlep-outZA-EWK.root");
//TFile* ftrue = TFile::Open("../root/True_template1-cutlep-outTTA.root");
TFile* ffake = TFile::Open("../root/Fake_template1-DEle17.root");
...
//            double chi2ToNdf = chi2;
		double chi2ToNdf = chi2/Ndf;
	        ...
	        double fit_err = mcp0->Integral(0, mcp0->FindFixBin(0.0272)-1)*result->Integral()*errP0/mcp0->Integral();
            fr = p0* (hfake->Integral(0,hfake->FindFixBin(0.0272)-1)/hfake->Integral())/(result->Integral(0,result->FindFixBin(0.0272)-1)/result->Integral());

            fakeValue = mcp0->IntegralAndError(0,nBins,fakeError); 
            fake_window = mcp0->IntegralAndError(0,mcp0->FindFixBin(0.0272)-1,fake_window_Error);
	        real_window = mcp1->IntegralAndError(0,mcp1->FindFixBin(0.0272)-1,real_window_Error);
            data_window = result->IntegralAndError(0,result->FindFixBin(0.0272)-1,data_window_Error);
...
//            fr = p0* (hfake->Integral(1,hfake->FindFixBin(0.03001))/hfake->Integral())/(hdata->Integral(1,hdata->FindFixBin(0.03001))/hdata->Integral());
            ...
            ofstream myfile1(dir+TString("txt/fakerate_") +name+ Form("_pt%0.f_%0.f.txt", lowpt, highpt),ios::out);
            myfile1<<fr<<endl;
            ...
            myfile <<fr<<"\t"<<fr_Error<<"\t"<<hfake->Integral(0,hfake->FindFixBin(0.0272)-1)<<"\t"<<fakeValue<<"\t"<<fake_window<<endl;
...
//	    file2<<Form("%0.f<pt<%0.f ",lowpt,highpt)<<"\t\t"<<Form("%0.f<chiso<%0.f",lowchiso,highchiso)<<"\t"<<fixed<<setprecision(2)<< mcFake<<"\t"<<fixed<<setprecision(2)<<fake_window<<"\t"<<fixed<<setprecision(2)<<mcFake_err<< fixed<<setprecision(2)<<"\t"<< fake_window_Error<<fixed<<setprecision(2)<<"\t"<<fit_err<<endl;
        ...
        c1->Print(dir + Form("eps/pt%0.f_%0.f.pdf",lowpt,highpt));
