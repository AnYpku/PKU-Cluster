...
def merge_year(th1,sample):
      ...
      for i in range(1,n):
          ...
          if th2.GetBinContent(i)<0:
             th2.SetBinContent(i,0)
          if th3.GetBinContent(i)<0:
             th3.SetBinContent(i,0)
      th1.Add(th2)
      th1.Add(th3)
...
f_interf = TFile.Open(fdir+'hist_ZA_interf16'+sys.argv[2]+'.root')
...
if sys.argv[2].find("ele") == -1:
   f_Data=TFile.Open(fdir+'hist_Muon16'+sys.argv[2]+'.root')
else:
   f_Data=TFile.Open(fdir+'hist_Ele16'+sys.argv[2]+'.root')

th1_ZA_sig=f_EW.Get('hist')

th1_ZA=f_ZA.Get('hist')
th1_interf=f_interf.Get('hist')
th1_ZA.Add(th1_interf)

th1_non_prompt=f_plj.Get('hist')
th1_others=f_others.Get('hist')
th1_Data=f_Data.Get('hist')
...
if sys.argv[2].find("ele") == -1:
   merge_year(th1_Data,'Muon')
else:
   merge_year(th1_Data,'Ele')
...
#   bin_content = others_bincontent+non_prompt_bincontent+ZA_bincontent+ZA_sig_bincontent
   bin_content = th1_Data.GetBinContent(i)
