...
void unroll_name(TString filename){
	...
	TFile* fout = new TFile("aa_"+filename+"_jes.root","RECREATE");
	Double_t ZA_scale= 1;
	std::ostringstream strs;
	std::string lumivalue = strs.str();

	TFile* f_ZA;TFile* f_ZA1;TFile* f_ZA2;
	f_ZA=new TFile("./hist_"+filename+"_jes16.root");
	f_ZA1=new TFile("./hist_"+filename+"_jes17.root");
	f_ZA2=new TFile("./hist_"+filename+"_jes18.root");
	TH1D* th1_ZA[num]; TH1D* th1_ZA1[num]; TH1D* th1_ZA2[num];
	TH1D* t_ZA[num];
	TCanvas* cc[num];
	TLegend *ll[num];
	for(Int_t i=0;i<num;i++){
		th1_ZA[i]=(TH1D*)f_ZA->Get(Form("hist_%d",i));
		th1_ZA1[i]=(TH1D*)f_ZA1->Get(Form("hist_%d",i));
		th1_ZA2[i]=(TH1D*)f_ZA2->Get(Form("hist_%d",i));
                th1_ZA[i]->Add(th1_ZA1[i],1);th1_ZA[i]->Add(th1_ZA2[i],1);
                const int nbins=th1_ZA[i]->GetNbinsX();
...
//        latex.DrawLatex(6.1,0.85*vec_ymax[0],"#Delta#eta_{jj}>6");
	...
	fPads2->Update();
	c1->Print("./figs/hist-2d_"+filename+"_full.eps");
	c1->Print("./figs/hist-2d_"+filename+"_full.pdf");
