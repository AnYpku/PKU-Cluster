...
void run(TString dir,TString name,TString cut1,TString cut2,TString tag,TString channel){
     ...
     TFile*file;
     if(name.Contains("EWK")==0 && name.Contains("ZA")==1)
...
//     if(tag.Contains("18"))lumi=Form("%f",58.7);cout<<lumi<<endl;
     ...
     for(int i=0;i<tree->GetEntries();i++){
	     ...
	     if(lep==13)       
             ...
             if(tag.Contains("17"))
		     actualWeight=scalef*pileupWeight*prefWeight*lumi/(muon1_track_scale*muon2_track_scale);
	...
	for(int k=0;k<tags.size()-1;k++){
		...
		for(int j=0;j<names.size();j++){     
			for(int i=0;i<channels.size();i++){
				if(names[j].Contains("EWK")==0 && names[j].Contains("ZA")==1){
					run(dir,"ZA",cut1,cut2,tags[k],channels[i]);
				}
				else{
