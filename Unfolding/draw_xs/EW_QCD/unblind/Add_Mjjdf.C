...
void run(TString var, TString recovar, TString title,TString tag){
        ...
        TFile*f14=new TFile("../../../pdf_draw/unfold_"+var+"_interf16.root");
        TFile*f24=new TFile("../../../pdf_draw/unfold_"+var+"_interf17.root");
        TFile*f34=new TFile("../../../pdf_draw/unfold_"+var+"_interf18.root");
        ...
        TH1D*hist_clone14=(TH1D*)f14->Get(var+"_0");
        TH1D*hist_clone24=(TH1D*)f24->Get(var+"_0");
        TH1D*hist_clone34=(TH1D*)f34->Get(var+"_0");
        hist_clone1->Add(hist_clone2,1);hist_clone1->Add(hist_clone3,1);
        hist_clone1->Add(hist_clone11,1);hist_clone1->Add(hist_clone22,1);hist_clone1->Add(hist_clone33,1);
        hist_clone1->Add(hist_clone14,1);hist_clone1->Add(hist_clone24,1);hist_clone1->Add(hist_clone34,1);
	...
	for(int i=0;i<hist_clone1->GetNbinsX();i++){
           hist_clone->SetBinContent(i+1,hist_clone1->GetBinContent(i+1));
           hist_clone->SetBinError(i+1,hist_clone1->GetBinError(i+1));
	}
	...
	double deltaEta[3]={2,1.5,0.5};
        ...
        ifstream f_in("/home/pku/anying/cms/PKU-Cluster/Unfolding/data_card/EW_QCD/combine/unblind/r_"+recovar+"_"+tag+".txt");
        ifstream f_sys("/home/pku/anying/cms/PKU-Cluster/Unfolding/data_card/EW_QCD/combine/breakdown/unblind/r_sys_"+recovar+"_"+tag+".txt");
        if(!f_in.is_open())cout<<"can not open the file "<<"/home/pku/anying/cms/PKU-Cluster/Unfolding/data_card/EW_QCD/combine/unblind/r_"+recovar+"_"+tag+".txt"<<endl;
        if(!f_sys.is_open())cout<<"/home/pku/anying/cms/PKU-Cluster/Unfolding/data_card/EW_QCD/combine/breakdown/unblind/r_sys_"+recovar+"_"+tag+".txt"<<endl;
...
//        cout<<"after scale "<<hist_clone->GetBinContent(1)<<" "<<hist_clone->GetBinError(1)<<endl;
	...
	fPads1->cd();
        ...
        hist_clone->GetYaxis()->SetTitleOffset(0.65);
	...
	for(int j=1;j<=h1[0]->GetNbinsX();j++){
        ...
        hist_clone->GetYaxis()->SetRangeUser(hist_clone->GetMinimum()*0.2,10);
...
//	const char *name[7]={"500~800","800~1200","1200~2000","500~800","800~1200","1200~2000","500~2000"};
	...
	hist_clone->Draw("E2");
        hist_clone->GetXaxis()->SetLabelSize(0);
        ...
        for(int i=0;i<hist_clone->GetNbinsX();i++){
           ...
           fout_txt<<var<<" bin"<<i<<" "<<fixed<<setprecision(2)<<r[i]<<" "<<Err_up[i]<<" "<<Err_down[i]<<" "<<fixed<<setprecision(4)<<hist_clone->GetBinContent(i+1)<<" "<<hist_clone->GetBinError(i+1)<<" "<<ybin[i]<<" "<<yerror_up[i]<<" "<<yerror_down[i]<<endl;
        }
        ...
        gr_sys->SetLineWidth(1);
        gr_sys->SetLineStyle(1);
        gr_sys->SetMarkerSize(1.2);
        gStyle->SetEndErrorSize(5);
        gr_sys->Draw("P SAME");

        double max=hist_clone->GetMaximum();
        TLine* vline1 = new TLine(hist_clone->GetBinLowEdge(4),0,hist_clone->GetBinLowEdge(4),max);
        TLine* vline2 = new TLine(hist_clone->GetBinLowEdge(7),0,hist_clone->GetBinLowEdge(7),0.5);
	...
	l1->Draw();
        ...
        latex.DrawLatex(0.2,0.0015,"#font[22]{2.5 #leq |#Delta#eta_{jj}| #leq 4.5}");
        latex.DrawLatex(3.6,0.0015,"#font[22]{4.5 < |#Delta#eta_{jj}| #leq 6}");
        latex.DrawLatex(6.1,0.056,"#font[22]{6 < |#Delta#eta_{jj}| #leq 6.5}");
...
//        fPads2->SetGridy();
	...
	nominal->SetTitle("");
	nominal->GetXaxis()->SetLabelSize(0.19);
...
//        nominal->GetYaxis()->SetTitleFont(22);
	...
	nominal->GetYaxis()->SetTitleOffset(0.58);
...
//	for (int i = 1; i<= hist_nomNoErr->GetNbinsX(); ++i){hist_nomNoErr->SetBinError(i,0);}
	hist_nominal->Divide(nomNoErr);
        ...
        cout<<"high="<<ye_h[pos_h]<<"; low="<<ye_h[pos_l]<<endl;
        nominal->GetYaxis()->SetRangeUser(ye_h[pos_l]-ye_l[pos_l]-0.3,3);
	nominal->GetYaxis()->SetRangeUser(0.27,1.7);
...
//	line->Draw();
	...
	fPads1->cd();
	fPads1->Update();
	c1->Print(var+"EWQCD_unfold_df.pdf");
