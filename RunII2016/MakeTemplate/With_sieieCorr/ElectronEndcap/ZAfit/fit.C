...
//TFile* ftrue = TFile::Open("../root/True_template1-outZA.root");
TFile* ftrue = TFile::Open("../root/True_template1-outZA-EWK.root");
...
//	TH1F* htrue_ = (TH1F*)ftrue->Get(Form("h1_pt%0.f-%0.f",lowpt,highpt));
	...
	hfake_->Add(hzaf_,-35.8);

	Int_t nBins = 10;
	Int_t sieie_bin=4;
	Double_t bins[11];
	for (Int_t i=0;i<(nBins+1);i++){
		bins[i] = 0.0072 + 0.005*i;
        }
	...
	RooRealVar sieie("sieie", "sieie", 0.0072, 0.0572);
...
//	RooRealVar nTrue("true number", "true number",800,0, nData);
	...
	sieie.setRange("window",0.0072,0.0272);
