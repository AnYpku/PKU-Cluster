...
void run(TString sample,TString vec_branchname,TString reco,vector<double> bins,TString cut1[num],int kk,TString tag){
     ...
     if(sample.Contains("ZA"))
	     file=new TFile("/home/pku/anying/cms/rootfiles/JESR/unfold_GenCutla-JESR_"+sample+tag+".root");
...
//					     cout<<k<<" "<<iy<<" "<<ix<<" "<<ix+(iy)*3<<endl;
     ...
     for(int j=0;j<kk;j++){
	     if(vec_branchname.Contains("Mjj"))
		     fout[j]=new TFile("./unfold_"+sample+"_2d"+vec_branchname+Form("_recobin%d",j+1)+"_jer"+tag+".root","recreate");
	     else
...
//		if(tags[k].Contains("16")==1) continue;
		for(int i=0;i<recovars.size();i++){
//                      if(recovars[i].Contains("Mjj")==0) continue;
