...
void EDBRHistoMaker::createAllHistos(TString isChannel) {
	...
	hs.setHisto("mT2_pf", 10, 50, 200);
	hs.setHisto("mT_puppi", 10, 50, 250);
	hs.setHisto("mT2_puppi", 10, 50, 200);
...
//        nentries=100;
	...
	double pfmet,pfmetphi,puppimet,puppimetphi;
	float sum=0;
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		actualWeight=1;
			...
			HLT= !(HLT_emu) && !(HLT_mm) && HLT_ee;
                ...
                if(isChannel=="emu") 
			lepton_channel= ( HLT && channel==1 && fabs(lep1_pid)==13 && fabs(lep2_pid)==11 && lep1pt>20 && lep2pt>25 && fabs(lep1eta) < 2.4 && fabs(lep1eta) < 2.5) && drll>0.5 && Filter;
		...
		if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && lep1_is_tight==1 && lep2_is_tight==1 && mll > 20 && ptll > 30 && /*PuppiMET_pt > 20 && mT_puppi > 50 && mT2_puppi > 0*/ MET_T1_pt > 20 && /*n_bjets20_medium_deepcsv_pc==0*/ n_bjets20_medium_deepFlavB_pc==0){
...
//	nentries=100;
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		bool photon_channel=false,lepton_channel=false;
	        ...
	        if( lepton_channel && n_loose_ele==1 && n_loose_mu==1 && mll > 20 && ptll > 30 && lepton_flag && /*PuppiMET_pt > 20 && mT_puppi > 50 && mT2_puppi > 0 &&*/ MET_T1Smear_pt > 20 && /*n_bjets20_medium_deepcsv_pc == 0*/ n_bjets20_medium_deepFlavB_pc==0){
			if(gen_weight>0) npp++;
			if(gen_weight<0) nmm++;
			numbe_out++;
			treename->Fill();
