void run(TString sample,TString tag){
        fstream ftxt("./jes_uncer"+tag+"CR.txt", ios::app );
	ofstream f1("./"+sample+"_jes_uncer"+tag+"CR.txt");
	TFile*file;
	if(sample.Contains("EWK"))
		file = new TFile("./root/hist_"+tag+"outJEC_"+sample+"_jes"+tag+"CR.root");
        else file=new TFile("./root/hist_"+sample+"_jes"+tag+"CR.root");
