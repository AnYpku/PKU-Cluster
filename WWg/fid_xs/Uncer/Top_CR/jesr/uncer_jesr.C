void run(TString tag,TString sample,TString type,TString vars,TString njets,bool flag_sig){
	...
	file= new TFile("./root/hist_"+sample+"_"+vars+"_"+njets+"_"+type+tag+".root");
        TString mc;
        if(flag_sig)mc="sig";else mc="sig_out";
	cout<<type<<endl;
...
int uncer_jesr(){
	...
	vector<TString> sample={"WWG_emu"};
	vector<TString> njets={"0jets","1jets","2jets"};
	for(int i=0;i<njets.size();i++){
		for(int j=1;j<tag.size();j++){
			for(int k=0;k<sample.size();k++){
				for(int n=0;n<vars.size();n++){
					run(tag[j],sample[k],"jer1",vars[n],njets[i],1);
					run(tag[j],sample[k],"jer1",vars[n],njets[i],0);
					run(tag[j],sample[k],"jesTotal",vars[n],njets[i],1);
					run(tag[j],sample[k],"jesTotal",vars[n],njets[i],0);
				}
			}
		}
	}
