...
// Setup names of data files for trees.
	...
	std::cout << "set data imformation, we have " << nDATA << "data file"
			<< std::endl;
	std::string dataLabels[nDATA] = { "Muon16","Ele16" };
...
// set mcsig information
	...
	for (int i = 0; i < nDATA; i++) {
		...
		sprintf(out_buffer, "./output-slimmed-rootfiles/optimal_aqgc_%s.root", dataLabels[i].c_str());
...
//		std::cout<<"OK"<<std::endl;
	...
	for (int i = 0; i < nMC; i++) {
		...
		sprintf(out_buffer, "./output-slimmed-rootfiles/optimal_aqgc_%s.root", mcLabels[i].c_str());
...
//			treeMC= (TTree*) fileMC->Get("ZPKUCandidates");
	...
	for (int  i = 0; i < nMCSig; i++) {
		...
		sprintf(out_buffer, "./output-slimmed-rootfiles/optimal_aqgc_%s.root", mcLabelsSig[i].c_str());
