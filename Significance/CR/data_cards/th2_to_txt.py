...
fdir = '/afs/cern.ch/user/y/yian/work/PKU-Cluster/Significance/CR/root/'
...
genbinerror=[]
arr={}
f=open('/afs/cern.ch/user/y/yian/work/PKU-Cluster/Significance/CR/Uncer/summary_uncer_'+sys.argv[2]+sys.argv[1]+'CR.txt')
import re
import numpy as np
for line in f:
        if not line.strip():
...
# bin error
   ...
   data= ZA_sig_bincontent+ ZA_bincontent+non_prompt_bincontent+TTA_bincontent+VV_bincontent+ST_bincontent+WA_bincontent
   ...
   f.write('process\t')
   f.write('Sig\tQCD\tnon_prompt\tTTA\tVV\tST\tWA\n')
   ...
   f.write('%0.2f\t%0.2f\t-\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n'%(arr['jes'+sys.argv[1]+'_ZA-EWK'][i-1],arr['jes'+sys.argv[1]+'_ZA'][i-1],arr['jes'+sys.argv[1]+'_TTA'][i-1],arr['jes'+sys.argv[1]+'_VV'][i-1],arr['jes'+sys.argv[1]+'_ST'][i-1],1.0))

   f.write('JER_%s\tlnN\t'%(sys.argv[1]))
   f.write('%0.2f\t%0.2f\t-\t%0.2f\t%0.2f\t%0.2f\t%0.2f\n'%(arr['jer'+sys.argv[1]+'_ZA-EWK'][i-1],arr['jer'+sys.argv[1]+'_ZA'][i-1],arr['jer'+sys.argv[1]+'_TTA'][i-1],arr['jer'+sys.argv[1]+'_VV'][i-1],arr['jer'+sys.argv[1]+'_ST'][i-1],1.0))
#
   f.write('pdf_EW\tlnN\t')
   f.write('%0.2f\t-\t-\t-\t-\t-\t-\n'%(arr['Sig_pdf'][i-1]))
#
