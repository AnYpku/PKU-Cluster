#define num 3
#define pi 3.1415926
void run( TString sample,vector<TString> vec_branchname,vector<vector<double>> bins,TString cut1,TH1D* th1[vec_branchname.size()]){
     const int kk = vec_branchname.size();
     TString dir1="/eos/user/y/yian/2018cutla/";     
...
//     TH1D*th1[kk];
     TString th1name[kk];
     for(int j=0;j<kk;j++){
	     th1name[j]=vec_branchname[j]+sample;
	     th1[j] = new TH1D(th1name[j],th1name[j],bins[j].size()-1,&bins[j][0]);
	     th1[j]->Sumw2(); 
     }
     for(int k=0;k<tree->GetEntries();k++){
	     ...
	     if (  tformula->EvalInstance() ){
			     for(int j=0;j<kk;j++){
				     th1[j]->Fill(variables[vec_branchname[j]],scalef);
			     }
	     }
     }
}
int Unfold_uncer_batch_bkg(){
     ...
     TH1D*hist[kk];TH1D*hist_up[kk];TH1D*hist_down[kk];
     run("",recovars, bins,cut1,hist);
     run("_up",recovars, bins,cut1,hist_up);
     run("_down",recovars, bins,cut1,hist_down);
     TFile*fout[kk];
     for(int j=0;j<kk;j++){
	    fout[j]= new TFile("unfold_"+recovars[j]+"_plj.root","recreate");
     }
     for(int j=0;j<kk;j++){
	     fout[j]->cd();
	     hist[j]->Write();
