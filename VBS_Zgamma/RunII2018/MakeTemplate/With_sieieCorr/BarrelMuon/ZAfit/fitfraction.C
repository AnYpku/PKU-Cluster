...
TFile* fdata = TFile::Open("../root/Data_template-DMuon18.root");
...
TFile* ffake = TFile::Open("../root/Fake_template-DMuon18.root");
...
//	    result->GetXaxis()->SetNdivisions(606);
            result->Draw();
            hdata->Draw("E P SAME");
...
//            double chi2ToNdf = chi2;
		double chi2ToNdf = chi2/Ndf;
	        ...
	        real_window = mcp1->IntegralAndError(0,mcp1->FindFixBin(0.01015),real_window_Error);
...
//	    file2<<Form("%0.f<pt<%0.f ",lowpt,highpt)<<"\t\t"<<Form("%0.f<chiso<%0.f",lowchiso,highchiso)<<"\t"<<fixed<<setprecision(2)<< mcFake<<"\t"<<fixed<<setprecision(2)<<fake_window<<"\t"<<fixed<<setprecision(2)<<mcFake_err<< fixed<<setprecision(2)<<"\t"<< fake_window_Error<<fixed<<setprecision(2)<<"\t"<<fit_err<<endl;
	    ...
	    textvalue->Draw();
            ...
            TString ptR=Form("%0.f < p_{T}^{#gamma} < %0.f",lowpt,highpt);
	    TLatex *texterror = new TLatex(0.6, 0.5, ptR);
	    texterror->SetNDC();
	    ...
	    leg->AddEntry(mcp1, "True #gamma", "L");
	    leg->AddEntry(mcp0, "Fake #gamma", "L");
	    leg->Draw("same");
	    CMS_lumi(fPad1, 4, 0., "59.7");
	    fPad1->Update();
	    c1->Print(dir + Form("eps/frac_pt%0.f_%0.f.pdf",lowpt,highpt));
