...
int Draw_fit(){
	...
	for(int j=0;j<tag.size();j++){
		...
		for(int i=0;i<channel.size();i++){
			...
			TH1F*h_ZA       =run(file,channel[i],tag[j],"ZA",index);
			...
			fout[j]->cd();
			h_ST->Write();h_ZA->Write();h_TTA->Write();h_non_prompt->Write();
			h_VV->Write();h_total->Write();h_Sig_out->Write();data->Write("data_"+channel[i]);
		}
	}
	file->Close();
	return 0;
