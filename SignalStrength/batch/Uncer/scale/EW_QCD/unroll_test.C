...
void unroll_name(TString sample,TString tag,int num,bool shape_turn=0){
	...
	for(Int_t i=0;i<num;i++){
		...
		double factor=t_ZA[0]->Integral()/t_ZA[p]->Integral();
//		if(sample.Contains("ewk")&&shape_turn) t_ZA[p]->Scale(factor);
		if(shape_turn) t_ZA[p]->Scale(factor);
...
int unroll_test(){
	...
	bool turn=0;
        for(int i=0;i<2;i++){
		turn=1;
//		if(sample[i].Contains("ewk")) turn=1;
                unroll_name(sample[i],"16",num[i],turn);
                unroll_name(sample[i],"17",num[i],turn);
                unroll_name(sample[i],"18",num[i],turn);
        }
