...
temp_nonprompt_2e_sel = []
temp_nonprompt_2mu_sel = []
temp_nonprompt_1e1mu_sel = []
...
if process.endswith('_ext'):
    ...
    for n_par, parquet_file in enumerate(parquet_list):
        ...
        temp_sel_jets = good_jets[_2lnj_sel]
        
        sel_leading_jet_sel = (ak.argsort(temp_sel_jets.pt,axis=1,ascending=False)==0)
        sel_subleading_jet_sel = (ak.argsort(temp_sel_jets.pt,axis=1,ascending=False)==1)
        
        sel_jets = temp_sel_jets[(sel_leading_jet_sel | sel_subleading_jet_sel)]
...
#         jet_btag_sel = np.sum(sel_jets.btagDeepFlavB>0.2783,axis=1)<1
        jet_btag_sel = np.sum(temp_sel_jets.btagDeepB>0.4184,axis=1)<1
...
#         jet_merged_sel = jet_anti_btag_sel & jet_mjj_sel & jet_deta_sel & jet_pt_sel
        ...
        temp_os_2e_sel.append(os_tight_2e_sel)
        temp_os_2mu_sel.append(os_tight_2mu_sel)
        temp_os_1e1mu_sel.append(os_tight_1e1mu_sel)
        ...
        temp_test_2e_sel.append(test_tight_2e_sel)
        temp_test_2mu_sel.append(test_tight_2mu_sel)
        temp_test_1e1mu_sel.append(test_tight_1e1mu_sel)
        ...
        temp_nonprompt_2e_sel.append(nonprompt_2e_sel)
        temp_nonprompt_2mu_sel.append(nonprompt_2mu_sel)
        temp_nonprompt_1e1mu_sel.append(nonprompt_1e1mu_sel)
    ...
    ak.to_parquet(ak.concatenate(mll_array_list),f"{args.output_dir}/mll_{process}.parquet")
    if isdata==False:
        ak.to_parquet(ak.concatenate(mll_weight_list)*mc_weight,f"{args.output_dir}/mll_weight_{process}.parquet")
    ak.to_parquet(ak.concatenate(temp_nonprompt_mll_array_list),f"{args.output_dir}/mll_{process}_nonprompt.parquet")
    ak.to_parquet(ak.concatenate(temp_nonprompt_mll_weight_list)*mc_weight,f"{args.output_dir}/mll_weight_{process}_nonprompt.parquet")
    ak.to_parquet(ak.concatenate(temp_wrong_sign_mll_array_list),f"{args.output_dir}/mll_{process}_wrong_sign.parquet")
    ak.to_parquet(ak.concatenate(temp_wrong_sign_mll_weight_list)*mc_weight,f"{args.output_dir}/mll_weight_{process}_wrong_sign.parquet")
    ak.to_parquet(ak.concatenate(mjj_array_list),f"{args.output_dir}/mjj_{process}.parquet")
    if isdata==False:
        ak.to_parquet(ak.concatenate(mjj_weight_list)*mc_weight,f"{args.output_dir}/mjj_weight_{process}.parquet")
    ak.to_parquet(ak.concatenate(temp_nonprompt_mjj_array_list),f"{args.output_dir}/mjj_{process}_nonprompt.parquet")
    ak.to_parquet(ak.concatenate(temp_nonprompt_mjj_weight_list)*mc_weight,f"{args.output_dir}/mjj_weight_{process}_nonprompt.parquet")
    ak.to_parquet(ak.concatenate(temp_wrong_sign_mjj_array_list),f"{args.output_dir}/mjj_{process}_wrong_sign.parquet")
    ak.to_parquet(ak.concatenate(temp_wrong_sign_mjj_weight_list)*mc_weight,f"{args.output_dir}/mjj_weight_{process}_wrong_sign.parquet")
    
    ak.to_parquet(ak.concatenate(temp_os_2e_sel), f"{args.output_dir}/os_2e_{process}.parquet")
    ak.to_parquet(ak.concatenate(temp_os_2mu_sel), f"{args.output_dir}/os_2mu_{process}.parquet")
    ak.to_parquet(ak.concatenate(temp_os_1e1mu_sel), f"{args.output_dir}/os_1e1mu_{process}.parquet")
    
    ak.to_parquet(ak.concatenate(temp_test_2e_sel), f"{args.output_dir}/ss_2e_{process}.parquet")
    ak.to_parquet(ak.concatenate(temp_test_2mu_sel), f"{args.output_dir}/ss_2mu_{process}.parquet")
    ak.to_parquet(ak.concatenate(temp_test_1e1mu_sel), f"{args.output_dir}/ss_1e1mu_{process}.parquet")
    
    ak.to_parquet(ak.concatenate(temp_nonprompt_2e_sel), f"{args.output_dir}/ss_2e_nonprompt_{process}.parquet")
    ak.to_parquet(ak.concatenate(temp_nonprompt_2mu_sel), f"{args.output_dir}/ss_2mu_nonprompt_{process}.parquet")
    ak.to_parquet(ak.concatenate(temp_nonprompt_1e1mu_sel), f"{args.output_dir}/ss_1e1mu_nonprompt_{process}.parquet")
