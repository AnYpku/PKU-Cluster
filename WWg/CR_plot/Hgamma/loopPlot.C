...
void loopPlot(int isBarrel,TString isChannel,std::string year) {
	...
	double lumiValue; 
        if(year=="18")     {lumiValue=59.7;}
	else if(year=="17"){lumiValue=41.5;}
	else if(year=="16"){lumiValue=35.9;}
...
//	vector<TString> pathToTrees = {"/home/pku/anying/cms/rootfiles/WWg/20"+year+"/old17/"};
	vector<TString> pathToTrees = {"/home/pku/anying/cms/rootfiles/WWg/Hgamma/20"+year+"/"};
//	vector<TString> pathToTrees = {"./rootfiles/"};
...
// Setup names of data files for trees.
	...
	for (int j = 0; j< pathToTrees.size(); j++) {
		for (int ii = 0; ii < nDATA; ii++) {
			...
			fData.push_back(pathToTrees[j] +"cutla-out"+ data[ii] + year+".root");
		}
	}
	...
	std::cout << "set data imformation, we have " << nMC << "mc file"
		<< std::endl;
        std::string mc[nMC] ={"plj","fakeL","VV","ST","ZGJets","TGJets","TTGJets","WGJets","tZq","WWG_emu","HGS","HGU","HGD"};
	std::vector< TString > fMC;
	for (int ii = 0; ii < nMC; ii++) {
		fMC.push_back(pathToTrees[0] +"cutla-out"+ mc[ii] + year+".root");
		cout<<pathToTrees[0]<<"cutla-out"<<mc[ii] +year<<".root"<<endl;
	}
	const int nmc=fMC.size();
	std::string bkgLabels[nmc] ={"plj","fakeL","VV","tW","QCD Z#gamma","t#gamma","t#bar{t}#gamma","QCD W#gamma","tZq","WW#gamma","H#gamma(s)","H#gamma(u)","H#gamma(d)"};
	std::string mcLabels[nmc] ={"plj","fakeL","VV","ST","ZGJets","TGJets","TTGJets","WGJets","tZq","WWG_emu","HGS","HGU","HGD"};
	double kFactorsMC_array[nmc] ={1,1,lumiValue,lumiValue,lumiValue,lumiValue,lumiValue,lumiValue,lumiValue,lumiValue,lumiValue,lumiValue,lumiValue};
	...
	std::cout << "set data imformation, we have " << nMCSig << "mcsig file"
		<< std::endl;
	std::string mcSig[nMCSig] = { "HGC_NLO"};
	...
	std::string mcLabelsSig[nmcsig] = { "HGC"};
...
//		continue;
		...
		TString tag,sample;
		if(fData.at(i).Contains("16")) {tag="16";rc.init("roccor/RoccoR2016bUL.txt");lumiValue=35.9; }
...
//		continue;
		...
		TString tag;
		if(fMC.at(i).Contains("16")) {tag="16";rc.init("roccor/RoccoR2016bUL.txt"); lumiValue=35.9;}
...
//		continue;
		...
		TString tag;
		if(fMCSig.at(i).Contains("16")) {tag="16";rc.init("roccor/RoccoR2016bUL.txt"); lumiValue=35.9;}
	...
	if (nMC > 0) {
		...
		sprintf(buffer, "./output-slimmed-rootfiles/histos_"+isChannel+channel+"_%s%s.root", mcLabels[0].c_str(),year.c_str());
